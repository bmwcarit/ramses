...
namespace ramses_internal
{
    ...
    int SceneViewer::loadAndRenderScene(const std::string& sceneFile)
    {
        ...
        auto loadedScene = client->loadSceneFromFile(sceneFile);
        ...
        SceneViewerGui gui(*loadedScene, sceneFile, imguiHelper);
    ...
    void SceneViewer::validateContent(const ramses::Scene& scene) const
    {
        ...
        const auto reportLevel = (m_guiMode == GuiMode::Off) ? ramses::EValidationSeverity::Info : ramses::EValidationSeverity::Warning;
        ...
        if (!m_validationOutput.empty())
        {
            ...
            validationFile << scene.getValidationReport(ramses::EValidationSeverity::Info) << std::endl;
            // dump unused objects
