...
namespace ramses::internal
{
    ...
    TYPED_TEST_P(AWindowWaylandWithEventHandling, singleKeyPressEventTriggersKeyPressedEventWithCorrectKeyCode)
    {
        ...
        this->testKeyCode(KEY_LEFTSHIFT,      EKeyCode_ShiftLeft,     EKeyModifier::Shift);
        this->testKeyCode(KEY_RIGHTALT,       EKeyCode_AltRight,      EKeyModifier::Alt);
    }
    ...
    TYPED_TEST_P(AWindowWaylandWithEventHandling, leftMouseButtonDownEventTriggersLeftButtonDownEvent)
    {
        ...
        EXPECT_CALL(this->m_eventHandlerMock, onMouseEvent(EMouseEvent::Move, _, _)).Times(2);
        EXPECT_CALL(this->m_eventHandlerMock, onMouseEvent(EMouseEvent::LeftButtonDown, _, _)).Times(1);
    ...
    TYPED_TEST_P(AWindowWaylandWithEventHandling, leftMouseButtonUpEventTriggersLeftButtonUpEvent)
    {
        ...
        EXPECT_CALL(this->m_eventHandlerMock, onMouseEvent(EMouseEvent::Move, _, _)).Times(3);
        Sequence seq;
        EXPECT_CALL(this->m_eventHandlerMock, onMouseEvent(EMouseEvent::LeftButtonDown, _, _)).Times(1).InSequence(seq);
        EXPECT_CALL(this->m_eventHandlerMock, onMouseEvent(EMouseEvent::LeftButtonUp, _, _)).Times(1).InSequence(seq);
        this->processAllEvents();
    }
    ...
    TYPED_TEST_P(AWindowWaylandWithEventHandling, rightMouseButtonDownEventTriggersRightButtonDownEvent)
    {
        ...
        EXPECT_CALL(this->m_eventHandlerMock, onMouseEvent(EMouseEvent::Move, _, _)).Times(2);
        EXPECT_CALL(this->m_eventHandlerMock, onMouseEvent(EMouseEvent::RightButtonDown, _, _)).Times(1);
        this->processAllEvents();
    ...
    TYPED_TEST_P(AWindowWaylandWithEventHandling, rightMouseButtonUpEventTriggersRightButtonUpEvent)
    {
        ...
        EXPECT_CALL(this->m_eventHandlerMock, onMouseEvent(EMouseEvent::Move, _, _)).Times(3);
        Sequence seq;
        EXPECT_CALL(this->m_eventHandlerMock, onMouseEvent(EMouseEvent::RightButtonDown, _, _)).Times(1).InSequence(seq);
        EXPECT_CALL(this->m_eventHandlerMock, onMouseEvent(EMouseEvent::RightButtonUp, _, _)).Times(1).InSequence(seq);
        this->processAllEvents();
    }
    ...
    TYPED_TEST_P(AWindowWaylandWithEventHandling, middleMouseButtonDownEventTriggersMiddleButtonDownEvent)
    {
        ...
        EXPECT_CALL(this->m_eventHandlerMock, onMouseEvent(EMouseEvent::Move, _, _)).Times(2);
        EXPECT_CALL(this->m_eventHandlerMock, onMouseEvent(EMouseEvent::MiddleButtonDown, _, _)).Times(1);
        this->processAllEvents();
    ...
    TYPED_TEST_P(AWindowWaylandWithEventHandling, middleMouseButtonUpEventTriggersMiddleButtonUpEvent)
    {
        ...
        EXPECT_CALL(this->m_eventHandlerMock, onMouseEvent(EMouseEvent::Move, _, _)).Times(3);
        Sequence seq;
        EXPECT_CALL(this->m_eventHandlerMock, onMouseEvent(EMouseEvent::MiddleButtonDown, _, _)).Times(1).InSequence(seq);
        EXPECT_CALL(this->m_eventHandlerMock, onMouseEvent(EMouseEvent::MiddleButtonUp, _, _)).Times(1).InSequence(seq);
        this->processAllEvents();
    }
    ...
    TYPED_TEST_P(AWindowWaylandWithEventHandling, positiveMouseWheelEventTriggersMouseWheelUpEvent)
    {
        ...
        EXPECT_CALL(this->m_eventHandlerMock, onMouseEvent(EMouseEvent::WheelUp, _, _)).Times(1);
        this->processAllEvents();
    }
    ...
    TYPED_TEST_P(AWindowWaylandWithEventHandling, negativeMouseWheelEventTriggersMouseWheelDownEvent)
    {
        ...
        EXPECT_CALL(this->m_eventHandlerMock, onMouseEvent(EMouseEvent::WheelDown, _, _)).Times(1);
        this->processAllEvents();
    }
    ...
    TYPED_TEST_P(AWindowWaylandWithEventHandling, mouseMoveEventTriggersMouseMoveEvent)
    {
        ...
        EXPECT_CALL(this->m_eventHandlerMock, onMouseEvent(EMouseEvent::Move, 5, 0)).Times(1);
        this->processAllEvents();
    }
