...
namespace ramses::internal
{
    ...
    public:
        void SetUp() override
        {
            ...
            EXPECT_CALL(this->m_eventHandlerMock, onMouseEvent(EMouseEvent::Move, _, _)).Times(AnyNumber());
            EXPECT_CALL(this->m_eventHandlerMock, onMouseEvent(EMouseEvent::LeftButtonDown, _, _)).Times(AnyNumber());
            EXPECT_CALL(this->m_eventHandlerMock, onMouseEvent(EMouseEvent::LeftButtonUp, _, _)).Times(AnyNumber());
            processAllEvents();
        }
        ...
        {
            deviceFd = open(deviceName, O_WRONLY); // NOLINT(cppcoreguidelines-pro-type-vararg)
            return (deviceFd >= 0);
        }
        ...
        void testKeyCode(uint32_t       virtualKeyCode,
                         ...
                         KeyModifiers expectedModifier = KeyModifiers())
        {
            sendKeyEvent(virtualKeyCode);
            EXPECT_CALL(this->m_eventHandlerMock, onKeyEvent(EKeyEvent::Pressed, expectedModifier, expectedRamsesKeyCode))
                .Times(1);
            EXPECT_CALL(this->m_eventHandlerMock,
                        onKeyEvent(EKeyEvent::Released, KeyModifiers(), expectedRamsesKeyCode))
                .Times(1);
            processAllEvents();
        }
