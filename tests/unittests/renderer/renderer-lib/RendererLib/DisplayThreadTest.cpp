...
namespace ramses::internal
{
    ...
    TEST_F(ADisplayThread, canStartStopUpdating)
    {
        ...
        EXPECT_CALL(m_displayBundleMock, doOneLoop(ELoopMode::UpdateAndRender, _)).Times(AnyNumber()).WillRepeatedly(Invoke([&](auto /*unused*/, auto /*unused*/)
        {
            m_loopCount++;
        }));
    ...
    TEST_F(ADisplayThread, willUpdateWithUpdateOnlyMode)
    {
        EXPECT_CALL(m_aliveHandlerMock, notifyAlive(ThreadAliveNotifierMock::dummyThreadId)).Times(AtLeast(9));
        EXPECT_CALL(m_aliveHandlerMock, calculateTimeout()).Times(AtLeast(0)).WillRepeatedly(Return(20ms));
        EXPECT_CALL(m_displayBundleMock, doOneLoop(ELoopMode::UpdateOnly, _)).Times(AnyNumber()).WillRepeatedly(Invoke([&](auto /*unused*/, auto /*unused*/)
        {
            m_loopCount++;
        }));
    ...
    {
        EXPECT_CALL(m_aliveHandlerMock, notifyAlive(ThreadAliveNotifierMock::dummyThreadId)).Times(AtLeast(10)).WillRepeatedly([this](auto /*unused*/) { m_notifyCounter++; });
