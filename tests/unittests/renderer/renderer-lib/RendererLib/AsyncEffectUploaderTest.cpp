...
namespace ramses::internal
{
    ...
    protected:
        ...
        EffectsRawResources createUniqueEffects(uint32_t count)
        {
            ...
            for(uint32_t i = 0u; i < count; ++i)
            {
                ...
                const EffectResource* effect = new EffectResource(randomString, "", "", {}, {}, {}, {}, "", EFeatureLevel_Latest);
    ...
    TEST_F(AnAsyncEffectUploader, UploadsShadersThatGetSubmittedForUploadWhileUploadThreadIsBusy)
    {
        ...
        EXPECT_CALL(platformMock.resourceUploadRenderBackendMock.deviceMock, uploadShader(Ref(*effectToUploadAndBlock[0]))).WillOnce(Invoke([&](const auto& /*unused*/) {
        ...
        EXPECT_CALL(platformMock.resourceUploadRenderBackendMock.deviceMock, uploadShader(_)).WillOnce(Invoke([&](const auto& /*unused*/) {
    ...
    {
        EXPECT_CALL(notifier, notifyAlive(ThreadAliveNotifierMock::dummyThreadId)).Times(AtLeast(5)).WillRepeatedly([this](auto /*unused*/) { notifyCounter++; });
    ...
    {
        EXPECT_CALL(notifier, notifyAlive(ThreadAliveNotifierMock::dummyThreadId)).Times(AtLeast(10)).WillRepeatedly([this](auto /*unused*/) { notifyCounter++; });
    ...
    TEST_F(AnAsyncEffectUploader, DestructionCancelsUpcomingShaderUploads)
    {
        ...
        EXPECT_CALL(platformMock.resourceUploadRenderBackendMock.deviceMock, uploadShader(_)).Times(AnyNumber()).WillRepeatedly(Invoke([&](const auto& /*unused*/) {
        ...
        EXPECT_CALL(platformMock.resourceUploadRenderBackendMock.deviceMock, uploadShader(Ref(*effectToUploadAndBlock))).WillOnce(Invoke([&](const auto& /*unused*/) {
