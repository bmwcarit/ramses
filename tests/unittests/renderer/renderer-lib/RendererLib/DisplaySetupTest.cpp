...
namespace ramses::internal
{
    ...
    TEST_F(ADisplaySetup, canSetClearFlagsForAnyRegisteredBuffer)
    {
        ...
        EXPECT_EQ(EClearFlag::All, displaySetup.getDisplayBuffer(bufferHandleOBint).clearFlags);

        displaySetup.setClearFlags(bufferHandleFB, EClearFlag::None);
        displaySetup.setClearFlags(bufferHandleOB, EClearFlag::Color);
        displaySetup.setClearFlags(bufferHandleOBint, EClearFlag::All);

        EXPECT_EQ(EClearFlag::None, displaySetup.getDisplayBuffer(bufferHandleFB).clearFlags);
        EXPECT_EQ(EClearFlag::Color, displaySetup.getDisplayBuffer(bufferHandleOB).clearFlags);
        EXPECT_EQ(EClearFlag::All, displaySetup.getDisplayBuffer(bufferHandleOBint).clearFlags);
    }
