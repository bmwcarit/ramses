...
namespace ramses::internal {
    ...
    public:
        ...
        {
            EXPECT_CALL(*resUploader, uploadResource(Ref(platform.renderBackendMock), _, _)).WillOnce(Invoke([hash, type, resultDeviceHandle](auto& /*unused*/, const auto& rd, auto& /*unused*/) {
        ...
        {
            EXPECT_CALL(*resUploader, uploadResource(Ref(platform.renderBackendMock), _, _)).WillOnce(Invoke([hash](auto& /*unused*/, const auto& rd, auto& /*unused*/) {
                EXPECT_EQ(hash, rd.hash);
                EXPECT_EQ(EResourceType::Effect, rd.type);
            ...
            {
                EXPECT_CALL(platform.resourceUploadRenderBackendMock.deviceMock, uploadShader(_)).WillRepeatedly(Invoke([](const auto& /*unused*/) {return std::move(std::unique_ptr<const GPUResource>{}); }));
                expectDeviceFlushOnWindows();
    ...
    TEST_F(ARendererResourceManager, PerformsCorrectlyResourceLifecycle)
    {
        ...
        // upload the resource
        expectResourceUploaded(resource, EResourceType::VertexArray);
        resourceManager.uploadAndUnloadPendingResources();
        ...
        expectResourceUnloaded(resource, EResourceType::VertexArray);
        resourceManager.uploadAndUnloadPendingResources();
    ...
    TEST_F(ARendererResourceManager, doesNotDeleteResourcesNeededByAnotherSceneWhenSceneDestroyed)
    {
        ...
        InSequence s;
        expectResourceUploaded(vertResource, EResourceType::VertexArray, vertDeviceHandle);
        expectResourceUploaded(indexResource, EResourceType::IndexArray, indexDeviceHandle);
        resourceManager.uploadAndUnloadPendingResources();

        resourceManager.unreferenceResourcesForScene(fakeSceneId, { vertResource, indexResource });
        expectResourceUnloaded(vertResource, EResourceType::VertexArray, vertDeviceHandle);
        resourceManager.uploadAndUnloadPendingResources();
        ...
        expectResourceUnloaded(indexResource, EResourceType::IndexArray, indexDeviceHandle);
        resourceManager.uploadAndUnloadPendingResources();
    }
    ...
    TEST_F(ARendererResourceManager, canUploadAndUpdateAndUnloadDataBuffer_IndexBuffer)
    {
        ...
        const uint32_t sizeInBytes = 1024u;
    ...
    TEST_F(ARendererResourceManager, canUploadAndUpdateAndUnloadTextureBuffer_WithOneMipLevel)
    {
        ...
        const uint32_t expectedSize = (10u * 20u) * 4u;
        EXPECT_CALL(platform.renderBackendMock.deviceMock, allocateTexture2D(10u, 20u, EPixelStorageFormat::RGBA8, DefaultTextureSwizzleArray, 1u, expectedSize));
        resourceManager.uploadTextureBuffer(textureBuffer, 10u, 20u, EPixelStorageFormat::RGBA8, 1u, fakeSceneId);

        EXPECT_EQ(DeviceMock::FakeTextureDeviceHandle, resourceManager.getTextureBufferDeviceHandle(textureBuffer, fakeSceneId));

        const std::byte dummyTexture[10] = {};
    ...
    TEST_F(ARendererResourceManager, canUploadAndUpdateAndUnloadTextureBuffer_WithSeveralMipLevels)
    {
        InSequence seq;
        const TextureBufferHandle textureBuffer(1u);
        const uint32_t expectedSize = (10u * 20u + 5u * 10u)* 4u;
        EXPECT_CALL(platform.renderBackendMock.deviceMock, allocateTexture2D(10u, 20u, EPixelStorageFormat::RGBA8, DefaultTextureSwizzleArray, 2u, expectedSize));
        resourceManager.uploadTextureBuffer(textureBuffer, 10u, 20u, EPixelStorageFormat::RGBA8, 2u, fakeSceneId);
    ...
    TEST_F(ARendererResourceManager, canUploadAndUnloadBlitPassRenderTargets)
    {
        ...
        EXPECT_CALL(platform.renderBackendMock.deviceMock,
            uploadRenderBuffer(800u, 600u, EPixelStorageFormat::RGBA8, ERenderBufferAccessMode::ReadWrite, 0u))
    ...
    TEST_F(ARendererResourceManager, canUploadAndUnloadRenderTargetWithBuffers)
    {
...

        {
            InSequence seq;
            EXPECT_CALL(platform.renderBackendMock.deviceMock, uploadRenderBuffer(800u, 600u, EPixelStorageFormat::RGBA8, ERenderBufferAccessMode::ReadWrite, 0u));
            EXPECT_CALL(platform.renderBackendMock.deviceMock, uploadRenderBuffer(800u, 600u, EPixelStorageFormat::Depth24, ERenderBufferAccessMode::ReadWrite, 0u));
    ...
    TEST_F(ARendererResourceManager, UploadsOffscreenBufferWithColorBuffer)
    {
        ...
        InSequence seq;
        EXPECT_CALL(platform.renderBackendMock.deviceMock, uploadRenderBuffer(1u, 1u, EPixelStorageFormat::RGBA8, ERenderBufferAccessMode::ReadWrite, 0u));
        EXPECT_CALL(platform.renderBackendMock.deviceMock, uploadRenderTarget(_));
        ...
        resourceManager.uploadOffscreenBuffer(bufferHandle, 1u, 1u, 0u, false, EDepthBufferType::None);
    ...
    TEST_F(ARendererResourceManager, UploadsOffscreenBufferWithColorAndDepthBuffers)
    {
        ...
        InSequence seq;
        EXPECT_CALL(platform.renderBackendMock.deviceMock, uploadRenderBuffer(1u, 1u, EPixelStorageFormat::RGBA8, ERenderBufferAccessMode::ReadWrite, 0u));
        EXPECT_CALL(platform.renderBackendMock.deviceMock, uploadRenderBuffer(1u, 1u, EPixelStorageFormat::Depth32, ERenderBufferAccessMode::WriteOnly, 0u));
        EXPECT_CALL(platform.renderBackendMock.deviceMock, uploadRenderTarget(_));
        EXPECT_CALL(platform.renderBackendMock.deviceMock, activateRenderTarget(_));
        EXPECT_CALL(platform.renderBackendMock.deviceMock, colorMask(true, true, true, true));
        EXPECT_CALL(platform.renderBackendMock.deviceMock, clearColor(glm::vec4{ 0.f, 0.f, 0.f, 1.f }));
        ...
        EXPECT_CALL(platform.renderBackendMock.deviceMock, scissorTest(EScissorTest::Disabled, scissorRegion));
        EXPECT_CALL(platform.renderBackendMock.deviceMock, clear(_));
        resourceManager.uploadOffscreenBuffer(bufferHandle, 1u, 1u, 0u, false, ramses::EDepthBufferType::Depth);
    ...
    TEST_F(ARendererResourceManager, UploadsOffscreenBufferWithColorAndDepthStencilAttached)
    {
        ...
        InSequence seq;
        EXPECT_CALL(platform.renderBackendMock.deviceMock, uploadRenderBuffer(1u, 1u, EPixelStorageFormat::RGBA8, ERenderBufferAccessMode::ReadWrite, 0u));
        EXPECT_CALL(platform.renderBackendMock.deviceMock, uploadRenderBuffer(1u, 1u, EPixelStorageFormat::Depth24_Stencil8, ERenderBufferAccessMode::WriteOnly, 0u));
        EXPECT_CALL(platform.renderBackendMock.deviceMock, uploadRenderTarget(_));
        EXPECT_CALL(platform.renderBackendMock.deviceMock, activateRenderTarget(_));
        EXPECT_CALL(platform.renderBackendMock.deviceMock, colorMask(true, true, true, true));
        EXPECT_CALL(platform.renderBackendMock.deviceMock, clearColor(glm::vec4{ 0.f, 0.f, 0.f, 1.f }));
        ...
        EXPECT_CALL(platform.renderBackendMock.deviceMock, scissorTest(EScissorTest::Disabled, scissorRegion));
        EXPECT_CALL(platform.renderBackendMock.deviceMock, clear(_));
        resourceManager.uploadOffscreenBuffer(bufferHandle, 1u, 1u, 0u, false, EDepthBufferType::DepthStencil);
    ...
    TEST_F(ARendererResourceManager, UploadsOffscreenBufferWithColorAndDepthStencilAttachedWithMSAAenabled)
    {
        const OffscreenBufferHandle bufferHandle(1u);
        InSequence seq;
        EXPECT_CALL(platform.renderBackendMock.deviceMock, uploadRenderBuffer(1u, 1u, EPixelStorageFormat::RGBA8, ERenderBufferAccessMode::ReadWrite, 4u));
        EXPECT_CALL(platform.renderBackendMock.deviceMock, uploadRenderBuffer(1u, 1u, EPixelStorageFormat::Depth24_Stencil8, ERenderBufferAccessMode::WriteOnly, 4u));
        EXPECT_CALL(platform.renderBackendMock.deviceMock, uploadRenderTarget(_));
        EXPECT_CALL(platform.renderBackendMock.deviceMock, activateRenderTarget(_));
        EXPECT_CALL(platform.renderBackendMock.deviceMock, colorMask(true, true, true, true));
        EXPECT_CALL(platform.renderBackendMock.deviceMock, clearColor(glm::vec4{ 0.f, 0.f, 0.f, 1.f }));
        ...
        EXPECT_CALL(platform.renderBackendMock.deviceMock, scissorTest(EScissorTest::Disabled, scissorRegion));
        EXPECT_CALL(platform.renderBackendMock.deviceMock, clear(_));
        resourceManager.uploadOffscreenBuffer(bufferHandle, 1u, 1u, 4u, false, EDepthBufferType::DepthStencil);
    ...
    TEST_F(ARendererResourceManager, UploadsOffscreenBufferWithColorBuffer_WithDoubleBuffering)
    {
        ...
        resourceManager.uploadOffscreenBuffer(bufferHandle, 1u, 1u, 0u, true, EDepthBufferType::None);
    ...
    TEST_F(ARendererResourceManager, UploadsOffscreenBufferWithColorAndDepthStencilBuffers_WithDoubleBuffering)
    {
        ...
        EXPECT_CALL(platform.renderBackendMock.deviceMock, clear(_));
        EXPECT_CALL(platform.renderBackendMock.deviceMock, pairRenderTargetsForDoubleBuffering(_, _));
        resourceManager.uploadOffscreenBuffer(bufferHandle, 1u, 1u, 0u, true, EDepthBufferType::DepthStencil);
    ...
    TEST_F(ARendererResourceManager, CanUnloadOffscreenBuffer_WithColorBuffer)
    {
        const OffscreenBufferHandle bufferHandle(1u);
        EXPECT_CALL(platform.renderBackendMock.deviceMock, uploadRenderTarget(_));
        EXPECT_CALL(platform.renderBackendMock.deviceMock, uploadRenderBuffer(_, _, _, _, _));
        EXPECT_CALL(platform.renderBackendMock.deviceMock, activateRenderTarget(_));
        EXPECT_CALL(platform.renderBackendMock.deviceMock, colorMask(true, true, true, true));
        EXPECT_CALL(platform.renderBackendMock.deviceMock, clearColor(glm::vec4{ 0.f, 0.f, 0.f, 1.f }));
        EXPECT_CALL(platform.renderBackendMock.deviceMock, depthWrite(EDepthWrite::Enabled));
    ...
    TEST_F(ARendererResourceManager, CanUnloadOffscreenBuffer_WithColorAndDepthStencilBuffers)
    {
        const OffscreenBufferHandle bufferHandle(1u);
        EXPECT_CALL(platform.renderBackendMock.deviceMock, uploadRenderTarget(_));
        EXPECT_CALL(platform.renderBackendMock.deviceMock, uploadRenderBuffer(_, _, _, _, _)).Times(2u);
        EXPECT_CALL(platform.renderBackendMock.deviceMock, activateRenderTarget(_));
        EXPECT_CALL(platform.renderBackendMock.deviceMock, colorMask(true, true, true, true));
        EXPECT_CALL(platform.renderBackendMock.deviceMock, clearColor(glm::vec4{ 0.f, 0.f, 0.f, 1.f }));
        EXPECT_CALL(platform.renderBackendMock.deviceMock, depthWrite(EDepthWrite::Enabled));
    ...
    TEST_F(ARendererResourceManager, CanUnloadDoubleBufferedOffscreenBuffer_WithColorAndDepthStencilBuffers)
    {
        const OffscreenBufferHandle bufferHandle(1u);
        EXPECT_CALL(platform.renderBackendMock.deviceMock, uploadRenderTarget(_)).Times(2u);
        EXPECT_CALL(platform.renderBackendMock.deviceMock, uploadRenderBuffer(_, _, _, _, _)).Times(3u);
        EXPECT_CALL(platform.renderBackendMock.deviceMock, activateRenderTarget(_)).Times(2u);
        EXPECT_CALL(platform.renderBackendMock.deviceMock, colorMask(true, true, true, true)).Times(2u);
        EXPECT_CALL(platform.renderBackendMock.deviceMock, clearColor(glm::vec4{ 0.f, 0.f, 0.f, 1.f })).Times(2u);
        EXPECT_CALL(platform.renderBackendMock.deviceMock, depthWrite(EDepthWrite::Enabled)).Times(2u);
    ...
    TEST_F(ARendererResourceManager, DoesNotUnregisterResourceThatWasScheduledForUpload)
    {
        ...
        EXPECT_CALL(*resUploader, uploadResource(Ref(platform.renderBackendMock), _, _)).WillOnce(Invoke([resHash](auto& /*unused*/, const auto& rd, auto& /*unused*/) {
    ...
    TEST_F(ARendererResourceManager, DoesNotUnloadEffectThatGetsUnreferencedAndReReferencedWhileCompiling)
    {
        ...
        EXPECT_CALL(platform.resourceUploadRenderBackendMock.deviceMock, uploadShader(_)).WillOnce(Invoke([&](const auto& /*unused*/) {
    ...
    TEST_F(ARendererResourceManager, DeletesEffectOnlyWhenDestructed_NotWhenUnrequested)
    {
        ...
        // delete resource
        expectResourceUnloaded(resource, EResourceType::Effect, DeviceMock::FakeShaderDeviceHandle);
    }
    ...
    TEST_F(ARendererResourceManager, unloadsAllSceneResources)
    {
        ...
        resourceManager.uploadTextureBuffer(textureBufferHandle, 1u, 2u, EPixelStorageFormat::RGBA8, 1u, fakeSceneId);
    ...
    TEST_F(ARendererResourceManager, providingResourceDataWhenAlreadyUploadedIsNoop)
    {
        ...
        resourceManager.unreferenceAllResourcesForScene(fakeSceneId);
        expectResourceUnloaded(MockResourceHash::EffectHash, EResourceType::Effect, DeviceMock::FakeShaderDeviceHandle);
    }

    TEST_F(ARendererResourceManager, rereferencingResourceKeptInCacheIsNotUploadedAgain)
    {
        const auto managedRes = MockResourceHash::GetManagedResource(MockResourceHash::EffectHash);
        // resource must be referenced by scene before providing data
        referenceResource(MockResourceHash::EffectHash, fakeSceneId);
    ...
    TEST_F(ARendererResourceManager, UnloadsAllRemainingOffscreenBuffersAndStreamBuffersAtDestruction)
    {
        ...
        EXPECT_CALL(platform.renderBackendMock.deviceMock, uploadRenderTarget(_)).Times(2u);
        EXPECT_CALL(platform.renderBackendMock.deviceMock, uploadRenderBuffer(_, _, _, _, _)).Times(4u);
        EXPECT_CALL(platform.renderBackendMock.deviceMock, activateRenderTarget(_)).Times(2u);
        EXPECT_CALL(platform.renderBackendMock.deviceMock, colorMask(true, true, true, true)).Times(2u);
        EXPECT_CALL(platform.renderBackendMock.deviceMock, clearColor(glm::vec4{ 0.f, 0.f, 0.f, 1.f })).Times(2u);
        EXPECT_CALL(platform.renderBackendMock.deviceMock, depthWrite(EDepthWrite::Enabled)).Times(2u);
        ...
        EXPECT_CALL(platform.renderBackendMock.deviceMock, clear(_)).Times(2u);
        resourceManager.uploadOffscreenBuffer(obHandle, 1u, 1u, 0u, false, EDepthBufferType::DepthStencil);
        resourceManager.uploadOffscreenBuffer(obHandle2, 1u, 1u, 0u, false, EDepthBufferType::DepthStencil);
