...
namespace ramses::internal
{
    ...
    TYPED_TEST(NodeTest, hasChild)
    {
        ...
        EXPECT_TRUE(parent.addChild(this->createNode("child")));
    ...
    {
        ramses::Scene& otherScene = *this->client.createScene(sceneId_t(1234u));
        ...
        EXPECT_FALSE(parent.addChild(this->createNode("child")));
        EXPECT_EQ(0u, parent.getChildCount());
    ...
    TYPED_TEST(NodeTest, shouldReplaceParentWithSetParent)
    {
        ...
        EXPECT_TRUE(child.setParent(parent1));
        EXPECT_TRUE(child.setParent(parent2));
        EXPECT_EQ(0u, parent1.getChildCount());
        EXPECT_EQ(&parent2, child.getParent());
    }
    ...
    TYPED_TEST(NodeTest, shouldOrderChildrenAccordingToTheirAddingToTheirParent)
    {
        ...
        ASSERT_TRUE(parent.addChild(child1));
        ASSERT_TRUE(parent.addChild(child2));
        ASSERT_TRUE(parent.addChild(child3));
        ASSERT_TRUE(parent.addChild(child4));
    ...
    TYPED_TEST(NodeTest, getsIdentityModelMatrixInitially)
    {
        Node& node = this->createNode("node");
        matrix44f modelMat;
        EXPECT_TRUE(node.getModelMatrix(modelMat));
    ...
    TYPED_TEST(NodeTest, setsRotationVectorAndConvention)
    {
        ...
        EXPECT_TRUE(node.setRotation({1.f, 2.f, 3.f}, ERotationType::Euler_XZX));

        const auto transformHandle = node.impl().getTransformHandle();

        vec3f euler;
        EXPECT_TRUE(node.getRotation(euler));
        EXPECT_EQ(euler.x, 1.f);
        EXPECT_EQ(euler.y, 2.f);
        EXPECT_EQ(euler.z, 3.f);
        EXPECT_EQ(ERotationType::Euler_XZX, node.getRotationType());
        EXPECT_EQ(glm::vec3(1.f, 2.f, 3.f), glm::vec3(this->m_scene.impl().getIScene().getRotation(transformHandle)));
        EXPECT_EQ(ERotationType::Euler_XZX, this->m_scene.impl().getIScene().getRotationType(transformHandle));

        EXPECT_TRUE(node.setRotation({11.f, 12.f, 13.f}, ERotationType::Euler_XYZ));
        ...
        EXPECT_EQ(glm::vec3(11.f, 12.f, 13.f), glm::vec3(this->m_scene.impl().getIScene().getRotation(transformHandle)));
        EXPECT_EQ(ERotationType::Euler_XYZ, this->m_scene.impl().getIScene().getRotationType(transformHandle));
    }
    ...
    TYPED_TEST(NodeTest, returnsErrorWhenSettingQuaternionConvention)
    {
        ...
        EXPECT_FALSE(node.setRotation({10.f, 20.f, 30.f}, ERotationType::Quaternion));

        // default value is still set
        EXPECT_EQ(ERotationType::Euler_XYZ, node.getRotationType());

        quat quaternion;
        EXPECT_TRUE(node.getRotation(quaternion));
    ...
    TYPED_TEST(NodeTest, setsRotationQuaternion)
    {
        ...
        EXPECT_TRUE(node.setRotation(q));

        const auto transformHandle = node.impl().getTransformHandle();

        quat qOut;
        EXPECT_TRUE(node.getRotation(qOut));
        EXPECT_EQ(qOut.x, q.x);
        EXPECT_EQ(qOut.y, q.y);
        EXPECT_EQ(qOut.z, q.z);
        EXPECT_EQ(qOut.w, q.w);
        EXPECT_EQ(glm::vec4(q.x, q.y, q.z, q.w), this->m_scene.impl().getIScene().getRotation(transformHandle));
        EXPECT_EQ(ERotationType::Quaternion, this->m_scene.impl().getIScene().getRotationType(transformHandle));
    }
    ...
    TYPED_TEST(NodeTest, setsRotationQuaternionThenEuler)
    {
        ...
        EXPECT_EQ(glm::vec3(11.f, 12.f, 13.f), glm::vec3(this->m_scene.impl().getIScene().getRotation(transformHandle)));
        EXPECT_EQ(ERotationType::Euler_XYZ, this->m_scene.impl().getIScene().getRotationType(transformHandle));
        EXPECT_FALSE(node.getRotation(qOut));
    }
    ...
    TYPED_TEST(NodeTest, setsRotationEulerThenQuaternion)
    {
        ...
        EXPECT_FALSE(node.getRotation(euler));
    ...
    TYPED_TEST(NodeTest, getRotationQuaternionReturnsErrorIfEuler)
    {
        Node& node = this->createNode("node");
        quat  q;
        EXPECT_TRUE(node.setRotation({90.f, 0.f, 0.f}, ERotationType::Euler_XYZ));
        EXPECT_FALSE(node.getRotation(q));
    }
    ...
    TYPED_TEST(NodeTest, getRotationQuaternionReturnsDefaultValuesWithoutSetBefore)
    {
        ...
        quat q;
        EXPECT_EQ(ERotationType::Euler_XYZ, node.getRotationType());
        EXPECT_TRUE(node.getRotation(q));
        EXPECT_EQ(0.f, q.x);
        EXPECT_EQ(0.f, q.y);
        EXPECT_EQ(0.f, q.z);
        EXPECT_EQ(1.f, q.w);
    ...
    TYPED_TEST(NodeTest, settingScaleOrTranlationMayNotBreakGetRotation)
    {
        ...
        EXPECT_TRUE(node.setTranslation({2, 2, 2}));
        EXPECT_TRUE(node.getRotation(euler));
    ...
    TYPED_TEST(NodeTest, canSetAndGetRotationAfterSettingScaleOrTranslation)
    {
        ...
        EXPECT_TRUE(node.setRotation({1.f, 2.f, 3.f}, ERotationType::Euler_ZYZ));
    ...
    TYPED_TEST(NodeTest, getsIdentityInverseModelMatrixInitially)
    {
        Node& node = this->createNode("node");
        matrix44f modelMat;
        EXPECT_TRUE(node.getInverseModelMatrix(modelMat));
    ...
    TYPED_TEST(NodeTest, getsModelMatrixComputedFromTransformationChain_SingleNode)
    {
        ...
        const auto rotMat = ramses::internal::Math3d::Rotation({ 7.f, 8.f, 9.f, 1.f }, ERotationType::Euler_ZYX);
    ...
    TYPED_TEST(NodeTest, getsInverseModelMatrixComputedFromTransformationChain_SingleNode)
    {
        ...
        const auto rotMat = glm::transpose(ramses::internal::Math3d::Rotation({ 7.f, 8.f, 9.f, 1.f }, ERotationType::Euler_YZX));
    ...
    TYPED_TEST(NodeTest, getsInverseModelMatrixComputedFromTransformationChain_MultipleNodes)
    {
        ...
        const auto transMat = glm::translate(glm::vec3{ -1.f, -2.f, -3.f });
        const auto scaleMat = glm::inverse(glm::scale(glm::vec3{ 4.f, 5.f, 6.f }));
        const auto rotMat = glm::transpose(ramses::internal::Math3d::Rotation({ 7.f, 8.f, 9.f, 1.f }, ERotationType::Euler_YXY));
        const auto expectedInverseModelMat = scaleMat * rotMat * transMat;
