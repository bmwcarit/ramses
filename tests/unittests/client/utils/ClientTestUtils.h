...
namespace ramses::internal
{
    ...
    {
    public:
        explicit LocalTestClient(ramses::EFeatureLevel featureLevel = ramses::EFeatureLevel_Latest)
            : framework{ GetDefaultFrameworkConfig(featureLevel) }
            , client(*framework.createClient("localTestClient"))
            , m_creationHelper(nullptr, &client)
        {
            EXPECT_EQ(featureLevel, framework.getFeatureLevel());
        ...
        static RamsesFrameworkConfig GetDefaultFrameworkConfig(ramses::EFeatureLevel featureLevel = ramses::EFeatureLevel_Latest)
        {
            ...
            RamsesFrameworkConfig config{ featureLevel };
    ...
    {
    public:
        explicit LocalTestClientWithScene(ramses::EFeatureLevel featureLevel = ramses::EFeatureLevel_Latest, EScenePublicationMode publicationMode = EScenePublicationMode::LocalOnly, ERenderBackendCompatibility compatibility = ERenderBackendCompatibility::OpenGL)
            : LocalTestClient{ featureLevel }
            , m_scene(CreateScene(client, publicationMode, compatibility))
        ...
        static ramses::Scene& CreateScene(RamsesClient& client, EScenePublicationMode publicationMode, ERenderBackendCompatibility compatibility)
        {
            SceneConfig config{sceneId_t(123u), publicationMode, compatibility};
