...
namespace ramses::internal
{
    ...
    public:
        AGlyphTextureAtlas()
            : m_client(*m_framework.createClient("test glyph atlas"))
        ...
        static bool GlyphMappingsOverlap(const GlyphGeometry& glyphGeometry1, const GlyphGeometry& glyphGeometry2)
        {
            ...
            const float x1_lo = glyphGeometry1.texcoords[1][0];
            const float y1_lo = glyphGeometry1.texcoords[1][1];
            const float x1_hi = glyphGeometry1.texcoords[3][0];
            const float y1_hi = glyphGeometry1.texcoords[3][1];

            const float x2_lo = glyphGeometry2.texcoords[1][0];
            const float y2_lo = glyphGeometry2.texcoords[1][1];
            const float x2_hi = glyphGeometry2.texcoords[3][0];
            const float y2_hi = glyphGeometry2.texcoords[3][0];
        ...
        static void VerifyGlyphMapping(const GlyphGeometry& glyphGeometry, float x_lo_expected, float y_lo_expected, float x_hi_expected, float y_hi_expected)
        {
            EXPECT_FLOAT_EQ(glyphGeometry.texcoords[1][0], x_lo_expected);
            EXPECT_FLOAT_EQ(glyphGeometry.texcoords[1][1], y_lo_expected);
            EXPECT_FLOAT_EQ(glyphGeometry.texcoords[3][0], x_hi_expected);
            EXPECT_FLOAT_EQ(glyphGeometry.texcoords[3][1], y_hi_expected);
        }

        static void ExpectGeometrySize(size_t numGlyphs, GlyphGeometry const& glyphGeometry)
        {
            EXPECT_EQ(glyphGeometry.positions.size(), numGlyphs * 4u);
            EXPECT_EQ(glyphGeometry.indices.size(), numGlyphs * 6u);
            EXPECT_EQ(glyphGeometry.texcoords.size(), numGlyphs * 4u);
        }

        static void ExpectGeometry(int left, int right, int bottom, int top, GlyphGeometry const& geo, int i = 0, int xoffset = 0, int yoffset = 0)
        {
            constexpr int padding = 1;
            constexpr float halfPadding = padding / 2.f;

            EXPECT_EQ(geo.positions[i * 4][0], left - halfPadding);
            EXPECT_EQ(geo.positions[i * 4][1], bottom - halfPadding);

            EXPECT_EQ(geo.positions[i * 4 + 1][0], left - halfPadding);
            EXPECT_EQ(geo.positions[i * 4 + 1][1], top + halfPadding);

            EXPECT_EQ(geo.positions[i * 4 + 2][0], right + halfPadding);
            EXPECT_EQ(geo.positions[i * 4 + 2][1], top + halfPadding);

            EXPECT_EQ(geo.positions[i * 4 + 3][0], right + halfPadding);
            EXPECT_EQ(geo.positions[i * 4 + 3][1], bottom - halfPadding);
            ...
            EXPECT_NEAR(geo.texcoords[i * 4][0], (xoffset + padding - halfPadding) / AtlasTextureWidth, 0.0001f);
            EXPECT_NEAR(geo.texcoords[i * 4][1], (yoffset + top - bottom + padding + halfPadding) / AtlasTextureHeight, 0.0001f);

            EXPECT_NEAR(geo.texcoords[i * 4 + 1][0], (xoffset + padding - halfPadding) / AtlasTextureWidth, 0.0001f);
            EXPECT_NEAR(geo.texcoords[i * 4 + 1][1], (yoffset + padding - halfPadding) / AtlasTextureHeight, 0.0001f);

            EXPECT_NEAR(geo.texcoords[i * 4 + 2][0], (xoffset + right - left + padding + halfPadding) / AtlasTextureWidth, 0.0001f);
            EXPECT_NEAR(geo.texcoords[i * 4 + 2][1], (yoffset + padding - halfPadding) / AtlasTextureHeight, 0.0001f);

            EXPECT_NEAR(geo.texcoords[i * 4 + 3][0], (xoffset + right - left + padding + halfPadding) / AtlasTextureWidth, 0.0001f);
            EXPECT_NEAR(geo.texcoords[i * 4 + 3][1], (yoffset + top - bottom + padding + halfPadding) / AtlasTextureHeight, 0.0001f);
        }
