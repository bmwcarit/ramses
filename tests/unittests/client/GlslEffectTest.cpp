...
namespace ramses::internal
{
    ...
    public:
        ...
        const HashMap<std::string, EFixedSemantics> emptySemanticInputs{};
    ...
    TEST_F(AGlslEffect, canParseBasicShaders)
    {
        ...
        std::unique_ptr<EffectResource> res(ge.createEffectResource());
        ASSERT_TRUE(res);
    ...
    TEST_F(AGlslEffect, canParseShaderInputs)
    {
        ...
        HashMap<std::string, EFixedSemantics> semantics;
    ...
    public:
        void run() override
        {
            ...
            const ramses::internal::HashMap<std::string, ramses::internal::EFixedSemantics> sems;
            const char* v = "void main(){gl_Position=vec4(0);}";
            const char* f = "void main(){gl_FragColor=vec4(0);}";

            GlslEffect eff(v, f, "", defs, sems, "myname");
            std::unique_ptr<EffectResource> resource(eff.createEffectResource());
    ...
    TEST_F(AGlslEffect, treatsUniformDeclaredInBothStagesWithSameNameAsSingleUniform)
    {
        ...
        GlslEffect ge(vertexShader, fragmentShader, "", emptyCompilerDefines, emptySemanticInputs, "");
        const std::unique_ptr<EffectResource> res(ge.createEffectResource());
        ASSERT_TRUE(res);
