...
namespace ramses::internal
{
    ...
    protected:
        ...
        SceneImpl& m_scene{ m_ramses.createScene()->impl() };

        LogicNodeDummyImpl m_nodeA{ m_scene, "A", false };
        LogicNodeDummyImpl m_nodeB{ m_scene, "B", false };
        RamsesBindingDummyImpl m_binding1{ m_scene };
        RamsesBindingDummyImpl m_binding2{ m_scene };
    };
    ...
    TEST_F(ALogicNodeDependencies, ConnectingTwoNodes_CreatesALink)
    {
        ...
        PropertyImpl& output = m_nodeA.getOutputs()->getChild("output1")->impl();
        PropertyImpl& input  = m_nodeB.getInputs()->getChild("input1")->impl();
    ...
    {
        auto nodeToDelete = std::make_unique<LogicNodeDummyImpl>(m_scene, "node");

        m_dependencies.addNode(*nodeToDelete);
        m_dependencies.addNode(m_nodeB);

        PropertyImpl& input = m_nodeB.getInputs()->getChild("input1")->impl();
        EXPECT_TRUE(m_dependencies.link(nodeToDelete->getOutputs()->getChild("output1")->impl(), input, false, m_errorReporting));
    ...
    TEST_F(ALogicNodeDependencies, RemovingTargetNode_RemovesLinks)
    {
        ...
        PropertyImpl& output = m_nodeA.getOutputs()->getChild("output1")->impl();
        EXPECT_TRUE(m_dependencies.link(output, nodeToDelete->getInputs()->getChild("input1")->impl(), false, m_errorReporting));
    ...
    {
        auto nodeToDelete = std::make_unique<LogicNodeDummyImpl>(m_scene, "M", false);

        m_dependencies.addNode(m_nodeA);
        m_dependencies.addNode(*nodeToDelete);
        m_dependencies.addNode(m_nodeB);

        PropertyImpl& output1A = m_nodeA.getOutputs()->getChild("output1")->impl();
        PropertyImpl& output2A = m_nodeA.getOutputs()->getChild("output2")->impl();
        PropertyImpl& output1M = nodeToDelete->getOutputs()->getChild("output1")->impl();
        PropertyImpl& input1M  = nodeToDelete->getInputs()->getChild("input1")->impl();
        PropertyImpl& input1B  = m_nodeB.getInputs()->getChild("input1")->impl();
        PropertyImpl& input2B  = m_nodeB.getInputs()->getChild("input2")->impl();
    ...
    TEST_F(ALogicNodeDependencies, ReversingDependencyOfTwoNodes_InvertsTopologicalOrder)
    {
        ...
        PropertyImpl& inputB  = m_nodeB.getInputs()->getChild("input1")->impl();
        PropertyImpl& outputA = m_nodeA.getOutputs()->getChild("output1")->impl();
        ...
        PropertyImpl& inputA  = m_nodeA.getInputs()->getChild("input1")->impl();
        PropertyImpl& outputB = m_nodeB.getOutputs()->getChild("output1")->impl();
    ...
    protected:
        ...
        ALogicNodeDependencies_NestedLinks()
        {
            ...
            m_nestedOutputA = &m_nodeANested->getOutputs()->getChild("outputStruct")->getChild("nested")->impl();
            m_nestedInputB  = &m_nodeBNested->getInputs()->getChild("inputStruct")->getChild("nested")->impl();
            m_arrayOutputA  = &m_nodeANested->getOutputs()->getChild("outputArray")->getChild(0)->impl();
            m_arrayInputB   = &m_nodeBNested->getInputs()->getChild("inputArray")->getChild(0)->impl();
        }

        std::unique_ptr<LogicNodeDummyImpl> m_nodeANested = std::make_unique<LogicNodeDummyImpl>(m_scene, "A", true);
        std::unique_ptr<LogicNodeDummyImpl> m_nodeBNested = std::make_unique<LogicNodeDummyImpl>(m_scene, "B", true);
    ...
    {
        PropertyImpl& structProperty = m_nodeBNested->getInputs()->getChild("inputStruct")->impl();
        EXPECT_FALSE(m_dependencies.unlink(*m_nestedOutputA, structProperty, m_errorReporting));
        EXPECT_EQ("Can't unlink properties of complex types directly!", m_errorReporting.getError()->message);
    }

    TEST_F(ALogicNodeDependencies_NestedLinks, ReportsErrorWhenUnlinkingArrayInputs_BasedOnTheirType)
    {
        PropertyImpl& arrayProperty = m_nodeBNested->getInputs()->getChild("inputArray")->impl();
    ...
    TEST_F(ALogicNodeDependencies_NestedLinks, ReportsErrorWhenUnlinkingStructs_WithLinkedChildren)
    {
        ...
        PropertyImpl& outputParentStruct = m_nodeANested->getOutputs()->getChild("outputStruct")->impl();
        PropertyImpl& inputParentStruct = m_nodeBNested->getInputs()->getChild("inputStruct")->impl();
    ...
    TEST_F(ALogicNodeDependencies_NestedLinks, ReversingDependencyOfTwoNodes_InvertsTopologicalOrder)
    {
        ...
        // Node B -> Node A  (output of node B linked to input of node A)
        EXPECT_TRUE(m_dependencies.unlink(*m_nestedOutputA, *m_nestedInputB, m_errorReporting));
        PropertyImpl& nestedInputA = m_nodeANested->getInputs()->getChild("inputStruct")->getChild("nested")->impl();
        PropertyImpl& nestedOutputB = m_nodeBNested->getOutputs()->getChild("outputStruct")->getChild("nested")->impl();
    ...
    {
        m_dependencies.addNode(m_nodeA);
        m_dependencies.addNode(m_binding1);

        m_dependencies.addBindingDependency(m_binding1, m_nodeA);

        expectSortedNodeOrder({ &m_binding1, &m_nodeA });
    }
    ...
    TEST_F(ALogicNodeDependencies, AddsDependencyToBinding_WithLinkedNodes)
    {
        ...
        // Sorted topologically
        expectSortedNodeOrder({ &m_binding1, &m_nodeA, &m_nodeB });
    ...
    TEST_F(ALogicNodeDependencies, RemovesDependencyToBinding)
    {
        ...
        m_dependencies.removeBindingDependency(m_binding1, m_nodeA);
        expectUnsortedNodeOrder({ &m_binding1, &m_nodeA });
    }
    ...
    TEST_F(ALogicNodeDependencies, RemovesDependencyToBindingAfterNodeRemoved)
    {
        ...
        m_dependencies.removeNode(m_nodeA);
        expectUnsortedNodeOrder({ &m_binding1 });
    }
    ...
    TEST_F(ALogicNodeDependencies, RemovesDependencyToBindingAfterBindingRemoved)
    {
        ...
        m_dependencies.removeNode(m_binding1);
    ...
    {
        m_dependencies.addNode(m_binding1);
        m_dependencies.addNode(m_binding2);

        m_dependencies.addBindingDependency(m_binding1, m_binding2);
        expectSortedNodeOrder({ &m_binding1, &m_binding2 });

        m_dependencies.removeBindingDependency(m_binding1, m_binding2);

        m_dependencies.addBindingDependency(m_binding2, m_binding1);
        expectSortedNodeOrder({ &m_binding2, &m_binding1 });
    }
}
