...
{
    class ALuaScriptWithModule : public ALogicEngine
    {
    protected:
    ...
    TEST_F(ALuaScriptWithModule, CanBeCreated)
    {
        ...
        EXPECT_THAT(script->impl().getModules(), ElementsAre(Pair("mymath", mod)));
    ...
    TEST_F(ALuaScriptWithModule, ErrorIfModuleReadsGlobalVariablesWhichDontExist)
    {
        ...
        EXPECT_THAT(getLastErrorMessage(), ::testing::HasSubstr("Trying to read global variable 'peterPan' in module! This can cause undefined behavior and is forbidden!"));
    }
    ...
    TEST_F(ALuaScriptWithModule, ErrorIfModuleWritesGlobalVariables)
    {
        ...
        EXPECT_THAT(getLastErrorMessage(), ::testing::HasSubstr("Declaring global variables is forbidden in modules! (found value of type 'number' assigned to variable 'someGlobalVar')"));
    }
    ...
    TEST_F(ALuaScriptWithModule, ErrorIfModuleDoesNotReturnTable)
    {
        ...
        for (const auto& moduleSrc : errorCases)
        {
            ...
            EXPECT_EQ("[mod] Error while loading module. Module script must return a table!", getLastErrorMessage());
        }
    }
    ...
    TEST_F(ALuaScriptWithModule, ReportsErrorWhenCustomLengthFunctionCalledOnInvalidType)
    {
        ...
        EXPECT_THAT(getLastErrorMessage(), ::testing::HasSubstr("lua: error: rl_len() called on an unsupported type 'number'"));
    }
    ...
    TEST_F(ALuaScriptWithModule, SecondLevelDependenciesAreHidden)
    {
        ...
        EXPECT_FALSE(m_logicEngine->update());
        EXPECT_THAT(getLastErrorMessage(), ::testing::HasSubstr("Unexpected global access to key 'mymath' in run()! Only 'GLOBAL' is allowed as a key"));
    }
    ...
    TEST_F(ALuaScriptWithModule, CanBeSerialized)
    {
        ...
        const auto module1 = m_logicEngine->findObject<LuaModule>("mymodule1");
        const auto module2 = m_logicEngine->findObject<LuaModule>("mymodule2");
        const auto script1 = m_logicEngine->findObject<LuaScript>("script1");
        const auto script2 = m_logicEngine->findObject<LuaScript>("script2");
        ASSERT_TRUE(module1 && module2 && script1 && script2);
        EXPECT_THAT(script1->impl().getModules(), UnorderedElementsAre(Pair("mymath", module1), Pair("mymathother", module2)));
        EXPECT_THAT(script2->impl().getModules(), UnorderedElementsAre(Pair("mymath", module2)));
    ...
    TEST_F(ALuaScriptWithModule, SerializesAndDeserializesScriptWithStructPropertyInInterfaceDefinedInModule)
    {
        withTempDirectory();
        {
            ...
            LogicEngine& otherLogicEngine = *m_logicEngine;
        ...
        const LuaScript* script = m_logicEngine->findObject<LuaScript>("script");
