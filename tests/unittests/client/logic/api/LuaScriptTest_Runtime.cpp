...
namespace ramses::internal
{
    ...
    TEST_F(ALuaScript_Runtime, ReportsErrorWhenAssigningVectorComponentsIndividually)
    {
        ...
        EXPECT_THAT(getLastErrorMessage(), ::testing::HasSubstr("Error while writing to 'vec3f'. Can't assign individual components of vector types, must assign the whole vector"));
    }
    ...
    TEST_F(ALuaScript_Runtime, ProducesErrorIfUndefinedInputIsUsedInRun)
    {
        ...
        EXPECT_THAT(getLastErrorMessage(), ::testing::HasSubstr("Tried to access undefined struct property 'undefined'"));
    }
    ...
    TEST_F(ALuaScript_Runtime, ReportsSourceNodeOnRuntimeError)
    {
        ...
        ASSERT_NE(nullptr, script);
        EXPECT_FALSE(m_logicEngine->update());
        expectErrorSubstring("this causes an error", script);
    }
    ...
    TEST_F(ALuaScript_Runtime, ProducesErrorWhenTryingToWriteInputValues)
    {
        ...
        EXPECT_FALSE(m_logicEngine->update());
        expectErrorSubstring("lua: error: Error while writing to 'value'. Writing input values is not allowed, only outputs!", script);
    }
    ...
    TEST_F(ALuaScript_Runtime, ProducesErrorWhenTryingToAccessPropertiesWithNonStringIndexAtRunTime)
    {
        ...
        for (const auto& singleCase : allErrorCases)
        {
            ...
            EXPECT_THAT(getLastErrorMessage(), ::testing::HasSubstr(singleCase.expectedErrorMessage));
    ...
    TEST_F(ALuaScript_Runtime, ProducesErrorWhenAssigningNestedProperties_Underspecified)
    {
        ...
        EXPECT_FALSE(m_logicEngine->update());
        EXPECT_THAT(getLastErrorMessage(), ::testing::HasSubstr("lua: error: Error while assigning struct 'data', expected a value for property 'field2' but found none!"));
    ...
    TEST_F(ALuaScript_Runtime, ProducesErrorWhenAssigningNestedProperties_Overspecified)
    {
        ...
        EXPECT_FALSE(m_logicEngine->update());
        EXPECT_THAT(getLastErrorMessage(), ::testing::HasSubstr("Unexpected property 'not_specified' while assigning values to struct 'data'!"));
    }
    ...
    TEST_F(ALuaScript_Runtime, ProducesErrorWhenAssigningNestedProperties_WhenFieldHasWrongType)
    {
        ...
        EXPECT_FALSE(m_logicEngine->update());
        EXPECT_THAT(getLastErrorMessage(), ::testing::HasSubstr("Assigning string to 'Int32' output 'field2'!"));
    ...
    TEST_F(ALuaScript_Runtime, ProducesErrorWhenAssigningNestedProperties_WhenNestedSubStructDoesNotMatch_AndInterruptsAssignment)
    {
        ...
        EXPECT_FALSE(m_logicEngine->update());
        EXPECT_THAT(getLastErrorMessage(), ::testing::HasSubstr("Unexpected property 'wrong_field' while assigning values to struct 'b_nested'"));
    ...
    TEST_F(ALuaScript_Runtime, ProducesErrorWhenAccessingArrayWithNonIntegerIndex)
    {
        ...
        for (const auto& invalidStatement : invalidStatements)
        {
            ...
            EXPECT_THAT(getLastErrorMessage(), ::testing::HasSubstr("Bad access to property 'array'! Error while extracting integer: expected a number, received"));
            m_logicEngine->destroy(*script);
        }
    }
    ...
    TEST_F(ALuaScript_Runtime, ProducesErrorWhenAssigningArrayWithFewerElementsThanRequired_UsingExplicitIndices)
    {
        ...
        EXPECT_THAT(getLastErrorMessage(), ::testing::HasSubstr("Error during assignment of array property 'int_array'! Expected a value at index 3"));
    }
    ...
    TEST_F(ALuaScript_Runtime, ProducesErrorWhenAssigningArrayFromLuaTableWithCorrectSizeButWrongIndices)
    {
        ...
        EXPECT_THAT(getLastErrorMessage(), ::testing::HasSubstr("Error during assignment of array property 'int_array'! Expected a value at index 2"));
    }
    ...
    TEST_F(ALuaScript_Runtime, ReportsErrorWhenAssigningArraysWithMismatchedSizes)
    {
        ...
        for (const auto& aCase : allCases)
        {
            ...
            EXPECT_THAT(getLastErrorMessage(), ::testing::HasSubstr(aCase.expectedErrorMessage));
            EXPECT_TRUE(m_logicEngine->destroy(*script));
        }
    }
    ...
    TEST_F(ALuaScript_Runtime, ProducesErrorWhenAssigningNilToIntOutputs)
    {
        ...
        EXPECT_FALSE(m_logicEngine->update());
        EXPECT_THAT(getLastErrorMessage(), ::testing::HasSubstr("Assigning nil to 'Int32' output 'int'!"));
    ...
    TEST_F(ALuaScript_Runtime, ProducesErrorWhenAssigningBoolToIntOutputs)
    {
        ...
        EXPECT_FALSE(m_logicEngine->update());
        EXPECT_THAT(getLastErrorMessage(), ::testing::HasSubstr("Assigning bool to 'Int32' output 'int'!"));
        EXPECT_EQ(0, *script->getOutputs()->getChild("int")->get<int32_t>());
    }
    ...
    TEST_F(ALuaScript_Runtime, ProducesErrorWhenAssigningBoolToStringOutputs)
    {
        ...
        EXPECT_FALSE(m_logicEngine->update());
        EXPECT_THAT(getLastErrorMessage(), ::testing::HasSubstr("Assigning bool to 'String' output 'str'!"));
    ...
    TEST_F(ALuaScript_Runtime, ProducesErrorWhenAssigningNumberToStringOutputs)
    {
        ...
        EXPECT_FALSE(m_logicEngine->update());
        EXPECT_THAT(getLastErrorMessage(), ::testing::HasSubstr("Assigning number to 'String' output 'str'!"));
    }
    ...
    TEST_F(ALuaScript_Runtime, ForbidsCallingInterfaceFunctionInsideTheRunFunction)
    {
        ...
        EXPECT_FALSE(m_logicEngine->update());
        EXPECT_THAT(getLastErrorMessage(), ::testing::HasSubstr("Unexpected global access to key 'interface' in run()!"));
    }
    ...
    TEST_F(ALuaScript_Runtime, HasNoInfluenceOnBindingsIfTheyAreNotLinked)
    {
        ...
        EffectDescription ramsesEffectDesc;
        ramsesEffectDesc.setVertexShader(vertexShaderSource.data());
        ramsesEffectDesc.setFragmentShader(fragmentShaderSource.data());
        auto ramsesEffect     = m_scene->createEffect(ramsesEffectDesc);
        auto ramsesAppearance = m_scene->createAppearance(*ramsesEffect);
        ...
        auto appearanceBinding = m_logicEngine->createAppearanceBinding(*ramsesAppearance, "AppearanceBinding");
        auto cameraBinding = m_logicEngine->createCameraBinding(*camera, "CameraBinding");
    ...
    TEST_F(ALuaScript_RuntimeIterators, Custom_Pairs_BehavesTheSameAsStandard_Pairs_Function_ForStructs)
    {
        std::string_view scriptSrc = R"(
            ...
            function run(IN,OUT)
                ...
                -- compare iteration results to a static reference table
                -- (reference table is split into 2 arrays to guarantee stable sorting)
                local refKeys = {"bool", "int", "nested"}
                local refValues = {false, 42, {int = 42, bool = false, nested = {}}}

                -- test multiple containers (which all have the same contents)
                local objectsToCheck = {IN, IN.nested, OUT, OUT.nested}
                for k, container in pairs(objectsToCheck) do
                    -- iterate manually over reference table...
                    local kKey, refKey = next(refKeys)
                    local kValue, refValue = next(refValues)
                    -- ...and compare to rl_pairs results
                    for key, value in rl_pairs(container) do
                        ...
                        -- progress refTable manually
                        kKey, refKey = next(refKeys, kKey)
                        kValue, refValue = next(refValues, kValue)
                    end

                    -- make sure there are no leftover elements
                    assert(refKey == nil)
                    assert(refValue == nil)
                end
