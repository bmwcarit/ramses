...
{
    class AnAnimationNode : public ALogicEngineBase, public ::testing::TestWithParam<bool>
    {
    public:
        void SetUp() override
        {
    ...
    TEST_P(AnAnimationNode, IsCreated)
    {
        ...
        EXPECT_EQ(animNode, m_logicEngine->findObject<AnimationNode>("animNode"));
    ...
    TEST_P(AnAnimationNode, IsDestroyed)
    {
        ...
        expectNoError();
        EXPECT_EQ(nullptr, m_logicEngine->findObject<AnimationNode>("animNode"));
    }
    ...
    TEST_P(AnAnimationNode, ChangesName)
    {
        ...
        EXPECT_EQ(animNode, m_logicEngine->findObject<AnimationNode>("an"));
        expectNoError();
    }
    ...
    TEST_P(AnAnimationNode, FailsToBeCreatedWithNoChannels)
    {
        ...
        EXPECT_EQ("Failed to create AnimationNode 'animNode': must provide at least one channel.", getLastErrorMessage());
    }

    TEST_P(AnAnimationNode, FailsToBeCreatedIfDataArrayFromOtherLogicInstance)
    {
        auto& otherInstance = *m_scene->createLogicEngine();
        ...
        EXPECT_EQ("Failed to create AnimationNode 'animNode': timestamps or keyframes were not found in this logic instance.", getLastErrorMessage());
        ...
        EXPECT_EQ("Failed to create AnimationNode 'animNode': tangents were not found in this logic instance.", getLastErrorMessage());
    ...
    TEST_P(AnAnimationNode, CanBeSerializedAndDeserialized)
    {
        ...
        const auto animNode1 = m_logicEngine->findObject<AnimationNode>("animNode1");
        const auto animNode2 = m_logicEngine->findObject<AnimationNode>("animNode2");
        ...
        const auto ts1 = m_logicEngine->findObject<DataArray>("ts1");
        const auto ts2 = m_logicEngine->findObject<DataArray>("ts2");
        const auto data1 = m_logicEngine->findObject<DataArray>("data1");
        const auto data2 = m_logicEngine->findObject<DataArray>("data2");
        const auto data3 = m_logicEngine->findObject<DataArray>("data3");
    ...
    TEST_P(AnAnimationNode, WillSerializeAnimationIncludingProgress)
    {
        ...
        ASSERT_TRUE(recreateFromFile("logic_animNodes.bin"));
        const auto animNode = m_logicEngine->findObject<AnimationNode>("animNode");
    ...
    protected:
        ...
        std::unique_ptr<AnimationNodeImpl> deserializeSerializedDataWithIssue(ESerializationIssue issue)
        {
...

            {
                ...
                const auto dataFb = DataArrayImpl::Serialize(data->impl(), flatBufferBuilder, serializationMap);
    ...
    TEST_P(AnAnimationNode_SerializationLifecycle, FailsDeserializationIfEssentialDataMissing)
    {
        ...
        for (const auto issue : { ESerializationIssue::NameMissing, ESerializationIssue::IdMissing, ESerializationIssue::ChannelsMissing, ESerializationIssue::RootInMissing, ESerializationIssue::RootOutMissing })
        {
            ...
            EXPECT_EQ("Fatal error during loading of AnimationNode from serialized data: missing name, id, channels or in/out property data!", m_errorReporting.getError()->message);
    ...
    TEST_P(AnAnimationNode_SerializationLifecycle, FailsDeserializationIfChannelDataMissing)
    {
        for (const auto issue : { ESerializationIssue::ChannelTimestampsMissing, ESerializationIssue::ChannelKeyframesMissing })
        {
            EXPECT_FALSE(deserializeSerializedDataWithIssue(issue));
            ASSERT_TRUE(m_errorReporting.getError().has_value());
            EXPECT_EQ("Fatal error during loading of AnimationNode 'animNode' channel data: missing name, timestamps or keyframes!", m_errorReporting.getError()->message);
            m_errorReporting.reset();
        }
    }
    ...
    TEST_P(AnAnimationNode_SerializationLifecycle, FailsDeserializationIfTangentsMissing)
    {
        for (const auto issue : { ESerializationIssue::ChannelTangentsInMissing, ESerializationIssue::ChannelTangentsOutMissing })
        {
            EXPECT_FALSE(deserializeSerializedDataWithIssue(issue));
            ASSERT_TRUE(m_errorReporting.getError().has_value());
            EXPECT_EQ("Fatal error during loading of AnimationNode 'animNode' channel 'channel' data: missing tangents!", m_errorReporting.getError()->message);
            m_errorReporting.reset();
        }
    }
    ...
    TEST_P(AnAnimationNode_SerializationLifecycle, FailsDeserializationIfInvalidInterpolationType)
    {
        ...
        ASSERT_TRUE(m_errorReporting.getError().has_value());
        EXPECT_EQ("Fatal error during loading of AnimationNode 'animNode' channel 'channel' data: missing or invalid interpolation type!", m_errorReporting.getError()->message);
    }
    ...
    TEST_P(AnAnimationNode_SerializationLifecycle, FailsDeserializationIfInvalidOrMissingProperties)
    {
        for (const auto issue : { ESerializationIssue::PropertyInMissing, ESerializationIssue::PropertyOutMissing, ESerializationIssue::PropertyInWrongName, ESerializationIssue::PropertyOutWrongName })
        {
            EXPECT_FALSE(deserializeSerializedDataWithIssue(issue));
            ASSERT_TRUE(m_errorReporting.getError().has_value());
            EXPECT_EQ("Fatal error during loading of AnimationNode 'animNode': missing or invalid properties!", m_errorReporting.getError()->message);
            m_errorReporting.reset();
        }
    }
    ...
    TEST_P(AnAnimationNode_SerializationLifecycle, FailsDeserializationIfInvalidChannelsData)
    {
        ...
        ASSERT_TRUE(m_errorReporting.getError().has_value());
        EXPECT_EQ("Fatal error during loading of AnimationNode 'animNode': missing or invalid channels data property!", m_errorReporting.getError()->message);
    }
}
