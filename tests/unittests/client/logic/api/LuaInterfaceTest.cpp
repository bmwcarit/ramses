...
{
    class ALuaInterface : public ALogicEngine
    {
    protected:
    ...
    TEST_F(ALuaInterface, ReportsErrorIfNoInterfaceFunctionDefined)
    {
        ...
        EXPECT_STREQ("[intf name] No 'interface' function defined!", getLastErrorMessage().c_str());
    }
    ...
    TEST_F(ALuaInterfaceWithModule, FailsToBeCreatedWhenOverwritingModuleData)
    {
        ...
        EXPECT_THAT(getLastErrorMessage(), ::testing::HasSubstr("Modifying module data is not allowed!"));
    }
    ...
    TEST_F(ALuaInterfaceWithModule, FailsToBeCreatedWhenUsingModuleOverwritingTypeSymbols)
    {
        ...
        EXPECT_EQ(nullptr, intf);
        EXPECT_THAT(getLastErrorMessage(), ::testing::HasSubstr("Special global 'Type' symbol should not be overwritten in modules!"));
    }
    ...
    TEST_F(ALuaInterfaceWithModule, FailsToBeCreatedWhenUsingModuleFromAnotherLogicInstance)
    {
        ...
        EXPECT_EQ(nullptr, intf);
        EXPECT_EQ(getLastErrorMessage(), "Failed to map Lua module 'mymod'! It was created on a different instance of LogicEngine.");
    }
    ...
    TEST_F(ALuaInterfaceWithModule, CanBeSerializedAndLoadedInBaseFeatureLevel_confidenceTest) // just to verify that module dependency does not affect serialized interface
    {
        ...
        const auto intf = m_logicEngine->findObject<LuaInterface>("intf");
        ASSERT_NE(intf, nullptr);
    ...
    TEST_F(ALuaInterfaceWithModule, VerifiesModulesDeclaredInScriptWhenCreatingInterface)
    {
        constexpr std::string_view intfSrc = R"(
        ...
        EXPECT_THAT(getLastErrorMessage(), ::testing::HasSubstr("Module dependencies declared in source code do not match those provided by LuaConfig"));
    }
        ...
        {
            {
                ...
                auto intf = rlogic_serialization::CreateLuaInterface(m_flatBufferBuilder,
                    issue == ESerializationIssue::NameIdMissing ? 0 : rlogic_serialization::CreateLogicObject(m_flatBufferBuilder, m_flatBufferBuilder.CreateString(name), 1u),
    ...
    TEST_F(ALuaInterface_Serialization, CanSerializeAndDeserializeLuaInterface)
    {
        ...
        // Serialize
        {
            LogicEngine& otherEngine = *m_logicEngine;
            LuaScript* inputsScript = otherEngine.createLuaScript(R"LUA_SCRIPT(
                function interface(IN,OUT)
                    IN.param1 = Type:Int32()
            ...
            LuaInterface* intf = otherEngine.createLuaInterface(R"(
    ...
    TEST_F(ALuaInterface_Serialization, FailsDeserializationIfEssentialDataMissing)
    {
        ...
        EXPECT_EQ("Fatal error during loading of LuaInterface from serialized data: missing name and/or ID!", m_errorReporting.getError()->message);
    }
    ...
    TEST_F(ALuaInterface_Serialization, FailsDeserializationIfNameEmpty)
    {
        ...
        ASSERT_TRUE(m_errorReporting.getError().has_value());
        EXPECT_EQ("Fatal error during loading of LuaInterface from serialized data: empty name!", m_errorReporting.getError()->message);
    }
    ...
    TEST_F(ALuaInterface_Serialization, FailsDeserializationIfRootPropertyMissing)
    {
        ...
        ASSERT_TRUE(m_errorReporting.getError().has_value());
        EXPECT_EQ("Fatal error during loading of LuaInterface from serialized data: missing root property!", m_errorReporting.getError()->message);
    }
    ...
    TEST_F(ALuaInterface_Serialization, FailsDeserializationIfRootNotStructType)
    {
        ...
        ASSERT_TRUE(m_errorReporting.getError().has_value());
        EXPECT_EQ("Fatal error during loading of LuaScript from serialized data: root property has unexpected type!", m_errorReporting.getError()->message);
    }
}
