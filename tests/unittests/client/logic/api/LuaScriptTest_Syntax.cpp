...
namespace ramses::internal
{
    ...
    TEST_F(ALuaScript_Syntax, ProducesErrorIfNoInterfaceIsPresent)
    {
        ...
        EXPECT_THAT(getLastErrorMessage(), ::testing::HasSubstr("[scriptNoInterface] No 'interface' function defined!"));
    }
    ...
    TEST_F(ALuaScript_Syntax, ProducesErrorIfNoRunIsPresent)
    {
        ...
        EXPECT_THAT(getLastErrorMessage(), ::testing::HasSubstr("[scriptNoRun] No 'run' function defined!"));
    }
    ...
    TEST_F(ALuaScript_Syntax, CannotBeCreatedFromSyntacticallyIncorrectScript)
    {
        ...
        EXPECT_THAT(getLastErrorMessage(), ::testing::HasSubstr("'<name>' expected near 'not'"));
    }
    ...
    TEST_F(ALuaScript_Syntax, ProducesErrorIfScriptReturnsValue)
    {
        ...
        ASSERT_EQ(nullptr, scriptNoRun);
        EXPECT_THAT(getLastErrorMessage(), ::testing::HasSubstr("Expected no return value in script source, but a value of type 'string' was returned!"));
    }
        ...
        for (const auto& typeSizePair : sizeOfEachType)
        {
            ...
            for (int32_t i = -1; i <= componentCount + 1; ++i)
            {
                ...
                if (i < 1 || i > componentCount)
                {
                    ...
                    if (i < 0)
                    {
                        expectErrorSubstring("Only non-negative integers supported as array index type! Error while extracting integer: expected non-negative number, received '-1'", script);
                    }
                    else
                    {
                        expectErrorSubstring(fmt::format("Bad index '{}', expected 1 <= i <= {}", i, componentCount), script);
                    }
                }
                else
                {
    ...
    TEST_F(ALuaScript_Syntax, RaisesErrorWhenTryingToGetSizeOfNonArrayTypes)
    {
        ...
        expectErrorSubstring("attempt to get length of field 'notArray' (a number value)", script);
    }
    ...
    TEST_F(ALuaScript_Syntax, ProdocesErrorWhenIndexingVectorWithNonIntegerIndices)
    {
        ...
        for (const auto& error : errorTypes)
        {
            ...
            EXPECT_FALSE(m_logicEngine->update());
            expectErrorSubstring("Only non-negative integers supported as array index type!", script);
        }
    }
    ...
    TEST_F(ALuaScript_Syntax, ReportsErrorWhenTryingToAssignVectorTypes_WithMismatchedComponentCount)
    {
        ...
        for(const auto& errorCase : allCases)
        {
            ...
            ASSERT_NE(nullptr, script);
            EXPECT_FALSE(m_logicEngine->update());
            expectErrorSubstring(errorCase.expectedErrorMessage, script);
    ...
    TEST_F(ALuaScript_Syntax, ProducesErrorIfRunFunctionDoesNotEndCorrectly)
    {
        ...
        EXPECT_THAT(getLastErrorMessage(), ::testing::HasSubstr("'=' expected near '<eof>'"));
    }
    ...
    TEST_F(ALuaScript_Syntax, ProducesErrorIfInterfaceFunctionDoesNotEndCorrectly)
    {
        ...
        EXPECT_THAT(getLastErrorMessage(), ::testing::HasSubstr("'=' expected near 'function'"));
    }
    ...
    TEST_F(ALuaScript_Syntax, ProducesErrorIfInterfaceFunctionDoesNotEndAtAll)
    {
        ...
        EXPECT_THAT(getLastErrorMessage(), ::testing::HasSubstr("'end' expected (to close 'function' at line 2) near '<eof>'"));
    }
    ...
    TEST_F(ALuaScript_Syntax, ProducesErrorIfRunFunctionDoesNotEndAtAll)
    {
        ...
        EXPECT_THAT(getLastErrorMessage(), ::testing::HasSubstr("'end' expected (to close 'function' at line 4) near '<eof>'"));
    }
