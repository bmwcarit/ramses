...
namespace ramses::internal
{
    ...
    {
    protected:
        ApiObjects& m_apiObjects = { m_logicEngine->impl().getApiObjects() };
    ...
    TEST_F(ALogicEngine_Dirtiness, CreatedObjectsAreDirtyAfterCreating)
    {
        ...
        EXPECT_TRUE(obj1->impl().isDirty());
        const auto obj2 = m_logicEngine->createLuaInterface(m_valid_empty_interface, "iface name");
        EXPECT_TRUE(obj2->impl().isDirty());
    }

    TEST_F(ALogicEngine_Dirtiness, CreatedBindingsAreNotDirtyAfterCreating)
    {
        const auto obj3 = m_logicEngine->createNodeBinding(*m_node, ramses::ERotationType::Euler_XYZ, "");
        EXPECT_FALSE(obj3->impl().isDirty());
        const auto obj4 = m_logicEngine->createAppearanceBinding(*m_appearance, "");
        EXPECT_FALSE(obj4->impl().isDirty());
        const auto obj5 = m_logicEngine->createRenderPassBinding(*m_renderPass, "");
        EXPECT_FALSE(obj5->impl().isDirty());
        const auto obj6 = createRenderGroupBinding();
        EXPECT_FALSE(obj6->impl().isDirty());
        const auto obj = m_logicEngine->createMeshNodeBinding(*m_meshNode);
        EXPECT_FALSE(obj->impl().isDirty());
    }
    ...
    TEST_F(ALogicEngine_Dirtiness, NotDirty_AfterCreatingObjectsAndCallingUpdate)
    {
        ...
        const auto obj3 = m_logicEngine->createNodeBinding(*m_node, ramses::ERotationType::Euler_XYZ, "");
        const auto obj4 = m_logicEngine->createAppearanceBinding(*m_appearance, "");
        const auto obj5 = m_logicEngine->createCameraBinding(*m_camera, "");
        const auto obj6 = m_logicEngine->createRenderPassBinding(*m_renderPass, "");
        const auto obj7 = createRenderGroupBinding();
        const auto obj8 = m_logicEngine->createMeshNodeBinding(*m_meshNode);

        m_logicEngine->update();
        EXPECT_FALSE(obj1->impl().isDirty());
        EXPECT_FALSE(obj2->impl().isDirty());
        EXPECT_FALSE(obj3->impl().isDirty());
        EXPECT_FALSE(obj4->impl().isDirty());
        EXPECT_FALSE(obj5->impl().isDirty());
        EXPECT_FALSE(obj6->impl().isDirty());
        EXPECT_FALSE(obj7->impl().isDirty());
        EXPECT_FALSE(obj8->impl().isDirty());
    }
    ...
    TEST_F(ALogicEngine_Dirtiness, Dirty_AfterSettingScriptInput)
    {
        ...
        EXPECT_TRUE(script->impl().isDirty());
        m_logicEngine->update();
        EXPECT_FALSE(script->impl().isDirty());
    }
    ...
    TEST_F(ALogicEngine_Dirtiness, Dirty_AfterSettingInterfaceInput)
    {
        ...
        EXPECT_TRUE(intf->impl().isDirty());
        m_logicEngine->update();
        EXPECT_FALSE(intf->impl().isDirty());
    }

    TEST_F(ALogicEngine_Dirtiness, Dirty_WhenSettingBindingInputToDefaultValue)
    {
        NodeBinding* binding = m_logicEngine->createNodeBinding(*m_node, ramses::ERotationType::Euler_XYZ, "");
        m_logicEngine->update();
    ...
    TEST_F(ALogicEngine_Dirtiness, Dirty_WhenSettingBindingInputToDifferentValue)
    {
        ...
        binding->getInputs()->getChild("translation")->set<vec3f>({ 1, 2, 3 });
        m_logicEngine->update();
        EXPECT_FALSE(binding->impl().isDirty());
    ...
    {
    protected:
        void link(Property& src, Property& dst)
        {
            if (GetParam())
            {
    ...
    TEST_P(ALogicEngine_DirtinessViaLink, Dirty_WhenAddingLink)
    {
        ...
        EXPECT_TRUE(script1->impl().isDirty());
        EXPECT_TRUE(script2->impl().isDirty());
        m_logicEngine->update();
        EXPECT_FALSE(script1->impl().isDirty());
        EXPECT_FALSE(script2->impl().isDirty());
    }
    ...
    {
        m_logicEngine->createNodeBinding(*m_node, ramses::ERotationType::Euler_XYZ, "");
        EXPECT_FALSE(m_apiObjects.bindingsDirty());
    }

    TEST_F(ALogicEngine_BindingDirtiness, NotDirtyAfterCreatingAppearanceBinding)
    {
        m_logicEngine->createAppearanceBinding(*m_appearance, "");
        EXPECT_FALSE(m_apiObjects.bindingsDirty());
    }
    ...
    TEST_F(ALogicEngine_BindingDirtiness, Dirty_WhenSettingDataToNestedAppearanceBindingInputs)
    {
        ...
        AppearanceBinding* binding = m_logicEngine->createAppearanceBinding(RamsesTestSetup::CreateTestAppearance(*m_scene, vertShader_array, fragShader_trivial), "");
