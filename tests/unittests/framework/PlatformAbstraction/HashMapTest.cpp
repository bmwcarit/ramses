...
struct std::hash<MyStruct>
{
    ...
    {
        return ramses::internal::HashValue(c.a, c.b);
    }
};
...
struct std::hash<SomeClass>
{
    ...
    {
        return ramses::internal::HashValue(c.i);
    }
};
...
using RCKey = ramses::internal::ComplexTestType<struct KeyTag>;
using RCValue = ramses::internal::ComplexTestType<struct ValueTag>;
...
public:
    ...
    static void ExpectRefCnt(int32_t refCnt)
    {
...
TEST_F(HashMapTest, CanInsertWithoutRehashWhenConstructedWithCapacity)
{
    ...
    {
        SCOPED_TRACE(param);
        HashMap<uint32_t, uint32_t> map(param);
