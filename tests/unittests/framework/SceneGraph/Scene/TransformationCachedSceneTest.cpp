...
namespace ramses::internal
{
    ...
    public:
        ATransformationCachedScene()
            : scene(SceneInfo())
        {
            this->nodeWithoutTransform = this->scene.allocateNode(0, {});
            this->nodeWithTransform = this->scene.allocateNode(0, {});
    ...
    TEST_F(ATransformationCachedScene, GivesCorrectValueWhenRotationConventionIsSetDifferentFromParent)
    {
        ...
        const NodeHandle childNode = this->scene.allocateNode(0, {});
    ...
    {
        // add parent with transform
        const NodeHandle parent = this->scene.allocateNode(0, {});
    ...
    {
        // add parent with transform
        const NodeHandle childLeft = this->scene.allocateNode(0, {});
        const NodeHandle childRight = this->scene.allocateNode(0, {});
    ...
    {
        const NodeHandle parent = this->scene.allocateNode(0, {});
        const NodeHandle child = this->scene.allocateNode(0, {});
        const TransformHandle parentTransform = this->scene.allocateTransform(parent, {});
