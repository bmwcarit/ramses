...
namespace ramses::internal
{
    ...
    TYPED_TEST(AScene, CreatesATransformDataSlot)
    {
        ...
        const DataSlotHandle handle = this->m_scene.allocateDataSlot({ EDataSlotType::TransformationConsumer, DataSlotId(6), NodeHandle(5), dataRef, textureRes, sampler }, requestedHandle);
    ...
    {
        const DataSlotHandle handle = this->m_scene.allocateDataSlot({EDataSlotType::TransformationConsumer, DataSlotId(6), NodeHandle(5), DataInstanceHandle(), ResourceContentHash::Invalid(), TextureSamplerHandle()}, {});
