...
namespace ramses::internal
{
    ...
    TYPED_TEST(AScene, SetsTheValueOfADataInstanceProperty)
    {
        ...
        EXPECT_EQ(false                             , this->m_scene.getDataSingleBoolean                  (containerHandle, DataFieldHandle(17u)));
        ...
        this->m_scene.setDataSingleBoolean        (containerHandle, DataFieldHandle(17u), true);
        ...
        EXPECT_EQ(true                                    , this->m_scene.getDataSingleBoolean         (containerHandle, DataFieldHandle(17u)));
    ...
    TYPED_TEST(AScene, DataInstanceWithFieldWithElementCountGreaterOneReturnsSameValue)
    {
        ...
        const std::array<int32_t, 4> inValues = { 1, 2, 3, 4 };
        this->m_scene.setDataIntegerArray(instance, field, 4, inValues.data());

        const int32_t* outValues = this->m_scene.getDataIntegerArray(instance, field);
    ...
    TYPED_TEST(AScene, DataInstanceFieldsWithElementCountGreaterOneDoesNotInfluenceOtherField)
    {
        ...
        const ResourceContentHash inValue1(std::numeric_limits<uint64_t>::max(), std::numeric_limits<uint64_t>::max());
        const uint32_t inDivisor = std::numeric_limits<uint32_t>::max();
        const uint16_t offsetInBytes{ 89u };
        const uint16_t stride{ 88u };
