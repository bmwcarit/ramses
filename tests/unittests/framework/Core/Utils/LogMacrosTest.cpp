...
namespace ramses::internal
{
    ...
    {
        LOG_INFO(CONTEXT_FRAMEWORK, "Foo {} {}", 123, 456.f);
    }

    TEST(ALogMacro, canUseFmtlibLambdaLog)
    {
        LOG_INFO_PF(CONTEXT_FRAMEWORK, ([&](fmt::memory_buffer& out) {
            fmt::format_to(std::back_inserter(out), "Foo {} {}", 123, 456.f);
        }));
        LOG_INFO_PF(CONTEXT_FRAMEWORK, ([&](auto& out) { fmt::format_to(std::back_inserter(out), "Foo {} {}", 123, 456.f); }));
    }
}
