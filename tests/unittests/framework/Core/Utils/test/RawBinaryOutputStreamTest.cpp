...
namespace ramses::internal
{
    ...
    {
        std::vector<std::byte> buffer(450);
        std::vector<std::byte> refBuffer(450);
        RawBinaryOutputStream  os(buffer.data(), buffer.size());
        ...
        std::vector<std::byte> dVec(432);
        std::generate(dVec.begin(), dVec.end(), [](){ static uint8_t i{126}; return std::byte(++i); });
    ...
    TEST(ARawBinaryOutputStream, canWriteExactlyFittingData)
    {
        ...
        std::vector<std::byte> refBuffer(4);
        RawBinaryOutputStream  os(buffer.data(), buffer.size());
