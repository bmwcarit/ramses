...
namespace ramses::internal
{
    ...
    {
    public:
        explicit LongRunningTestTask(uint32_t howLong = 50)
            : time(howLong)
        {
    ...
    class AThreadedTaskExecutorParam : public ::testing::TestWithParam<int>
    {
    };
    // Notification interval 0 is not expected to notify (forbidden by RamsesFrameworkConfig)
    INSTANTIATE_TEST_SUITE_P(AThreadedTaskExecutorParam_TestInstances, AThreadedTaskExecutorParam, ::testing::Values(1, 2, 100));

    TEST_P(AThreadedTaskExecutorParam, reportsToWatchDogNotifierWhenAllThreadsReportAlive)
    {
        PlatformEvent syncWaiter;
        PlatformWatchdogMockCallback mockCallback;
        EXPECT_CALL(mockCallback, registerThread(ERamsesThreadIdentifier::Workers));
        ThreadWatchdogConfig config;
        config.setThreadWatchDogCallback(&mockCallback);
        config.setWatchdogNotificationInterval(ERamsesThreadIdentifier::Workers, GetParam());

        EXPECT_CALL(mockCallback, notifyThread(ERamsesThreadIdentifier::Workers))
            .Times(AtLeast(3))
        ...
        EXPECT_CALL(mockCallback, unregisterThread(ERamsesThreadIdentifier::Workers));
    }
    ...
    TEST(AThreadedTaskExecutor, doesNotReportToWatchDogIfOneThreadIsBlocked)
    {
        ...
        ON_CALL(mockCallback, notifyThread(_)).WillByDefault([&](auto /*unused*/) { notifyCounter++; });
        ...
        ThreadWatchdogConfig config;
        config.setWatchdogNotificationInterval(ERamsesThreadIdentifier::Workers, 0); // no time limitations - always notify
        config.setThreadWatchDogCallback(&mockCallback);
    ...
    {
        auto* ex = new ThreadedTaskExecutor(16);
    ...
    TEST(AThreadedTaskExecutor, unblockThreadsWhenStopping)
    {
        ...
        ThreadWatchdogConfig config;
        config.setThreadWatchDogCallback(&mockCallback);
        config.setWatchdogNotificationInterval(ERamsesThreadIdentifier::Workers, 10000);
        ...
        EXPECT_TRUE(syncWaiter.wait(config.getWatchdogNotificationInterval(ERamsesThreadIdentifier::Workers)));
    ...
    {
        auto* ex = new ThreadedTaskExecutor(1);
