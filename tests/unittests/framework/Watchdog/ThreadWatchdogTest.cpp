...
namespace ramses::internal
{
    ...
    public:
        void SetUp() override
        {
            ...
            config.setWatchdogNotificationInterval(ERamsesThreadIdentifier::Workers, 0);
            EXPECT_CALL(mockCallback, registerThread(_));
        }

        void createWatchdog()
        {
            watchdog = std::make_unique<ThreadWatchdog>(config, ERamsesThreadIdentifier::Workers);
        }
    ...
    TEST(AThreadWatchdog_, registersUnregistersWithCorrectThreadIdentifier)
    {
...

        {
            ...
            ThreadWatchdog watchdog(config, ERamsesThreadIdentifier::Workers);

            Mock::VerifyAndClearExpectations(&mockCallback);
            EXPECT_CALL(mockCallback, unregisterThread(ERamsesThreadIdentifier::Workers));
        }
        {
            EXPECT_CALL(mockCallback, registerThread(ERamsesThreadIdentifier::Renderer));
            ThreadWatchdog watchdog(config, ERamsesThreadIdentifier::Renderer);

            Mock::VerifyAndClearExpectations(&mockCallback);
            EXPECT_CALL(mockCallback, unregisterThread(ERamsesThreadIdentifier::Renderer));
        }
    }
    ...
    {
        config.setWatchdogNotificationInterval(ERamsesThreadIdentifier::Workers, static_cast<uint32_t>(-1));
        createWatchdog();
        auto t0 = watchdog->registerThread();
        auto t1 = watchdog->registerThread();
