...
namespace ramses::internal
{
    ...
    public:
        ...
        ramses::internal::TestRenderer& getTestRenderer();
        TestScenes& getScenesRegistry();
        ramses::RamsesClient& getClient();

        RenderingTestCase& createTestCase(uint32_t id, IRendererTest& rendererTest, const std::string& name);
        RenderingTestCase& createTestCaseWithDefaultDisplay(uint32_t id, IRendererTest& rendererTest, const std::string& name, bool iviWindowStartVisible = true);
        RenderingTestCase& createTestCaseWithoutRenderer(uint32_t id, IRendererTest& rendererTest, const std::string& name);
        ...
        ramses::displayBufferId_t   createOffscreenBuffer(uint32_t testDisplayIdx, uint32_t width, uint32_t height, bool interruptible, uint32_t sampleCount = 0u, ramses::EDepthBufferType depthBufferType = ramses::EDepthBufferType::DepthStencil);
        ...
        void setClearFlags(uint32_t testDisplayIdx, ramses::displayBufferId_t ob, ramses::ClearFlags clearFlags);
        ...
        bool renderAndCompareScreenshotOffscreenBuffer(const std::string& expectedImageName,
                                                    uint32_t testDisplayIdx, ramses::displayBufferId_t displayBuffer, uint32_t width, uint32_t height,
                                                    float maxAveragePercentErrorPerPixel = RendererTestUtils::DefaultMaxAveragePercentPerPixel);
        bool renderAndCompareScreenshotSubimage(const std::string& expectedImageName,
                                                uint32_t subimageX, uint32_t subimageY, uint32_t subimageWidth, uint32_t subimageHeight,
        ...
        void filterTestCases(const std::vector<std::string>& filterIn, const std::vector<std::string>& filterOut);

        bool runAllTests();
        [[nodiscard]] std::string generateReport() const;

        static bool NameMatchesFilter(const std::string& name, const std::vector<std::string>& filter);
    ...
    private:
        ...
        ramses::internal::TestRenderer&  m_testRenderer;
