...
namespace ramses::internal
{
    ...
    private:
        ...
        sceneId_t createAndShowScene(EmbeddedCompositingTestsFramework& testFramework, uint32_t sceneState, uint32_t vpWidth, uint32_t vpHeight, uint32_t displayIdx)
        {
            const sceneId_t sceneId = testFramework.getScenesRegistry().createScene<INTEGRATION_SCENE>(sceneState, { 0, 0, 0 }, vpWidth, vpHeight);
