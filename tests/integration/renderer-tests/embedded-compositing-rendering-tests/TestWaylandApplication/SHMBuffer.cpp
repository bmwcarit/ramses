...
namespace ramses::internal
{
        ...
        , m_isFree(true)
    {
        ...
        if (fd < 0)
        {
            LOG_ERROR(CONTEXT_RENDERER, "SHMBuffer::SHMBuffer CreateAnonymousFile failed!");
            return;
        }

        m_pixelData = static_cast<uint8_t*>(mmap(nullptr, m_size, PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0));
        if (m_pixelData == MAP_FAILED) // NOLINT(cppcoreguidelines-pro-type-cstyle-cast)
        {
            LOG_ERROR(CONTEXT_RENDERER, "SHMBuffer::SHMBuffer mmap failed!");
            close(fd);
            return;
        }
    ...
    void SHMBuffer::ReleaseCallback(void* data, [[maybe_unused]] wl_buffer* buffer)
    {
    ...
    int SHMBuffer::CreateTmpFileCloexec(const std::string& tmpname)
    {
        ...
        {
            LOG_ERROR(CONTEXT_RENDERER, "SHMBuffer::CreateTmpFileCloexec failed!");
        }
        return fd;
    }
    ...
    int SHMBuffer::CreateAnonymousFile(off_t size)
    {
        ...
        if (!path)
        {
            LOG_ERROR(CONTEXT_RENDERER, "SHMBuffer::CreateAnonymousFile XDG_RUNTIME_DIR not set!");
            return -1;
        }
