...
namespace ramses::internal
{
    ...
    {
    public:
        EmbeddedCompositingTestsFramework(bool generateScreenshots, TestForkingController& testForkingController, const RamsesFrameworkConfig& config);
        ...
        bool                            waitUntilNumberOfCommitedFramesForIviSurface(WaylandIviSurfaceId iviSurface, uint64_t numberOfComittedBuffers, uint32_t timeoutMilliseconds = std::numeric_limits<uint32_t>::max());
        ...
        void                            sendSetShellSurfaceTitleToTestApplication(TestApplicationShellSurfaceId shellSurfaceId, std::string_view title, uint32_t testAppIdx = 0u);
        ...
        std::string                     getTitleOfIviSurface(WaylandIviSurfaceId iviSurface);
