...
{
    TextScene::TextScene(ramses::Scene& scene, uint32_t state, const glm::vec3& cameraPosition, uint32_t vpWidth, uint32_t vpHeight)
        : TextScene_Base(scene, cameraPosition, vpWidth, vpHeight)
    {
        ...
        std::optional<ramses::UniformInput> colorInput = textEffect->findUniformInput("u_color");
    ...
    void TextScene::setState(uint32_t state)
    {
        switch (state)
        {
        case EState_INITIAL:
