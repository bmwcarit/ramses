...
FUNCTION(ADD_TESTCOVERAGE_TARGETS name dependency)
    ...
    ACME_INFO(ACME_TESTCOVERAGE_TESTCOMMAND_${name}="${ACME_TESTCOVERAGE_TESTCOMMAND_${name}}")
    ADD_CUSTOM_COMMAND(OUTPUT RUNTESTS_${name}
        COMMAND ${ACME_TESTCOVERAGE_TESTCOMMAND_${name}}
        DEPENDS CreateInitialCoverage_${name}
        WORKING_DIRECTORY "${CMAKE_BINARY_DIR}"
        COMMENT "Codecoverage (${name}): Run tests to gather coverage information"
    )

    ACME_INFO(ACME_TESTCOVERAGE_EXCLUDE_${name}="${ACME_TESTCOVERAGE_EXCLUDE_${name}}")
    ADD_CUSTOM_COMMAND(OUTPUT CreateTestCoverage_Data_${name}
        ...
        COMMAND lcov
         --remove ${WORKDIR}/combined_${name}.info /usr/*.* ${ACME_TESTCOVERAGE_EXCLUDE_${name}}
...
FOREACH(CURRENT_NAME ${ACME_TESTCOVERAGE_NAMES})
   IF(ACME_TESTCOVERAGE_TESTCOMMAND_${CURRENT_NAME})
      IF(ACME_TESTCOVERAGE_EXCLUDE_${CURRENT_NAME})
      ...
      ELSE()
        ACME_ERROR("Must provide ACME_TESTCOVERAGE_EXCLUDE_${CURRENT_NAME}")
      ENDIF()
   ELSE()
       ACME_ERROR("Must provide ACME_TESTCOVERAGE_TESTCOMMAND_${CURRENT_NAME}")
  ENDIF()
ENDFOREACH()
