...
namespace ramses::internal
{
    ...
    protected:
        static void checkBaseContents(LogicEngine& logicEngine, ramses::Scene& ramsesScene)
        {
            ...
            const auto triLogicIntf = logicEngine.findByName<LuaInterface>("Interface_CameraCrane");
            ASSERT_TRUE(triLogicIntf);
            const auto triLogicScript = logicEngine.findByName<LuaScript>("CameraCrane");
            ASSERT_TRUE(triLogicScript);
            const auto triCamNode = logicEngine.findByName<RamsesNodeBinding>("triangleCamNodeBinding");
            ASSERT_TRUE(triCamNode);
            const auto triCamBinding = logicEngine.findByName<RamsesCameraBinding>("triangleCamBinding");
