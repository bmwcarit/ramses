...
namespace ramses::internal
{
    ...
    {
        {
            ...
            (void)PropertyImpl::Serialize(*rootImpl->m_impl, m_flatBufferBuilder, m_serializationMap);
        }
    ...
    TEST_F(AProperty, DoesNotSetLogicNodeToDirtyIfValueIsNotChanged)
    {
        ...
        int32Property->m_impl->setValue(42);
        int64Property->m_impl->setValue(int64_t{ 421 });
        floatProperty->m_impl->setValue(42.f);
        vec2fProperty->m_impl->setValue(vec2f{4.f, 2.f});
        vec3iProperty->m_impl->setValue(vec3i{4, 2, 3});
        stringProperty->m_impl->setValue(std::string("42"));

        int32Property->m_impl->getLogicNode().setDirty(false);
        ...
        stringProperty->m_impl->getLogicNode().setDirty(false);
        ...
        EXPECT_FALSE(int32Property->m_impl->getLogicNode().isDirty());
        EXPECT_FALSE(int64Property->m_impl->getLogicNode().isDirty());
        EXPECT_FALSE(floatProperty->m_impl->getLogicNode().isDirty());
        EXPECT_FALSE(vec2fProperty->m_impl->getLogicNode().isDirty());
        EXPECT_FALSE(vec3iProperty->m_impl->getLogicNode().isDirty());
        EXPECT_FALSE(stringProperty->m_impl->getLogicNode().isDirty());
    }
    ...
    TEST_F(AProperty, SetsLogicNodeToDirtyIfValueIsChanged)
    {
        ...
        EXPECT_TRUE(int32Property->m_impl->getLogicNode().isDirty());
        EXPECT_TRUE(int64Property->m_impl->getLogicNode().isDirty());
        EXPECT_TRUE(floatProperty->m_impl->getLogicNode().isDirty());
        EXPECT_TRUE(vec2fProperty->m_impl->getLogicNode().isDirty());
        EXPECT_TRUE(vec3iProperty->m_impl->getLogicNode().isDirty());
        EXPECT_TRUE(stringProperty->m_impl->getLogicNode().isDirty());
    }
    ...
    TEST_F(AProperty, FailsToSetINT64ValueThatCannotBeRepresentedInLua)
    {
        ...
        ScopedLogContextLevel logCollector{ ELogLevel::Error, [&](ELogLevel type, std::string_view message)
        {
            logType = type;
            logMessage = message;
        }
