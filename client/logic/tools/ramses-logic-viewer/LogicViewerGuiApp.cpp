...
namespace ramses
{
    ...
    int LogicViewerGuiApp::init(int argc, char const* const* argv)
    {
        ...
        auto exitCode = GetFeatureLevelFromFiles(args.sceneFile(), args.logicFile(), featureLevel);
        if (exitCode != 0)
        {
            return exitCode;
        }

        exitCode = LogicViewerApp::loadScene(args, featureLevel);
        if (exitCode != 0)
        {
            return exitCode;
        }
        ...
        if (m_headless)
        {
            exitCode = LogicViewerApp::createViewer(args, featureLevel, LogicViewer::ScreenshotFunc());
        }
        else
        {
            m_sceneSetup->apply();
            exitCode = LogicViewerApp::createViewer(args, featureLevel, takeScreenshot);
        }
