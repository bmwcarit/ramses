...
namespace ramses
{
    ...
    int LogicViewerApp::GetFeatureLevelFromFiles(const std::string& sceneFilename, const std::string& logicFilename, EFeatureLevel& featureLevel)
    {
        EFeatureLevel tmpLevel{EFeatureLevel_01};
        if (!RamsesClient::GetFeatureLevelFromFile(sceneFilename, tmpLevel))
        {
        ...
        if (!LogicEngine::GetFeatureLevelFromFile(logicFilename, featureLevel))
        {
            std::cerr << "Could not parse feature level from logic file " << logicFilename << std::endl;
            return static_cast<int>(ExitCode::ErrorLoadLogic);
        }
        if (tmpLevel != featureLevel)
        {
            std::cerr << "Feature levels of the scene file and the logic file do not match!" << std::endl;
            return static_cast<int>(ExitCode::ErrorLoadScene);
        }
        return 0;
    }
    ...
    int LogicViewerApp::createViewer(const Arguments& args, EFeatureLevel featureLevel, LogicViewer::ScreenshotFunc&& fScreenshot)
    {
        ...
        m_viewer = std::make_unique<ramses::LogicViewer>(featureLevel, fScreenshot);
