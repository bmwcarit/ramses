...
namespace ramses_internal
{
    ...
    TEST_F(ASceneCommandBuffer, canUseAllCommands)
    {
        ...
        {
            SceneCommandValidationRequest cmd{ramses::EValidationSeverity::Error, "bar"};
            buffer.enqueueCommand(cmd);
    ...
    TEST_F(ASceneCommandBuffer, canBeUsedFromMultipleThreads)
    {
        ...
        std::thread t1([&]() {
                           ...
                           buffer.enqueueCommand(SceneCommandValidationRequest{ramses::EValidationSeverity::Error, "bar"});
        ...
        std::thread t3([&]() {
                           Sequence seqT1;
                           Sequence seqT2;
                           EXPECT_CALL(visitor, handleSceneCommandValidationRequest(SceneCommandValidationRequest{ramses::EValidationSeverity::Error, "bar"})).InSequence(seqT1);
