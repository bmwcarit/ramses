...
namespace ramses
{
    ...
    TYPED_TEST(RamsesObjectTest, isOfTypeForAllDefinedBaseClasses)
    {
        ...
        while (type != ERamsesObjectType::Invalid)
        {
            ...
            type = RamsesObjectTraits[static_cast<int>(type)].baseClassTypeID;
        }
    }
    ...
    TYPED_TEST(RamsesObjectTest, getSetName)
    {
        ...
        {
            EXPECT_EQ(nullptr, this->m_scene.findObjectByName("object"));
            EXPECT_EQ(&obj, this->m_scene.findObjectByName("newName"));
        }
    }
    ...
    TYPED_TEST(RamsesObjectTest, validationStringContainsObjectTypeAndName)
    {
        ...
        std::ignore = obj.validate();
        const std::string validationReport = obj.getValidationReport(EValidationSeverity::Info);

        EXPECT_THAT(validationReport, ::testing::HasSubstr(RamsesObjectTypeUtils::GetRamsesObjectTypeName(obj.getType())));
        EXPECT_THAT(validationReport, ::testing::HasSubstr(obj.getName()));
    }
    ...
    TYPED_TEST(RamsesObjectTest, validationIgnoresWrongAPIUsage)
    {
        ...
        std::ignore = obj.validate();
        EXPECT_THAT(obj.getValidationReport(EValidationSeverity::Warning), ::testing::Not(::testing::HasSubstr("dummy error msg")));

        // will not appear in validation of scene either
        std::ignore = this->m_scene.validate();
        EXPECT_THAT(this->m_scene.getValidationReport(EValidationSeverity::Warning), ::testing::Not(::testing::HasSubstr("dummy error msg")));
    }
