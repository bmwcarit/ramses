...
namespace ramses
{
    ...
    TEST(ASceneConfig, CanBeCopyAndMoveConstructed)
    {
        ...
        EXPECT_EQ(EScenePublicationMode::LocalAndRemote, configCopy.m_impl.get().getPublicationMode());

        SceneConfig configMove{ std::move(config) };
        EXPECT_EQ(EScenePublicationMode::LocalAndRemote, configMove.m_impl.get().getPublicationMode());
    }
...
#endif
        config = config;
        EXPECT_EQ(EScenePublicationMode::LocalAndRemote, config.m_impl.get().getPublicationMode());
...
#pragma clang diagnostic pop
    ...
    TEST(DistributedSceneTest, givesErrorWhenRemotePublishingAfterEnablingLocalOnlyOptmisations)
    {
        ...
        EXPECT_NE(StatusOK, distributedScene->publish(EScenePublicationMode::LocalAndRemote));
    }
    ...
    TEST(DistributedSceneTest, canPublishRemotelyIfSetInConfig)
    {
        ...
        config.setPublicationMode(EScenePublicationMode::LocalAndRemote);
        Scene* distributedScene = remoteClient.createScene(sceneId_t(1u), config);
        EXPECT_EQ(StatusOK, distributedScene->publish(EScenePublicationMode::LocalAndRemote));
    }
    ...
    TEST(DistributedSceneTest, canPublishLocallyWhenEnablingLocalOnlyOptimisations)
    {
        ...
        config.setPublicationMode(EScenePublicationMode::LocalOnly);
        Scene* distributedScene = remoteClient.createScene(sceneId_t(1u), config);
        EXPECT_EQ(StatusOK, distributedScene->publish(EScenePublicationMode::LocalOnly));
    }
    ...
    TEST_F(AScene, removesMeshNodeFromRenderGroupsOnDestruction)
    {
        ...
        EXPECT_TRUE(group->m_impl.getAllMeshes().empty());
        EXPECT_TRUE(group2->m_impl.getAllMeshes().empty());
    }
    ...
    TEST_F(AScene, failsToCreateTextureSamplerWhenTextureIsFromAnotherScene)
    {
        Scene& anotherScene(*client.createScene(sceneId_t{ 0xf00 }));
        {
            ...
            TextureSampler* textureSampler = m_scene.createTextureSampler(ETextureAddressMode::Clamp, ETextureAddressMode::Clamp, ETextureSamplingMethod::Nearest, ETextureSamplingMethod::Linear, *texture);
        ...
        }
        {
            ...
            Texture3D* texture = anotherScene.createTexture3D(ETextureFormat::RGBA8, 2, 1, 2, 1, &mipLevelData, false, ramses::ResourceCacheFlag_DoNotCache, {});
            ASSERT_TRUE(nullptr != texture);

            TextureSampler* textureSampler = m_scene.createTextureSampler(ETextureAddressMode::Clamp, ETextureAddressMode::Clamp, ETextureAddressMode::Clamp, ETextureSamplingMethod::Nearest, ETextureSamplingMethod::Linear, *texture);
            EXPECT_TRUE(nullptr == textureSampler);
        }
        {
        ...
        {
            RenderBuffer* renderBuffer = anotherScene.createRenderBuffer(4u, 4u, ERenderBufferType::Color, ERenderBufferFormat::RGB8, ERenderBufferAccessMode::ReadWrite, 4u);
    ...
    TEST_F(AScene, failsToCreateTextureSamplerWhenRenderTargetIsFromAnotherScene)
    {
        ...
        RenderBuffer* renderBuffer = anotherScene.createRenderBuffer(100u, 100u, ERenderBufferType::Color, ERenderBufferFormat::RGBA8, ERenderBufferAccessMode::ReadWrite);
        ASSERT_TRUE(nullptr != renderBuffer);

        TextureSampler* textureSampler = m_scene.createTextureSampler(ETextureAddressMode::Clamp, ETextureAddressMode::Clamp, ETextureSamplingMethod::Nearest, ETextureSamplingMethod::Linear, *renderBuffer);
        EXPECT_TRUE(nullptr == textureSampler);
    ...
    {
        RenderBuffer* renderBuffer = m_scene.createRenderBuffer(100u, 100u, ERenderBufferType::Color, ERenderBufferFormat::RGBA8, ERenderBufferAccessMode::WriteOnly);
        ASSERT_TRUE(nullptr != renderBuffer);
    ...
    TEST_F(ASceneWithContent, checksMeshVisibilityChangedByParentVisibilityNode)
    {
        ...
        EXPECT_EQ(m_mesh1a.m_impl.getFlattenedVisibility(), EVisibilityMode::Invisible);
        EXPECT_EQ(m_mesh1b.m_impl.getFlattenedVisibility(), EVisibilityMode::Invisible);
        ...
        EXPECT_EQ(m_mesh1a.m_impl.getFlattenedVisibility(), EVisibilityMode::Invisible);
        EXPECT_EQ(m_mesh1b.m_impl.getFlattenedVisibility(), EVisibilityMode::Invisible);
        EXPECT_EQ(m_mesh2a.m_impl.getFlattenedVisibility(), EVisibilityMode::Invisible);
        EXPECT_EQ(m_mesh2b.m_impl.getFlattenedVisibility(), EVisibilityMode::Invisible);

        m_vis1.setVisibility(EVisibilityMode::Off);
        m_vis2.setVisibility(EVisibilityMode::Visible);
        m_scene.flush();

        EXPECT_EQ(m_mesh1a.m_impl.getFlattenedVisibility(), EVisibilityMode::Off);
        EXPECT_EQ(m_mesh1b.m_impl.getFlattenedVisibility(), EVisibilityMode::Off);
        EXPECT_EQ(m_mesh2a.m_impl.getFlattenedVisibility(), EVisibilityMode::Visible);
        EXPECT_EQ(m_mesh2b.m_impl.getFlattenedVisibility(), EVisibilityMode::Visible);
    }
    ...
    TEST_F(ASceneWithContent, checksMeshVisibilityAddedToInvisibleParent)
    {
        ...
        m_scene.flush();
        EXPECT_EQ(addMeshNode->m_impl.getFlattenedVisibility(), EVisibilityMode::Invisible);
    ...
    TEST_F(ASceneWithContent, checksMeshVisibilityAddedToOffParent)
    {
        ...
        m_scene.flush();
        EXPECT_EQ(addMeshNode->m_impl.getFlattenedVisibility(), EVisibilityMode::Off);
    ...
    TEST_F(ASceneWithContent, checksMeshVisibilityChangedByGrandParentVisibilityNode)
    {
        ...
        EXPECT_EQ(m_mesh2a.m_impl.getFlattenedVisibility(), EVisibilityMode::Visible);
        EXPECT_EQ(m_mesh2b.m_impl.getFlattenedVisibility(), EVisibilityMode::Visible);
        EXPECT_EQ(m_node9.m_impl.getFlattenedVisibility(), EVisibilityMode::Invisible);
    ...
    TEST_F(ASceneWithContent, visibleMeshBecomesOffWhenMovedUnderOffNode)
    {
        ...
        m_mesh2a.setParent(m_vis1);
        m_scene.flush();
        EXPECT_EQ(m_mesh2a.m_impl.getFlattenedVisibility(), EVisibilityMode::Off);
    }
    ...
    TEST_F(ASceneWithContent, visibleMeshWithoutParentBecomesInvisibleWhenSetInvisibleNodeAsParent)
    {
        ...
        EXPECT_EQ(meshOrphan.m_impl.getFlattenedVisibility(), EVisibilityMode::Invisible);
    }
    ...
    TEST_F(ASceneWithContent, visibleMeshWithoutParentBecomesOffWhenSetOffNodeAsParent)
    {
        ...
        EXPECT_EQ(meshOrphan.m_impl.getFlattenedVisibility(), EVisibilityMode::Off);
    }
    ...
    TEST_F(AScene, canCreateATransformDataSlot)
    {
        ...
        EXPECT_EQ(0u, m_scene.m_impl.getIScene().getDataSlotCount());

        EXPECT_EQ(StatusOK, m_scene.createTransformationDataConsumer(*node, dataConsumerId_t(666u)));

        EXPECT_EQ(1u, m_scene.m_impl.getIScene().getDataSlotCount());
        ramses_internal::DataSlotHandle slotHandle(0u);
        ASSERT_TRUE(m_scene.m_impl.getIScene().isDataSlotAllocated(slotHandle));
        EXPECT_EQ(node->m_impl.getNodeHandle(), m_scene.m_impl.getIScene().getDataSlot(slotHandle).attachedNode);
        EXPECT_EQ(ramses_internal::DataSlotId(666u), m_scene.m_impl.getIScene().getDataSlot(slotHandle).id);
        EXPECT_EQ(ramses_internal::EDataSlotType_TransformationConsumer, m_scene.m_impl.getIScene().getDataSlot(slotHandle).type);
    }
    ...
    TEST_F(AScene, removesTransformDataSlotsOfNodeOnDestruction)
    {
        ...
        ASSERT_TRUE(m_scene.m_impl.getIScene().isDataSlotAllocated(linkHandle));

        m_scene.destroy(*node);

        EXPECT_FALSE(m_scene.m_impl.getIScene().isDataSlotAllocated(linkHandle));
    }
    ...
    TEST_F(AScene, canCreateADataObjectDataSlot)
    {
        ...
        ramses_internal::DataSlotHandle slotHandle(0u);
        ASSERT_TRUE(m_scene.m_impl.getIScene().isDataSlotAllocated(slotHandle));
        EXPECT_EQ(dataObject->m_impl.getDataReference(), m_scene.m_impl.getIScene().getDataSlot(slotHandle).attachedDataReference);
        EXPECT_EQ(ramses_internal::DataSlotId(666u), m_scene.m_impl.getIScene().getDataSlot(slotHandle).id);
        EXPECT_EQ(ramses_internal::EDataSlotType_DataConsumer, m_scene.m_impl.getIScene().getDataSlot(slotHandle).type);
    }
    ...
    TEST_F(AScene, reportsErrorWhenCreateTextureConsumerWithSamplerFromAnotherScene)
    {
        ...
        const TextureSampler* sampler = anotherScene.createTextureSampler(ETextureAddressMode::Clamp, ETextureAddressMode::Clamp, ETextureSamplingMethod::Nearest, ETextureSamplingMethod::Linear, *texture);
    ...
    TEST_F(AScene, canCreateATextureProviderDataSlot)
    {
        ...
        ramses_internal::DataSlotHandle slotHandle(0u);
        ASSERT_TRUE(m_scene.m_impl.getIScene().isDataSlotAllocated(slotHandle));
        EXPECT_EQ(texture->m_impl.getLowlevelResourceHash(), m_scene.m_impl.getIScene().getDataSlot(slotHandle).attachedTexture);
        EXPECT_EQ(ramses_internal::DataSlotId(666u), m_scene.m_impl.getIScene().getDataSlot(slotHandle).id);
        EXPECT_EQ(ramses_internal::EDataSlotType_TextureProvider, m_scene.m_impl.getIScene().getDataSlot(slotHandle).type);
    }
    ...
    TEST_F(AScene, canUpdateATextureProviderDataSlot)
    {
        ...
        ramses_internal::DataSlotHandle slotHandle(0u);
        ASSERT_TRUE(m_scene.m_impl.getIScene().isDataSlotAllocated(slotHandle));
        EXPECT_EQ(texture2->m_impl.getLowlevelResourceHash(), m_scene.m_impl.getIScene().getDataSlot(slotHandle).attachedTexture);
        EXPECT_EQ(ramses_internal::DataSlotId(666u), m_scene.m_impl.getIScene().getDataSlot(slotHandle).id);
        EXPECT_EQ(ramses_internal::EDataSlotType_TextureProvider, m_scene.m_impl.getIScene().getDataSlot(slotHandle).type);
    }
    ...
    TEST_F(AScene, canCreateATextureConsumerDataSlot)
    {
        ...
        const TextureSampler* sampler = m_scene.createTextureSampler(ETextureAddressMode::Clamp, ETextureAddressMode::Clamp, ETextureSamplingMethod::Nearest, ETextureSamplingMethod::Linear, *texture);
        ASSERT_TRUE(nullptr != sampler);
        ...
        ramses_internal::DataSlotHandle slotHandle(0u);
        ASSERT_TRUE(m_scene.m_impl.getIScene().isDataSlotAllocated(slotHandle));
        EXPECT_EQ(sampler->m_impl.getTextureSamplerHandle(), m_scene.m_impl.getIScene().getDataSlot(slotHandle).attachedTextureSampler);
        EXPECT_EQ(ramses_internal::DataSlotId(666u), m_scene.m_impl.getIScene().getDataSlot(slotHandle).id);
        EXPECT_EQ(ramses_internal::EDataSlotType_TextureConsumer, m_scene.m_impl.getIScene().getDataSlot(slotHandle).type);
    }
    ...
    TEST_F(AScene, canNotCreateATextureConsumerDataSlotIfOtherThan2DTextureAssigned)
    {
        ...
        const TextureSampler* sampler = m_scene.createTextureSampler(ETextureAddressMode::Clamp, ETextureAddressMode::Clamp, ETextureAddressMode::Clamp, ETextureSamplingMethod::Nearest, ETextureSamplingMethod::Linear, *texture);
        ASSERT_TRUE(nullptr != sampler);
    ...
    TEST_F(AScene, removesDataSlotsOfTextureSamplerOnDestruction)
    {
        ...
        TextureSampler* sampler = m_scene.createTextureSampler(ETextureAddressMode::Clamp, ETextureAddressMode::Clamp, ETextureSamplingMethod::Nearest, ETextureSamplingMethod::Linear, *texture);
        ASSERT_TRUE(nullptr != sampler);
    ...
    TEST_F(AScene, canNotCreateMoreThanOneTextureConsumerOrProviderWithTheSameId)
    {
        ...
        TextureSampler* sampler = m_scene.createTextureSampler(ETextureAddressMode::Clamp, ETextureAddressMode::Clamp, ETextureSamplingMethod::Nearest, ETextureSamplingMethod::Linear, *texture);
        ASSERT_TRUE(nullptr != sampler);
        TextureSampler* sampler2 = m_scene.createTextureSampler(ETextureAddressMode::Clamp, ETextureAddressMode::Clamp, ETextureSamplingMethod::Nearest, ETextureSamplingMethod::Linear, *texture2);
    ...
    TEST_F(AScene, canCreateTextureSamplerForTexture2DWithDefaultAnisotropyLevel)
    {
        ...
        const TextureSampler* sampler = this->m_scene.createTextureSampler(ETextureAddressMode::Clamp, ETextureAddressMode::Clamp, ETextureSamplingMethod::Nearest, ETextureSamplingMethod::Linear, texture2D);
    ...
    TEST_F(AScene, canCreateTextureSamplerForRenderBufferWithDefaultAnisotropyLevel)
    {
        ...
        const TextureSampler* sampler = this->m_scene.createTextureSampler(ETextureAddressMode::Clamp, ETextureAddressMode::Clamp, ETextureSamplingMethod::Nearest, ETextureSamplingMethod::Linear, renderBuffer);
    ...
    TEST_F(AScene, canCreateTextureSamplerForTextureCubeWithDefaultAnisotropyLevel)
    {
        ...
        const TextureSampler* sampler = this->m_scene.createTextureSampler(ETextureAddressMode::Clamp, ETextureAddressMode::Clamp, ETextureSamplingMethod::Linear, ETextureSamplingMethod::Linear, textureCube);
    ...
    TEST_F(AScene, cantCreateTextureSamplerForTexture2DWithWrongAnisotropyValue)
    {
        ...
        TextureSampler* textureSampler = m_scene.createTextureSampler(ETextureAddressMode::Clamp, ETextureAddressMode::Clamp, ETextureSamplingMethod::Nearest, ETextureSamplingMethod::Linear, texture, 0);
        EXPECT_TRUE(nullptr == textureSampler);
    }
    ...
    {
        const TextureCube& textureCube = createObject<TextureCube>("testTextureCube");
        const TextureSampler* textureSampler = m_scene.createTextureSampler(ETextureAddressMode::Clamp, ETextureAddressMode::Clamp, ETextureSamplingMethod::Nearest, ETextureSamplingMethod::Linear, textureCube, 0);
        EXPECT_TRUE(nullptr == textureSampler);
    }
    ...
    {
        const RenderBuffer& renderBuffer = createObject<RenderBuffer>("renderBuffer");
        const TextureSampler* textureSampler = m_scene.createTextureSampler(ETextureAddressMode::Clamp, ETextureAddressMode::Clamp, ETextureSamplingMethod::Nearest, ETextureSamplingMethod::Linear, renderBuffer, 0);
        EXPECT_TRUE(nullptr == textureSampler);
    }
    ...
    {
        Scene& anotherScene = *client.createScene(sceneId_t(12u));
        const RenderBuffer* sourceRenderBuffer = anotherScene.createRenderBuffer(100u, 100u, ERenderBufferType::Color, ERenderBufferFormat::RGBA8, ERenderBufferAccessMode::ReadWrite);
    ...
    {
        Scene& anotherScene = *client.createScene(sceneId_t(12u));
        const RenderBuffer* destinationRenderBuffer = anotherScene.createRenderBuffer(100u, 100u, ERenderBufferType::Color, ERenderBufferFormat::RGBA8, ERenderBufferAccessMode::ReadWrite);
    ...
    {
        const RenderBuffer* sourceRenderBuffer = m_scene.createRenderBuffer(100u, 100u, ERenderBufferType::Color, ERenderBufferFormat::R8, ERenderBufferAccessMode::ReadWrite);
        const RenderBuffer* destinationRenderBuffer = m_scene.createRenderBuffer(100u, 100u, ERenderBufferType::Depth, ERenderBufferFormat::Depth24, ERenderBufferAccessMode::ReadWrite);
        ASSERT_TRUE(nullptr != sourceRenderBuffer);
        ASSERT_TRUE(nullptr != destinationRenderBuffer);
    ...
    {
        const RenderBuffer* sourceRenderBuffer = m_scene.createRenderBuffer(100u, 100u, ERenderBufferType::Color, ERenderBufferFormat::R8, ERenderBufferAccessMode::ReadWrite);
        const RenderBuffer* destinationRenderBuffer = m_scene.createRenderBuffer(100u, 100u, ERenderBufferType::Color, ERenderBufferFormat::RGBA8, ERenderBufferAccessMode::ReadWrite);
        ASSERT_TRUE(nullptr != sourceRenderBuffer);
        ASSERT_TRUE(nullptr != destinationRenderBuffer);
    ...
    {
        const RenderBuffer* sourceRenderBuffer = m_scene.createRenderBuffer(1u, 100u, ERenderBufferType::Color, ERenderBufferFormat::RGBA8, ERenderBufferAccessMode::ReadWrite);
        const RenderBuffer* destinationRenderBuffer = m_scene.createRenderBuffer(100u, 100u, ERenderBufferType::Color, ERenderBufferFormat::RGBA8, ERenderBufferAccessMode::ReadWrite);
        ASSERT_TRUE(nullptr != sourceRenderBuffer);
        ASSERT_TRUE(nullptr != destinationRenderBuffer);
    ...
    {
        const RenderBuffer* sourceRenderBuffer = m_scene.createRenderBuffer(100u, 1u, ERenderBufferType::Color, ERenderBufferFormat::RGBA8, ERenderBufferAccessMode::ReadWrite);
        const RenderBuffer* destinationRenderBuffer = m_scene.createRenderBuffer(100u, 100u, ERenderBufferType::Color, ERenderBufferFormat::RGBA8, ERenderBufferAccessMode::ReadWrite);
        ASSERT_TRUE(nullptr != sourceRenderBuffer);
        ASSERT_TRUE(nullptr != destinationRenderBuffer);
    ...
    {
        const RenderBuffer* rb = m_scene.createRenderBuffer(100u, 1u, ERenderBufferType::Color, ERenderBufferFormat::RGBA8, ERenderBufferAccessMode::ReadWrite);
    ...
    {
        EXPECT_EQ(0u, m_scene.m_impl.getStatisticCollection().statFlushesTriggered.getCounterValue());
        m_scene.flush();
        EXPECT_EQ(1u, m_scene.m_impl.getStatisticCollection().statFlushesTriggered.getCounterValue());
    }
    ...
    {
        EXPECT_EQ(ramses_internal::FlushTime::InvalidTimestamp, m_scene.m_impl.getIScene().getEffectTimeSync());
        EXPECT_EQ(StatusOK, m_scene.resetUniformTimeMs());
        const auto timeSync = m_scene.m_impl.getIScene().getEffectTimeSync();
        ...
        EXPECT_EQ(StatusOK, m_scene.publish(ramses::EScenePublicationMode::LocalOnly));
