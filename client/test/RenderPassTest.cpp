...
namespace ramses
{
    ...
    protected:
        ...
        {
            const RenderBuffer* renderBuffer = m_scene.createRenderBuffer(16u, 12u, ERenderBufferType::Color, ERenderBufferFormat::RGBA8, ERenderBufferAccessMode::ReadWrite);
    ...
    TEST_F(ARenderPass, reportsErrorWhenSetRenderTargetFromAnotherScene)
    {
        ...
        const RenderBuffer* renderBuffer = anotherScene.createRenderBuffer(16u, 12u, ERenderBufferType::Color, ERenderBufferFormat::RGBA8, ERenderBufferAccessMode::ReadWrite);
        RenderTargetDescription rtDesc;
        rtDesc.addRenderBuffer(*renderBuffer);
    ...
    {
        EXPECT_EQ(static_cast<uint32_t>(ramses::EClearFlags::EClearFlags_All), renderpass.getClearFlags());

        EXPECT_EQ(StatusOK, renderpass.setClearFlags(ramses::EClearFlags::EClearFlags_None));
        EXPECT_EQ(static_cast<uint32_t>(ramses::EClearFlags::EClearFlags_None), renderpass.getClearFlags());

        EXPECT_EQ(StatusOK, renderpass.setClearFlags(ramses::EClearFlags::EClearFlags_Depth));
        EXPECT_EQ(static_cast<uint32_t>(ramses::EClearFlags::EClearFlags_Depth), renderpass.getClearFlags());
