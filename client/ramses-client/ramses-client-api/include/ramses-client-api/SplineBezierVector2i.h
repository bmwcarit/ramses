...
#ifndef RAMSES_SPLINEBEZIERVECTOR2I_H
...
{
    /**
    * @brief The SplineBezierVector2i stores spline keys of type Vector2i that can be used for animation with Bezier interpolation.
    */
    class RAMSES_API SplineBezierVector2i : public Spline
    {
    public:
        /**
        ...
        */
        status_t setKey(splineTimeStamp_t timeStamp, int32_t x, int32_t y, float tangentIn_x, float tangentIn_y, float tangentOut_x, float tangentOut_y);
        ...
        *         to resolve error message using getStatusMessage().
        */
        status_t getKeyValues(splineKeyIndex_t keyIndex, splineTimeStamp_t& timeStamp, int32_t& x, int32_t& y, float& tangentIn_x, float& tangentIn_y, float& tangentOut_x, float& tangentOut_y) const;

    protected:
        /**
        * @brief AnimationSystemData is the factory for creating SplineBezierVector2i instances.
        */
        friend class AnimationSystemData;

        /**
        * @brief Constructor of SplineBezierVector2i
        *
        * @param[in] pimpl Internal data for implementation specifics of Spline (sink - instance becomes owner)
        */
        explicit SplineBezierVector2i(SplineImpl& pimpl);

        /**
        * @brief Destructor of the SplineBezierVector2i
        */
        virtual ~SplineBezierVector2i();
    };
}
