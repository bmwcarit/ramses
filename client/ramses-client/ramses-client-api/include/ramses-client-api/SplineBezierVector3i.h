...
#ifndef RAMSES_SPLINEBEZIERVECTOR3I_H
#define RAMSES_SPLINEBEZIERVECTOR3I_H
...
{
    /**
    * @brief The SplineBezierVector3i stores spline keys of type Vector3i that can be used for animation with Bezier interpolation.
    */
    class RAMSES_API SplineBezierVector3i : public Spline
    {
    public:
        /**
        ...
        */
        status_t setKey(splineTimeStamp_t timeStamp, int32_t x, int32_t y, int32_t z, float tangentIn_x, float tangentIn_y, float tangentOut_x, float tangentOut_y);
        ...
        *         to resolve error message using getStatusMessage().
        */
        status_t getKeyValues(splineKeyIndex_t keyIndex, splineTimeStamp_t& timeStamp, int32_t& x, int32_t& y, int32_t& z, float& tangentIn_x, float& tangentIn_y, float& tangentOut_x, float& tangentOut_y) const;

    protected:
        /**
        * @brief AnimationSystemData is the factory for creating SplineBezierVector3i instances.
        */
        friend class AnimationSystemData;

        /**
        * @brief Constructor of SplineBezierVector3i
        *
        * @param[in] pimpl Internal data for implementation specifics of Spline (sink - instance becomes owner)
        */
        explicit SplineBezierVector3i(SplineImpl& pimpl);

        /**
        * @brief Destructor of the SplineBezierVector3i
        */
        virtual ~SplineBezierVector3i();
    };
}
