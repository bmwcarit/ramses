...
#ifndef RAMSES_ARRAYRESOURCEUTILS_H
#define RAMSES_ARRAYRESOURCEUTILS_H
...
namespace ramses
{
    class ArrayResourceUtils
    {
    public:
        static EDataType ConvertDataTypeForResourceFromInternal(ramses_internal::EDataType type)
        {
            switch (type)
            {
            ...
            default:
                assert(!"Resource is not allowed to have that type");
                return EDataType::UInt16;
            }
        }

        static ramses_internal::EDataType ConvertDataTypeForResourceToInternal(EDataType type)
        {
            switch (type)
            {
