...
namespace ramses
{
    ...
    Texture2DBuffer* SceneImpl::createTexture2DBuffer(size_t mipLevels, uint32_t width, uint32_t height, ETextureFormat textureFormat, std::string_view name)
    {
        ...
        const size_t maxMipCount = ramses_internal::TextureMathUtils::GetMipLevelCount(width, height, 1u);
    ...
    Texture2D* SceneImpl::createTexture2D(uint32_t width, uint32_t height, ETextureFormat format, size_t mipMapCount, const MipLevelData mipLevelData[], bool generateMipChain, const TextureSwizzle& swizzle, resourceCacheFlag_t cacheFlag, std::string_view name)
    {
        ramses_internal::ManagedResource res = getClientImpl().createManagedTexture(
            ramses_internal::EResourceType_Texture2D, width, height, 1u, format, static_cast<uint32_t>(mipMapCount), mipLevelData, generateMipChain, swizzle, cacheFlag, name);
        if (!res)
        {
    ...
    // NOLINTNEXTLINE(modernize-avoid-c-arrays)
    Texture3D* SceneImpl::createTexture3D(uint32_t width, uint32_t height, uint32_t depth, ETextureFormat format, size_t mipMapCount, const MipLevelData mipLevelData[], bool generateMipChain, resourceCacheFlag_t cacheFlag, std::string_view name)
    {
        ramses_internal::ManagedResource res = getClientImpl().createManagedTexture(
            ramses_internal::EResourceType_Texture3D, width, height, depth, format, static_cast<uint32_t>(mipMapCount), mipLevelData, generateMipChain, {}, cacheFlag, name);
        if (!res)
        {
    ...
    // NOLINTNEXTLINE(modernize-avoid-c-arrays)
    TextureCube* SceneImpl::createTextureCube(uint32_t size, ETextureFormat format, size_t mipMapCount, const CubeMipLevelData mipLevelData[], bool generateMipChain, const TextureSwizzle& swizzle, resourceCacheFlag_t cacheFlag, std::string_view name)
    {
        ramses_internal::ManagedResource res = getClientImpl().createManagedTexture(
            ramses_internal::EResourceType_TextureCube, size, 1u, 1u, format, static_cast<uint32_t>(mipMapCount), mipLevelData, generateMipChain, swizzle, cacheFlag, name);
        if (!res)
        {
    ...
    status_t SceneImpl::saveToFile(std::string_view fileName, bool compress) const
    {
        ...
        size_t offsetLLResourcesStart = 0;
