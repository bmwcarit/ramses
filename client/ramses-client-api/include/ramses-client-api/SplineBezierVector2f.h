...
#ifndef RAMSES_SPLINEBEZIERVECTOR2F_H
#define RAMSES_SPLINEBEZIERVECTOR2F_H
...
{
    /**
    * @brief The SplineBezierVector2f stores spline keys of type Vector2f that can be used for animation with Bezier interpolation.
    */
    class RAMSES_API SplineBezierVector2f : public Spline
    {
    public:
        /**
        ...
        */
        status_t setKey(splineTimeStamp_t timeStamp, float x, float y, float tangentIn_x, float tangentIn_y, float tangentOut_x, float tangentOut_y);
        ...
        *         to resolve error message using getStatusMessage().
        */
        status_t getKeyValues(splineKeyIndex_t keyIndex, splineTimeStamp_t& timeStamp, float& x, float& y, float& tangentIn_x, float& tangentIn_y, float& tangentOut_x, float& tangentOut_y) const;

    protected:
        /**
        * @brief AnimationSystemData is the factory for creating SplineBezierVector2f instances.
        */
        friend class AnimationSystemData;

        /**
        * @brief Constructor of SplineBezierVector2f
        *
        * @param[in] pimpl Internal data for implementation specifics of Spline (sink - instance becomes owner)
        */
        explicit SplineBezierVector2f(SplineImpl& pimpl);

        /**
        * @brief Destructor of the SplineBezierVector2f
        */
        virtual ~SplineBezierVector2f();
    };
}
