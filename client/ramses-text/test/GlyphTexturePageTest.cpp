...
namespace ramses
{
    ...
    protected:
        ...
        {
            SceneObjectIterator bufferIter(m_scene, ERamsesObjectType_Texture2DBuffer);
            SceneObjectIterator samplerIter(m_scene, ERamsesObjectType_TextureSampler);
            EXPECT_EQ(!bufferIter.getNext(), !expect);
            EXPECT_EQ(!samplerIter.getNext(), !expect);
        }
    };

    constexpr uint32_t AGlyphTexturePage::PageWidth;
    constexpr uint32_t AGlyphTexturePage::PageHeight;

    AGlyphTexturePage::AGlyphTexturePage()
        : m_client("text test", m_framework)
