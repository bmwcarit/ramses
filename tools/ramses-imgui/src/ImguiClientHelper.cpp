...
namespace ramses::internal
{
    namespace
    {
        ...
        uint32_t GetCharCode(ramses::KeyModifiers keyModifiers, ramses::EKeyCode keyCode)
        {
            const bool isShift = keyModifiers.isSet(ramses::EKeyModifier::Shift);
            uint32_t   retval  = 0U;
    ...
    {
        ramses::SceneConfig local(sceneid);
        m_imguiscene = client.createScene(local, "imgui scene");
        ...
        textureInput = effect->findUniformInput("textureSampler");
    ...
    void ImguiClientHelper::draw()
    {
        ...
        for (int n = 0; n < draw_data->CmdListsCount; n++)
        {
            ...
            auto geobinding = m_imguiscene->createGeometry(*effect);
    ...
    void ImguiClientHelper::keyEvent(ramses::displayId_t displayId, ramses::EKeyEvent eventType, ramses::KeyModifiers keyModifiers, ramses::EKeyCode keyCode)
    {
        if (!m_displayId.isValid() || displayId == m_displayId)
        {
            ImGuiIO&   io        = ImGui::GetIO();
