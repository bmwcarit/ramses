...
namespace ramses::internal
{
    ...
    namespace imgui
    {
        namespace
        {
            ...
            void ApplySwizzle(std::array<uint8_t, 4>& rgba, const TextureSwizzleArray& swizzle)
            {
            ...
            template <class T> void ConvertToRgba(std::vector<uint8_t>& rgbaData, const uint8_t* data, const size_t size, const TextureSwizzleArray& swizzle, const T& readPixel)
            {
            ...
            const char* descriptionText, std::string* inputString, uint64_t& value, std::string& currentComboBoxString, absl::Span<const ComboBoxEntry> comboboxItems)
        {
            ...
            {
                if (value == item.contentId)
                {
                    *inputString          = std::to_string(item.contentId);
                    currentComboBoxString = item.preview;
                    break;
                }
            }
            ...
            if (ImGui::BeginCombo(fmt::format("##custom combo{}", descriptionText).c_str(), currentComboBoxString.c_str()))
            {
                for (const auto& item : comboboxItems)
                {
                    bool is_selected = (currentComboBoxString == item.preview);
                    if (ImGui::Selectable(item.preview.c_str(), is_selected))
                    {
                        currentComboBoxString = item.preview;
                        *inputString          = std::to_string(item.contentId);
                        value                 = std::stoi(*inputString);
        ...
        std::string SaveToPng(const uint8_t* data, size_t size, EPixelStorageFormat fmt, uint32_t width, uint32_t height, const std::string& filename, const TextureSwizzleArray& swizzle)
        {
            std::string errorMsg;
            std::vector<uint8_t> imageData;
            const auto byteSize = width * height * 4u;
            imageData.reserve(byteSize);
            if (fmt == EPixelStorageFormat::RGBA8)
            {
                // just copy the blob
            ...
            else
            {
                ...
                case EPixelStorageFormat::R8:
                    ConvertToRgba(imageData, data, size, swizzle, [](const auto* ptr, std::array<uint8_t, 4>& rgba) {
                        rgba[0] = *ptr++;
                        return ptr;
                    });
                    break;
            ...
            if (errorMsg.empty() && !RamsesUtils::SaveImageBufferToPng(filename, imageData, width, height))
            {
        ...
        std::string SaveTextureToPng(const TextureResource* resource, const std::string& filename)
        {
            ...
            if (resource)
            {
                ...
                const auto* textureResource = resource->convertTo<TextureResource>();
                const auto& blob = textureResource->getResourceData();
                // NOLINTNEXTLINE(cppcoreguidelines-pro-type-reinterpret-cast)
                errorMsg = SaveToPng(reinterpret_cast<const uint8_t*>(blob.data()), blob.size(), resource->getTextureFormat(), resource->getWidth(), resource->getHeight(), filename, resource->getTextureSwizzle());
            }
            return errorMsg;
        }
    } // namespace imgui
