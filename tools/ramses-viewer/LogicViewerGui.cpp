...
namespace ramses::internal
{
    ...
    LogicViewerGui::LogicViewerGui(LogicViewer& viewer, ViewerSettings& settings, std::string luafile)
        : m_settings(settings)
        , m_viewer(viewer)
        , m_logicEngine(viewer.getLogic())
    ...
    void LogicViewerGui::draw()
    {
        ...
        if (m_settings.showWindow && m_settings.showLogicWindow)
        {
    ...
    void LogicViewerGui::setRendererInfo(ramses::RamsesRenderer& renderer, ramses::displayId_t displayId, ramses::displayBufferId_t displayBufferId, const glm::vec4& initialClearColor)
    {
        m_renderer = &renderer;
    ...
    {
        if (ImGui::MenuItem("Show Logic Viewer Window", nullptr, &m_settings.showLogicWindow))
        {
    ...
    void LogicViewerGui::drawErrorPopup()
    {
        if (!m_lastErrorMessage.empty())
            ImGui::OpenPopup("ErrorLogic");

        if (ImGui::BeginPopupModal("ErrorLogic", nullptr, ImGuiWindowFlags_AlwaysAutoResize))
        {
    ...
    {
        if (!ImGui::Begin(fmt::format("Logic Viewer (FeatureLevel 0{})", m_logicEngine.getScene().getRamsesClient().getRamsesFramework().getFeatureLevel()).c_str(), &m_settings.showLogicWindow, ImGuiWindowFlags_MenuBar))
        {
            ImGui::End();
            return;
        }
    ...
    void LogicViewerGui::drawNodeBindings()
    {
        ...
        if (ImGui::BeginPopupContextItem("NodeBindingsContextMenu"))
        {
            ...
            {
                copyInputs(LogicViewer::ltnNode, m_logicEngine.getCollection<NodeBinding>());
            }
            ImGui::EndPopup();
        }
        if (openBindings)
        {
            for (auto* obj : m_logicEngine.getCollection<NodeBinding>())
            {
                const bool open = DrawTreeNode(obj);
    ...
    void LogicViewerGui::drawCameraBindings()
    {
        ...
        if (ImGui::BeginPopupContextItem("CameraBindingsContextMenu"))
        {
            ...
            {
                copyInputs(LogicViewer::ltnCamera, m_logicEngine.getCollection<CameraBinding>());
            }
            ImGui::EndPopup();
        }
        if (openBindings)
        {
            for (auto* obj : m_logicEngine.getCollection<CameraBinding>())
            {
                const bool open = DrawTreeNode(obj);
    ...
    void LogicViewerGui::drawRenderPassBindings()
    {
        ...
        if (ImGui::BeginPopupContextItem("RenderPassBindingsContextMenu"))
        {
            ...
            {
                copyInputs(LogicViewer::ltnRenderPass, m_logicEngine.getCollection<RenderPassBinding>());
            }
            ImGui::EndPopup();
        }
        if (openBindings)
        {
            for (auto* obj : m_logicEngine.getCollection<RenderPassBinding>())
            {
                const bool open = DrawTreeNode(obj);
    ...
    void LogicViewerGui::drawRenderGroupBindings()
    {
        ...
        if (ImGui::BeginPopupContextItem("RenderGroupBindingsContextMenu"))
        {
            ...
            {
                copyInputs(LogicViewer::ltnRenderGroup, m_logicEngine.getCollection<RenderGroupBinding>());
            }
            ImGui::EndPopup();
        }
        if (openBindings)
        {
            for (auto* obj : m_logicEngine.getCollection<RenderGroupBinding>())
            {
                const bool open = DrawTreeNode(obj);
    ...
    void LogicViewerGui::drawMeshNodeBindings()
    {
        ...
        if (ImGui::BeginPopupContextItem("MeshNodeBindingsContextMenu"))
        {
            ...
            {
                copyInputs(LogicViewer::ltnMeshNode, m_logicEngine.getCollection<MeshNodeBinding>());
            }
            ImGui::EndPopup();
        }
        if (openBindings)
        {
            for (auto* obj : m_logicEngine.getCollection<MeshNodeBinding>())
            {
                const bool open = DrawTreeNode(obj);
        ...
        if (open)
        {
            ...
            if (TreeNode("Executed", fmt::format("Executed Nodes ({}):", executed.size())))
            {
                for (auto& timedNode : executed)
                {
                    ...
                    if (TreeNode(node, fmt::format("{}[{}]: {} [time:{} ms, {}%]", TypeName(node), node->getSceneObjectId().getValue(), node->getName(), timedNode.second, percentage)))
                    {
                        drawNode(node);
                        ImGui::TreePop();
                    }
            ...
            if (TreeNode("Skipped", fmt::format("Skipped Nodes ({}):", skipped.size())))
            {
                ...
                {
                    if (TreeNode(node, fmt::format("{}[{}]: {}", TypeName(node), node->getSceneObjectId().getValue(), node->getName())))
                    {
                        drawNode(node);
                        ImGui::TreePop();
                    }
    ...
    void LogicViewerGui::drawAppearanceBindings()
    {
        ...
        if (ImGui::BeginPopupContextItem("AppearanceBindingsContextMenu"))
        {
            ...
            {
                copyInputs(LogicViewer::ltnAppearance, m_logicEngine.getCollection<AppearanceBinding>());
            }
            ImGui::EndPopup();
        }
        if (openBindings)
        {
            for (auto* obj : m_logicEngine.getCollection<AppearanceBinding>())
            {
                const bool open = DrawTreeNode(obj);
    ...
    void LogicViewerGui::drawDisplaySettings()
    {
        ...
        {
            if (ImGui::DragFloat4("Clear color", glm::value_ptr(m_clearColor), 0.1f, 0.f, 1.f))
            {
                m_renderer->setDisplayBufferClearColor(m_displayId, m_displayBufferId, m_clearColor);
                m_renderer->flush();
            }
