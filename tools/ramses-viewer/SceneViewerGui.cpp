...
namespace ramses::internal
{
    ...
    {
        const auto issueType = m_validationCache.getIssueType(obj);
        const bool isUnused = (obj.getType() != ERamsesObjectType::LogicObject) &&  !m_usedObjects.contains(&obj);
        if (issueType.has_value() && *issueType == EIssueType::Error)
        {
            ImGui::PushStyleColor(ImGuiCol_Text, ImColor(255, 0, 0).Value);
        }
        else if (issueType.has_value())
        {
        ...
        if (issueType.has_value())
        {
            if (isOpen)
                DrawIssues(obj, m_validationCache);
            ImGui::PopStyleColor();
        }
        else if (isUnused)
        {
    ...
    void SceneViewerGui::DrawIssues(const SceneObjectImpl& obj, const ValidationCache& report)
    {
        const auto range = report.getIssues(obj);
        for (auto it = range.first; it != range.second; ++it)
        {
            auto m = *it;
            // hide child issues unless it's a logic object
            bool showObj = m.object->isOfType(ERamsesObjectType::LogicObject) || (m.object == &obj.getRamsesObject());
            if (showObj && m.type <= ramses::EIssueType::Warning)
            {
                switch (m.type)
                {
                case ramses::EIssueType::Error:
    ...
    SceneViewerGui::SceneViewerGui(ViewerGuiApp& app, ramses::Scene& scene, std::string& errorMessage, ProgressMonitor& progressMonitor)
        : m_scene(scene)
    ...
    void SceneViewerGui::draw(SceneObjectImpl& obj)
    {
        if (drawRamsesObject(obj))
        {
            ...
            case ramses::ERamsesObjectType::LogicEngine:
                drawLogicEngine(static_cast<LogicEngineImpl&>(obj));
                break;
            case ramses::ERamsesObjectType::LogicObject:
                drawLogicObject(static_cast<LogicObjectImpl&>(obj));
                break;
    ...
    void SceneViewerGui::drawLogicEngine(LogicEngineImpl& obj)
    {
    ...
    void SceneViewerGui::drawLogicObject(LogicObjectImpl& obj)
    {
        ...
        auto* logicNode = obj.getLogicObject().as<LogicNode>();
        assert(logicNode);
        auto* appearanceBinding = logicNode->as<AppearanceBinding>();
        auto* cameraBinding = logicNode->as<CameraBinding>();
        auto* nodeBinding = logicNode->as<NodeBinding>();
        auto* renderGroupBinding = logicNode->as<RenderGroupBinding>();
        auto* renderPassBinding = logicNode->as<RenderPassBinding>();
        auto* meshNodeBinding = logicNode->as<MeshNodeBinding>();
        auto* skinBinding = logicNode->as<SkinBinding>();
        auto* anchorPoint = logicNode->as<AnchorPoint>();
        auto* animationNode = logicNode->as<AnimationNode>();
        auto* renderBufferBinding = logicNode->as<RenderBufferBinding>();
        if (appearanceBinding)
        {
            engineGui.drawNode(appearanceBinding);
            draw(appearanceBinding->getRamsesAppearance().impl());
        }
        else if (animationNode)
        {
            engineGui.draw(animationNode);
        }
        else if (cameraBinding)
        {
            engineGui.drawNode(cameraBinding);
            draw(cameraBinding->getRamsesCamera().impl());
        }
        else if (nodeBinding)
        {
            engineGui.drawNode(nodeBinding);
            draw(nodeBinding->getRamsesNode().impl());
        }
        else if (meshNodeBinding)
        {
            engineGui.drawNode(meshNodeBinding);
            draw(meshNodeBinding->getRamsesMeshNode().impl());
        }
        else if (renderGroupBinding)
        {
            engineGui.drawNode(renderGroupBinding);
            draw(renderGroupBinding->getRamsesRenderGroup().impl());
        }
        else if (renderPassBinding)
        {
            engineGui.drawNode(renderPassBinding);
            draw(renderPassBinding->getRamsesRenderPass().impl());
        }
        else if (skinBinding)
        {
            engineGui.drawNode(skinBinding);
            ImGui::TextUnformatted(fmt::format("Ramses Uniform input: {}", skinBinding->getAppearanceUniformInput().getName()).c_str());
            draw(skinBinding->getAppearanceBinding().getRamsesAppearance().impl());
        }
        else if (renderBufferBinding)
        {
            engineGui.drawNode(renderBufferBinding);
            draw(renderBufferBinding->getRenderBuffer().impl());
        }
        else if (anchorPoint)
        {
            engineGui.drawNode(anchorPoint);
            draw(anchorPoint->impl().getNodeBinding().getRamsesNode().impl());
            draw(anchorPoint->impl().getCameraBinding().getRamsesCamera().impl());
        }
        else
        {
            engineGui.drawNode(logicNode);
        }
    }
    ...
    void SceneViewerGui::drawCameraNode(CameraNodeImpl& obj)
    {
        ...
        if (obj.isFrustumPlanesBound())
        {
            ...
            auto frustum = const_cast<ramses::DataObject*>(findDataObject(obj.getFrustumPlanesHandle()));
            if (frustum != nullptr)
            {
                ImGui::SameLine();
                draw(frustum->impl());
            ...
            auto nearFar = const_cast<ramses::DataObject*>(findDataObject(obj.getFrustumNearFarPlanesHandle()));
            if (nearFar != nullptr)
            {
                ImGui::SameLine();
                draw(nearFar->impl());
    ...
    void SceneViewerGui::drawTexture2DBuffer(Texture2DBufferImpl& obj)
    {
        ...
        for (auto it = tb.mipMaps.begin(); it != tb.mipMaps.end(); ++it)
        {
            ...
            if (ImGui::Button("Save png"))
            {
                const auto filename = fmt::format("{:04}_Texture2DBuffer.png", obj.getSceneObjectId());
    ...
    void SceneViewerGui::drawDataObject(ramses::DataObject& obj)
    {
        ...
        drawRefs<ramses::Camera>("Used by Camera", obj.impl(), [&](const ramses::Camera* ref) {
            auto fp = findDataObject(ref->impl().getFrustumPlanesHandle());
            auto nf = findDataObject(ref->impl().getFrustumNearFarPlanesHandle());
    ...
    void SceneViewerGui::drawRenderHierarchy()
    {
        if (ImGui::CollapsingHeader("Render hierarchy"))
        {
            ...
            for (auto pObj : m_renderInfo.renderPassVector)
            {
                ...
                const auto handle = renderPass.getSceneObjectId().getValue();
                const char* name = renderPass.getName().c_str();

                auto treeNode = [&]() { return ImGui::TreeNode(&renderPass, "RenderPass[%lu]: %s", handle, name); };
                auto treeNodeRT = [&]() { return ImGui::TreeNode(&renderPass, "RenderPass[%lu]: %s <RT:%ux%u>", handle, name, rt->getWidth(), rt->getHeight()); };
    ...
    {
        if (!m_validationCache.getAllIssues().empty())
        {
            if (ImGui::CollapsingHeader("Objects with warnings/errors"))
            {
                if (ImGui::Button("Copy to clipboard"))
                {
                    ...
                    for (auto& issue : m_validationCache.getAllIssues())
                    {
                        if (issue.object)
                        {
                ...
                for (auto& msg : m_validationCache.getAllIssues())
                {
    ...
    void SceneViewerGui::drawGlobalContextMenuItems()
    {
        drawMenuItemCopyTexture2D();
        drawMenuItemStorePng();
        drawMenuItemExportShaderSources();
    ...
    void SceneViewerGui::processObjectsAsync(F&& func, ramses::ERamsesObjectType objType, const char* message)
    {
        ...
        for (size_t i = 0; i < tasks; ++i)
        {
            ...
            if (i + 1 == tasks)
            {
                futures.push_back(std::async(std::launch::async, func, std::ref(m_progressMonitor), SceneObjectVector(begin, objects.end())));
            }
            else
            {
        ...
        m_progressMonitor.start(std::move(futures), static_cast<uint32_t>(objects.size()), message);
    }
    ...
    void SceneViewerGui::drawMenuItemStorePng()
    {
        ...
        {
            auto storeAllTextures = [&](ProgressMonitor& progress, SceneObjectVector objects) {
                std::vector<std::string> errorList;
                for (auto it = objects.begin(); it != objects.end() && !progress.canceled; ++it)
                {
    ...
    void SceneViewerGui::drawMenuItemExportShaderSources()
    {
        ...
        {
            auto exportShaders = [&](ProgressMonitor& progress, SceneObjectVector objects) {
                std::vector<std::string> errorList;
                for (auto it = objects.begin(); it != objects.end() && !progress.canceled; ++it)
                {
    ...
    std::string SceneViewerGui::saveTexture2D(const Texture2DImpl& obj) const
    {
        ...
        {
            const auto* textureResource = resource->convertTo<ramses::internal::TextureResource>();
            const auto filename = fmt::format("{:04}_{}.png", obj.getSceneObjectId(), obj.getName());
