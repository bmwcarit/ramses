...
namespace ramses::internal
{
    ...
    ViewerGuiApp::ExitCode ViewerGuiApp::setup()
    {
        ...
        getScene()->flush();
        const auto& clearColor = m_displayConfig.impl().getInternalDisplayConfig().getClearColor();
        renderer->setDisplayBufferClearColor(displayId, m_sceneSetup->getOffscreenBuffer(), clearColor);
        m_sceneSetup->apply();
        ...
        m_gui = std::make_unique<SceneViewerGui>(*this, *getScene());
        m_gui->setSceneTexture(m_sceneSetup->getTextureSampler(), winWidth, winHeight);

        if (getLogicViewer())
        {
            m_logicGui = std::make_unique<LogicViewerGui>(*getLogicViewer(), *getSettings(), getLuaFile());
            m_logicGui->setRendererInfo(*renderer, displayId, m_sceneSetup->getOffscreenBuffer(), clearColor);
        }
    ...
    bool ViewerGuiApp::doOneLoop()
    {
        ...
        if (m_guiMode != GuiMode::Off)
        {
            ...
            if (m_logicGui)
            {
                ...
                if (!loadLuaStatus.ok())
                    m_logicGui->openErrorPopup(loadLuaStatus.getMessage());
                if (!updateStatus.ok())
                    m_logicGui->openErrorPopup(updateStatus.getMessage());
            }
            ImGui::EndFrame();
            m_imguiHelper->draw();
        }
