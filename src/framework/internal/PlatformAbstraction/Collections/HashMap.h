...
namespace ramses::internal
{
    ...
    private:
        ...
        private:
            HashMapEntry* next{nullptr}; // pointer to the next entry (chaining)
            HashMapEntry* previous{nullptr}; // pointer to the previous entry (chaining)
            // NOLINTNEXTLINE(modernize-avoid-c-arrays)
            alignas(Pair) char keyValuePairMemory[sizeof(Pair)]{0}; // properly aligned memory for key and value
            bool isChainElement{false}; // true if the element is not the first element in a chain
    ...
    private:
        ...
        uint8_t        mBitCount{0}; // bit size for the hash function
        size_t         mSize{0}; // the size of the data list
        size_t         mThreshold{0}; // defines when a rehashing may occur
        HashMapEntry** mBuckets{nullptr}; // bucket list
        HashMapEntry*  mData{nullptr}; // placeholder for the data
        HashMapEntry*  mLastHashMapEntry{nullptr}; // pointer to the last entry in the map
        HashMapEntry*  mFirstFreeHashMapEntry{nullptr}; // start of pointer list of free entries
        size_t mCount{0u}; // the current entry count
    ...
    */
    ...
    inline uint8_t HashMap<Key, T>::BitSizeFromElementCount(size_t count)
    {
        ...
        const auto countRespectingLoadFactor = static_cast<size_t>(std::ceil(static_cast<double>(count) / DefaultHashMapMaxLoadFactor));
