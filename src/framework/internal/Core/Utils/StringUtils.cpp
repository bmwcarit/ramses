...
namespace
{
    ...
    void tokenizeTo(std::string_view string, const char splitChar, F f)
    {
        while (!string.empty())
        {
            ...
            if (const auto s = string.substr(0, pos); !s.empty() && s != " ")
            {
                f(s);
            }
...
{
    std::string StringUtils::Trim(std::string_view string)
    {
        return std::string{TrimView(string)};
    }

    std::string_view StringUtils::TrimView(std::string_view string)
    {
        if (const auto pos = string.find_first_not_of("\t\n\v\f\r "); pos != std::string_view::npos)
        {
    ...
    std::vector<std::string> StringUtils::Tokenize(std::string_view string, const char splitChar)
    {
    ...
    HashSet<std::string> StringUtils::TokenizeToSet(std::string_view string, const char split)
    {
        HashSet<std::string> result{};
        auto f = [&result](const auto token) { result.put(std::string{token}); };
