...
namespace ramses::internal
{
        ...
        , m_geometryShaderInputType(geometryShaderInputType)
    {
        ...
        const auto dataLength = static_cast<uint32_t>(vertexShader.size() + 1 + fragmentShader.size() + 1 + geometryShader.size() + 1); // including 3x terminating '0'
    ...
    EffectResource::EffectResource(EffectInputInformationVector&& uniformInputs, EffectInputInformationVector&& attributeInputs, std::optional<EDrawMode> geometryShaderInputType,
    ...
    std::unique_ptr<IResource> EffectResource::CreateResourceFromMetadataStream(IInputStream& input, std::string_view name)
    {
        ...
        return std::unique_ptr<EffectResource>(new EffectResource(std::move(uniformInputs), std::move(attributeInputs), gsInputType, name, fragementShaderOffset, geometryShaderOffset));
    }
