...
{
    class MergeScene : public IScene
    {
    public:
        explicit MergeScene(IScene& originalScene, SceneMergeHandleMapping& mapping);
    ...
    private:
        template<typename T>
        [[nodiscard]] TypedMemoryHandle<T> getOffsetHandle() const;

        template<typename T>
        [[nodiscard]] inline TypedMemoryHandle<T> getMappedHandle(TypedMemoryHandle<T> handle) const;

        template<typename T>
        [[nodiscard]] inline TypedMemoryHandle<T> addMapping(TypedMemoryHandle<T> handle, TypedMemoryHandle<T> newHandle);

        IScene& m_originalScene;
        SceneMergeHandleMapping& m_mapping;

        RenderableHandle m_offsetRenderableHandle{0u};
        RenderStateHandle m_offsetRenderStateHandle{0u};
        CameraHandle m_offsetCameraHandle{0u};
        NodeHandle m_offsetNodeHandle{0u};
        TransformHandle m_offsetTransformHandle{0u};
        DataLayoutHandle m_offsetDataLayoutHandle{0u};
        DataInstanceHandle m_offsetDataInstanceHandle{0u};
        UniformBufferHandle m_offsetUniformBufferHandle{0u};
        TextureSamplerHandle m_offsetTextureSamplerHandle{0u};
        RenderGroupHandle m_offsetRenderGroupHandle{0u};
        RenderPassHandle m_offsetRenderPassHandle{0u};
        BlitPassHandle m_offsetBlitPassHandle{0u};
        PickableObjectHandle m_offsetPickableObjectHandle{0u};
        RenderTargetHandle m_offsetRenderTargetHandle{0u};
        RenderBufferHandle m_offsetRenderBufferHandle{0u};
        DataBufferHandle m_offsetDataBufferHandle{0u};
        TextureBufferHandle m_offsetTextureBufferHandle{0u};
        DataSlotHandle m_offsetDataSlotHandle{0u};
        SceneReferenceHandle m_offsetSceneReferenceHandle{0u};
    };
}
