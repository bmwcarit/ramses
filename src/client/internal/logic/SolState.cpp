...
#include "internal/logic/LuaTypeConversions.h"
...
namespace ramses::internal
{
    ...
    sol::environment SolState::createEnvironment(const StandardModules& stdModules, const ModuleMapping& userModules, bool exposeDebugLogFunctions)
    {
        ...
        for (const auto& module : userModules)
        {
            ...
            protectedEnv[module.first] = module.second->impl().getModule();
        }
    ...
    std::optional<std::string_view> SolState::GetStdModuleName(EStandardModule m)
    {
        switch (m)
        {
