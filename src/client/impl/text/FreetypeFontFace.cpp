...
namespace ramses::internal
{
    ...
    namespace
    {
        // NOLINTNEXTLINE(google-runtime-int): see FT_Stream_IoFunc
        unsigned long ReadFTStream(FT_Stream stream, unsigned long offset, unsigned char *buffer, unsigned long count)
        {
            ...
            auto* bofis = static_cast<BinaryOffsetFileInputStream*>(stream->descriptor.pointer);
            bofis->seek(static_cast<int64_t>(offset), IInputStream::Seek::FromBeginning);
            if (count > 0)
        ...
        , m_fileStream(fd, offset, length)
    {
        ...
        m_fontStream.size = static_cast<unsigned long>(length); // NOLINT(google-runtime-int): see FT_StreamRec
