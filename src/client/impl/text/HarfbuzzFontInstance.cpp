...
namespace ramses::internal
{
    ...
    void HarfbuzzFontInstance::activateHBFontSize()
    {
        ...
        const auto x_scale = static_cast<uint64_t>(m_size->metrics.x_scale);
        const auto y_scale = static_cast<uint64_t>(m_size->metrics.y_scale);
        const auto units_per_EM = static_cast<uint64_t>(m_face->units_per_EM);
