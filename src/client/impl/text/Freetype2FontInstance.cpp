...
namespace ramses::internal
{
    ...
    const GlyphMetrics* Freetype2FontInstance::getGlyphMetricsData(GlyphId glyphId)
    {
        ...
        metrics.posX = static_cast<int32_t>(glyphMetrics.horiBearingX / 64);
        metrics.posY = static_cast<int32_t>((glyphMetrics.horiBearingY - glyphMetrics.height) / 64);
        metrics.advance = static_cast<int32_t>(glyphMetrics.horiAdvance / 64);
    ...
    const Freetype2FontInstance::GlyphBitmapData* Freetype2FontInstance::getGlyphBitmapData(GlyphId glyphId)
    {
        ...
        auto error = FT_Get_Glyph(m_face->glyph, &ftGlyph);
        {
            ...
            const auto bitmapGlyph = reinterpret_cast<FT_BitmapGlyph>(ftGlyph);
    ...
    std::unordered_set<char32_t> Freetype2FontInstance::getAllSupportedCharacters()
    {
        ...
        std::unordered_set<char32_t> allSupportedCharactersSet;
