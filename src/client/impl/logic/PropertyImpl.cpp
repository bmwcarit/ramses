...
#include "internal/logic/SerializationHelper.h"
...
namespace ramses::internal
{
        ...
        DeserializationMap& deserializationMap)
    {
        ...
        if (!prop.name())
        {
            errorReporting.set("Fatal error during loading of Property from serialized data: missing name!", nullptr);
            return nullptr;
        }
        ...
        if (!convertedType)
        {
            errorReporting.set("Fatal error during loading of Property from serialized data: invalid type!", nullptr);
            return nullptr;
        }
        ...
        if (prop.rootType() == rlogic_serialization::EPropertyRootType::Primitive)
        {
            ...
            case rlogic_serialization::PropertyValue::float_s:
                ...
                {
                    errorReporting.set("Fatal error during loading of Property from serialized data: invalid union!", nullptr);
                    return nullptr;
                }
        ...
        else
        {
            ...
            if (!prop.children())
            {
                errorReporting.set("Fatal error during loading of Property from serialized data: complex type has no child type info!", nullptr);
                return nullptr;
            }
            ...
            for (const auto* child : *prop.children())
            {
                if (!child)
                {
                    ...
                    errorReporting.set("Fatal error during loading of Property from serialized data: corrupt child data!", nullptr);
                    return nullptr;
                }
