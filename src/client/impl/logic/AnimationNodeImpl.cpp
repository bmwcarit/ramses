...
{
    AnimationNodeImpl::AnimationNodeImpl(SceneImpl& scene, AnimationChannels channels, bool exposeDataAsProperties, std::string_view name, sceneObjectId_t id) noexcept
        ...
        , m_hasChannelDataExposedViaProperties{ exposeDataAsProperties }
    {
        ...
        for (size_t i = 0u; i < m_channels.size(); ++i)
        {
            ...
            m_channelsWorkData[i].keyframes = m_channels[i].keyframes->impl().getDataVariant();
        ...
        std::visit([outputValueProp](const auto& v) {
            ...
            if constexpr (std::is_same_v<ValueType, std::vector<float>>)
            {
                ...
                for (size_t arrayIdx = 0u; arrayIdx < v.size(); ++arrayIdx)
                    outputValueProp->getChild(arrayIdx)->impl().setValue(v[arrayIdx]);
            }
            else
            {
                outputValueProp->impl().setValue(PropertyValue{ v });
            }
            }, interpolatedValue);
    }
        ...
        SerializationMap& serializationMap)
    {
        ...
        for (const auto& channel : animNode.m_channels)
        {
            ...
            channelsFB.push_back(rlogic_serialization::CreateChannel(
                ...
                serializationMap.resolveDataArrayOffset(channel.timeStamps->getSceneObjectId()),
                serializationMap.resolveDataArrayOffset(channel.keyframes->getSceneObjectId()),
                interpTypeFB,
                channel.tangentsIn ? serializationMap.resolveDataArrayOffset(channel.tangentsIn->getSceneObjectId()) : 0,
                channel.tangentsOut ? serializationMap.resolveDataArrayOffset(channel.tangentsOut->getSceneObjectId()) : 0
                ));
        }

        const auto logicObject = LogicObjectImpl::Serialize(animNode, builder);
        const auto inputPropertyObject = PropertyImpl::Serialize(animNode.getInputs()->impl(), builder, serializationMap);
        const auto ouputPropertyObject = PropertyImpl::Serialize(animNode.getOutputs()->impl(), builder, serializationMap);
        ...
        DeserializationMap& deserializationMap)
    {
        ...
        {
            errorReporting.set("Fatal error during loading of AnimationNode from serialized data: missing name, id, channels or in/out property data!", nullptr);
            return nullptr;
        }
        ...
        for (const auto* channelFB : *animNodeFB.channels())
        {
            if (!channelFB->name() ||
                ...
                !channelFB->keyframes())
            {
                errorReporting.set(fmt::format("Fatal error during loading of AnimationNode '{}' channel data: missing name, timestamps or keyframes!", name), nullptr);
                return nullptr;
            }
            ...
            case rlogic_serialization::EInterpolationType::Cubic_Quaternions:
                ...
                break;
            default:
                errorReporting.set(fmt::format("Fatal error during loading of AnimationNode '{}' channel '{}' data: missing or invalid interpolation type!", name, channel.name), nullptr);
                return nullptr;
            }
            ...
            if (channel.interpolationType == EInterpolationType::Cubic || channel.interpolationType == EInterpolationType::Cubic_Quaternions)
            {
                ...
                {
                    errorReporting.set(fmt::format("Fatal error during loading of AnimationNode '{}' channel '{}' data: missing tangents!", name, channel.name), nullptr);
                    return nullptr;
                }
        ...
        auto deserialized = std::make_unique<AnimationNodeImpl>(deserializationMap.getScene(), std::move(channels), hasChannelDataProperties, name, id);
        ...
        {
            errorReporting.set(fmt::format("Fatal error during loading of AnimationNode '{}': missing or invalid properties!", name), nullptr);
            return nullptr;
        }
        ...
        {
            errorReporting.set(fmt::format("Fatal error during loading of AnimationNode '{}': missing or invalid channels data property!", name), nullptr);
            return nullptr;
        }
    ...
    void AnimationNodeImpl::initAnimationDataPropertyValues()
    {
        ...
        for (size_t channelIdx = 0u; channelIdx < channelsData->getChildCount(); ++channelIdx)
        {
            ...
            for (size_t i = 0u; i < timestamps.size(); ++i)
                timestampsProp->getChild(i)->impl().setValue(timestamps[i]);
            ...
            std::visit([&](const auto& keyframes) {
                ...
                else
                {
                    for (size_t i = 0u; i < keyframes.size(); ++i)
                        keyframesProp->getChild(i)->impl().setValue(keyframes[i]);
                }
