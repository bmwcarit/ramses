...
namespace ramses::internal
{
    ...
    void Window_Windows::handleKeyEvent(uint32_t windowsMsg, WPARAM wParam, LPARAM lParam)
    {
        ...
        if (keyPressed)
        {
            m_keyModifiers.setFlag(keyModifier, true);
        }
        else if (keyReleased)
        {
            m_keyModifiers.setFlag(keyModifier, false);
        }

        const EKeyCode keyCode = convertVirtualKeyCodeIntoRamsesKeyCode(wParam, lParam);
        m_eventHandler.onKeyEvent(keyPressed ? EKeyEvent::Pressed : EKeyEvent::Released, m_keyModifiers, keyCode);
    }
    ...
    LRESULT WINAPI Window_Windows::WindowProcedure(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
    {
        ...
        case WM_MOUSELEAVE:
            ...
            window->handleMouseEvent(EMouseEvent::WindowLeave, window->m_mousePosX, window->m_mousePosY);
            break;
        case WM_MOUSEMOVE:
            if (NULL == window)
                break;
            ...
            if (!window->m_isMouseTracked)
            {
                ...
                window->handleMouseEvent(EMouseEvent::WindowEnter, pt.x, pt.y);
            }

            window->m_mousePosX = pt.x;
            window->m_mousePosY = pt.y;
            window->handleMouseEvent(EMouseEvent::Move, pt.x, pt.y);
        ...
        case WM_LBUTTONDOWN:
        {
            if (NULL != window)
            {
                ...
                window->handleMouseEvent(EMouseEvent::LeftButtonDown, pt.x, pt.y);
            }
        }
        break;
        case WM_LBUTTONUP:
        {
            if (NULL != window)
            {
                ...
                window->handleMouseEvent(EMouseEvent::LeftButtonUp, pt.x, pt.y);
            }
        }
        break;
        case WM_RBUTTONDOWN:
        {
            if (NULL != window)
            {
                ...
                window->handleMouseEvent(EMouseEvent::RightButtonDown, pt.x, pt.y);
            }
        }
        break;
        case WM_RBUTTONUP:
        {
            if (NULL != window)
            {
                ...
                window->handleMouseEvent(EMouseEvent::RightButtonUp, pt.x, pt.y);
            }
        }
        break;
        case WM_MBUTTONDOWN:
        {
            if (NULL != window)
            {
                ...
                window->handleMouseEvent(EMouseEvent::MiddleButtonDown, pt.x, pt.y);
            }
        }
        break;
        case WM_MBUTTONUP:
        {
            if (NULL != window)
            {
                ...
                window->handleMouseEvent(EMouseEvent::MiddleButtonUp, pt.x, pt.y);
            }
        }
        break;
        case WM_MOUSEWHEEL:
        {
            if (NULL != window)
            {
                ...
                EMouseEvent event = (totalDelta > 0) ? EMouseEvent::WheelUp : EMouseEvent::WheelDown;
    ...
    void Window_Windows::handleMouseEvent(EMouseEvent type, int32_t posX, int32_t posY)
    {
