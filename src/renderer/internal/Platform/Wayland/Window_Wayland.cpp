...
namespace ramses::internal
{
    ...
    bool Window_Wayland::init()
    {
        ...
        m_wlContext.native_window = wl_egl_window_create(m_wlContext.surface, static_cast<int>(m_width), static_cast<int>(m_height));
    ...
    void Window_Wayland::RegistryGlobalCreated(void* data, wl_registry* wl_registry, uint32_t name, const char* interface, [[maybe_unused]] uint32_t version)
    {
        Window_Wayland& window = *static_cast<Window_Wayland*>(data);
        window.registryGlobalCreated(wl_registry, name, interface, version);
    }

    void Window_Wayland::RegistryGlobalRemoved([[maybe_unused]] void* data, [[maybe_unused]] wl_registry* wl_registry, [[maybe_unused]] uint32_t name)
    {
        // callback to destroy globals not implemented in neither winston or weston, globals are destroyed in destructor.
    }

    void Window_Wayland::FrameRenderingDoneCallback(void* userData, wl_callback* callbackWaylandObject, uint32_t /*unused*/)
    {
        auto* window = static_cast<Window_Wayland*>(userData);
    ...
    bool Window_Wayland::setFullscreen([[maybe_unused]] bool fullscreen)
    {
        return true;
    }
    ...
    void Window_Wayland::registryGlobalCreated(wl_registry* wl_registry, uint32_t name, const char* interface, [[maybe_unused]] uint32_t version)
    {
