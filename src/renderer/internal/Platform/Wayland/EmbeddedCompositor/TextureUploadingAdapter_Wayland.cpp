...
namespace ramses::internal
{
    ...
    void TextureUploadingAdapter_Wayland::uploadTextureFromWaylandResource(DeviceResourceHandle textureHandle, wl_resource *bufferResource)
    {
        if (m_waylandEglExtensionProcs.areExtensionsSupported())
        {
            ...
            EGLImage eglImage = m_waylandEglExtensionProcs.eglCreateImageKHR(nullptr, EGL_WAYLAND_BUFFER_WL, bufferResource, nullptr);
    ...
    TextureUploadingAdapter_Wayland::DmabufEglImage* TextureUploadingAdapter_Wayland::importDmabufToEglImage(LinuxDmabufBufferData* dmabuf)
    {
        ...
        attribs[atti++] = static_cast<EGLint>(dmabuf->getFormat());
        ...
        if (dmabuf->getNumPlanes() > 0)
        {
            ...
            attribs[atti++] = static_cast<EGLint>(dmabuf->getOffset(0));
            attribs[atti++] = EGL_DMA_BUF_PLANE0_PITCH_EXT;
            attribs[atti++] = static_cast<EGLint>(dmabuf->getStride(0));

            if (hasModifier)
            {
                attribs[atti++] = EGL_DMA_BUF_PLANE0_MODIFIER_LO_EXT;
                attribs[atti++] = static_cast<EGLint>(dmabuf->getModifier(0) & 0xffffffff);
                attribs[atti++] = EGL_DMA_BUF_PLANE0_MODIFIER_HI_EXT;
                attribs[atti++] = static_cast<EGLint>(dmabuf->getModifier(0) >> 32);
            }
        }
        ...
        if (dmabuf->getNumPlanes() > 1)
        {
            ...
            attribs[atti++] = static_cast<EGLint>(dmabuf->getOffset(1));
            attribs[atti++] = EGL_DMA_BUF_PLANE1_PITCH_EXT;
            attribs[atti++] = static_cast<EGLint>(dmabuf->getStride(1));

            if (hasModifier)
            {
                attribs[atti++] = EGL_DMA_BUF_PLANE1_MODIFIER_LO_EXT;
                attribs[atti++] = static_cast<EGLint>(dmabuf->getModifier(1) & 0xffffffff);
                attribs[atti++] = EGL_DMA_BUF_PLANE1_MODIFIER_HI_EXT;
                attribs[atti++] = static_cast<EGLint>(dmabuf->getModifier(1) >> 32);
            }
        }
        ...
        if (dmabuf->getNumPlanes() > 2)
        {
            ...
            attribs[atti++] = static_cast<EGLint>(dmabuf->getOffset(2));
            attribs[atti++] = EGL_DMA_BUF_PLANE2_PITCH_EXT;
            attribs[atti++] = static_cast<EGLint>(dmabuf->getStride(2));

            if (hasModifier)
            {
                attribs[atti++] = EGL_DMA_BUF_PLANE2_MODIFIER_LO_EXT;
                attribs[atti++] = static_cast<EGLint>(dmabuf->getModifier(2) & 0xffffffff);
                attribs[atti++] = EGL_DMA_BUF_PLANE2_MODIFIER_HI_EXT;
                attribs[atti++] = static_cast<EGLint>(dmabuf->getModifier(2) >> 32);
            }
        }
        ...
        if (dmabuf->getNumPlanes() > 3)
        {
            ...
            attribs[atti++] = static_cast<EGLint>(dmabuf->getOffset(3));
            attribs[atti++] = EGL_DMA_BUF_PLANE3_PITCH_EXT;
            attribs[atti++] = static_cast<EGLint>(dmabuf->getStride(3));

            if (hasModifier)
            {
                attribs[atti++] = EGL_DMA_BUF_PLANE3_MODIFIER_LO_EXT;
                attribs[atti++] = static_cast<EGLint>(dmabuf->getModifier(3) & 0xffffffff);
                attribs[atti++] = EGL_DMA_BUF_PLANE3_MODIFIER_HI_EXT;
                attribs[atti++] = static_cast<EGLint>(dmabuf->getModifier(3) >> 32);
            }
        }
    ...
    bool TextureUploadingAdapter_Wayland::uploadTextureFromLinuxDmabuf(DeviceResourceHandle textureHandle, LinuxDmabufBufferData* dmabuf)
    {
        ...
        if (iter == m_dmabufEglImagesMap.end())
        {
            ...
            dmabuf->setDestroyCallback([this](LinuxDmabufBufferData* buf) { handleDmabufDestroy(buf); });
