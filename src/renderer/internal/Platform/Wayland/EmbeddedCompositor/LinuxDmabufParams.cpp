...
namespace ramses::internal
{
        ...
        : m_clientCredentials(client.getCredentials())
    {
        ...
        m_resource = client.resourceCreate(&zwp_linux_buffer_params_v1_interface, static_cast<int>(version), id);
    ...
    {
        auto* dmabufParams = static_cast<LinuxDmabufParams*>(wl_resource_get_user_data(dmabufParamsResource));
    ...
    void LinuxDmabufParams::DmabufParamsDestroyCallback([[maybe_unused]] wl_client* client, wl_resource* dmabufParamsResource)
    {
        auto* params = static_cast<LinuxDmabufParams*>(wl_resource_get_user_data(dmabufParamsResource));
        delete params;
    }

    void LinuxDmabufParams::DmabufParamsAddCallback([[maybe_unused]] wl_client* client, wl_resource* dmabufParamsResource, int32_t fd, uint32_t plane_idx, uint32_t offset, uint32_t stride, uint32_t modifier_hi, uint32_t modifier_lo)
    {
        auto* params = static_cast<LinuxDmabufParams*>(wl_resource_get_user_data(dmabufParamsResource));
    ...
    {
        auto* data = static_cast<LinuxDmabufBufferData*>(wl_resource_get_user_data(bufferResource));
        assert(nullptr != data);
        delete data;
    }
    ...
    void LinuxDmabufParams::createBuffer(IWaylandClient& client, uint32_t bufferId, int32_t width, int32_t height, uint32_t format, uint32_t flags)
    {
        ...
        for (unsigned int i = 0; i < m_data->getNumPlanes(); ++i)
        {
            auto const offset = static_cast<uint64_t>(m_data->getOffset(i));
            auto const stride = static_cast<uint64_t>(m_data->getStride(i));
    ...
    void LinuxDmabufBuffer::DmabufBufferDestroyCallback([[maybe_unused]] wl_client* client, wl_resource* bufferResource)
    {
    ...
    LinuxDmabufBufferData* LinuxDmabufBuffer::fromWaylandBufferResource(WaylandBufferResource& resource)
    {
        ...
        if (wl_resource_instance_of(nativeResource, &wl_buffer_interface, &m_bufferInterface) == 0)
        {
            return nullptr;
        }

        auto* ret = static_cast<LinuxDmabufBufferData*>(wl_resource_get_user_data(nativeResource));
        assert(nullptr != ret);
        return ret;
    }
}
