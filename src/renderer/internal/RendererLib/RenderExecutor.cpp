...
namespace ramses::internal
{
    ...
    bool RenderExecutor::executeRenderPass(const RendererCachedScene& scene, const RenderPassHandle pass) const
    {
        ...
        {
            ClearFlags clearFlags = EClearFlag::None;
            glm::vec4 clearColor;
            ...
            else if (renderContext.displayBufferClearPending != EClearFlag::None)
            {
                clearFlags = renderContext.displayBufferClearPending;
                clearColor = renderContext.displayBufferClearColor;
                renderContext.displayBufferClearPending = EClearFlag::None;
            }
    ...
    void RenderExecutor::executeDrawCall() const
    {
        ...
        {
            device.drawIndexedTriangles(static_cast<int32_t>(renderable.startIndex), static_cast<int32_t>(renderable.indexCount), renderable.instanceCount);
        }
        else
        {
            device.drawTriangles(static_cast<int32_t>(renderable.startIndex), static_cast<int32_t>(renderable.indexCount), renderable.instanceCount);
        }
    }
            ...
            for (uint32_t i = 0u; i < uint32_t(orderedPasses.size()); ++i)
            {
                ...
                if (rpInfo.getType() == ERenderingPassType::RenderPass)
                {
                    ...
                    if (FindDepthRenderBufferInRenderTarget(scene, rpData.renderTarget) == depthRB)
                        return rpData.clearFlags.isSet(EClearFlag::Depth) && rpData.clearFlags.isSet(EClearFlag::Stencil);
                }
                else
                {
    ...
    RenderBufferHandle RenderExecutor::FindDepthRenderBufferInRenderTarget(const IScene& scene, RenderTargetHandle renderTarget)
    {
        ...
        for (uint32_t rbIdx = 0u; rbIdx < rbCount; ++rbIdx)
        {
            ...
            if (IsDepthOrStencilFormat(scene.getRenderBuffer(rb).format))
                return rb;
        }
