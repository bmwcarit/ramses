...
namespace ramses::internal
{
    ...
    RendererSceneUpdater::~RendererSceneUpdater()
    {
        ...
        assert(m_scenesToBeMapped.empty());
    ...
    bool RendererSceneUpdater::hasResourceManager() const
    {
        return m_displayResourceManager != nullptr;
    }
    ...
    void RendererSceneUpdater::consolidatePendingSceneActions(SceneId sceneID, SceneUpdate&& sceneUpdate)
    {
        ...
        const uint32_t numActions = sceneUpdate.actions.numberOfActions();
    ...
    bool RendererSceneUpdater::checkIfForceMapNeeded(SceneId sceneId)
    {
        ...
        {
            LOG_WARN_F(CONTEXT_RENDERER, [&](StringOutputStream& logger)
            {
    ...
    bool RendererSceneUpdater::handleBufferCreateRequest(OffscreenBufferHandle buffer, uint32_t width, uint32_t height, uint32_t sampleCount, bool isDoubleBuffered, EDepthBufferType depthStencilBufferType)
    {
        bool success = false;
        if (!m_renderer.hasDisplayController())
        {
    ...
    bool RendererSceneUpdater::handleDmaBufferCreateRequest(OffscreenBufferHandle buffer, uint32_t width, uint32_t height, DmaBufferFourccFormat dmaBufferFourccFormat, DmaBufferUsageFlags dmaBufferUsageFlags, DmaBufferModifiers dmaBufferModifiers)
    {
        bool success = false;
        int dmaBufferFD = -1;
    ...
    void RendererSceneUpdater::handleSetClearFlags(OffscreenBufferHandle buffer, ClearFlags clearFlags)
    {
        if (!m_renderer.hasDisplayController())
        {
    ...
    void RendererSceneUpdater::handleSceneDataLinkRequest(SceneId providerSceneId, DataSlotId providerId, SceneId consumerSceneId, DataSlotId consumerId)
    {
        if (m_rendererScenes.hasScene(providerSceneId) && m_rendererScenes.hasScene(consumerSceneId))
        {
            ...
            if (providerSlotHandle.isValid() && consumerSlotHandle.isValid())
            {
                ...
                if (providerSlotType == EDataSlotType::TextureProvider)
                {
    ...
    void RendererSceneUpdater::handlePickEvent(SceneId sceneId, glm::vec2 coordsNormalizedToBufferSize)
    {
        ...
        const auto width = static_cast<float>(buffer.viewport.width);
        const auto height = static_cast<float>(buffer.viewport.height);
        const glm::ivec2 coordsInBufferSpace = { static_cast<int32_t>(std::lroundf((coordsNormalizedToBufferSize.x + 1.f) * width / 2.f)) ,
                                                static_cast<int32_t>(std::lroundf((coordsNormalizedToBufferSize.y + 1.f) * height / 2.f)) };
    ...
    void RendererSceneUpdater::setLimitFlushesForceApply(size_t limitForPendingFlushesForceApply)
    {
        m_maximumPendingFlushes = limitForPendingFlushesForceApply;
    }

    void RendererSceneUpdater::setLimitFlushesForceUnsubscribe(size_t limitForPendingFlushesForceUnsubscribe)
    {
    ...
    void RendererSceneUpdater::logMissingResources(const ResourceContentHashVector& neededResources, SceneId sceneId) const
    {
        ...
        constexpr size_t MaxMissingResourcesToLog = 10u;
