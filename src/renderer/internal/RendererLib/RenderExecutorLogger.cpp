...
{
    RenderExecutorLogger::RenderExecutorLogger(IDevice& device, RenderingContext& renderContext, RendererLogContext& context)
    ...
    void RenderExecutorLogger::logRenderPass(const RendererCachedScene& scene, const RenderPassHandle pass) const
    {
        ...
        if (!orderedRenderables.empty())
        {
            ...
            if (renderTarget.isValid())
            {
                ...
                m_logContext << "    Access mode: " << EnumToString(renderBuffer.accessMode) << RendererLogContext::NewLine;
                m_logContext << "    Sample count: " << renderBuffer.sampleCount << RendererLogContext::NewLine;

                const auto clearFlags = rp.clearFlags;
                m_logContext << "    Clear flags: "
                            << (clearFlags.isSet(EClearFlag::Color) ? "Color|" : "")
                            << (clearFlags.isSet(EClearFlag::Depth) ? "Depth|" : "")
                            << (clearFlags.isSet(EClearFlag::Stencil) ? "Stencil" : "")
                            << ((clearFlags == EClearFlag::None) ? "None" : "")
