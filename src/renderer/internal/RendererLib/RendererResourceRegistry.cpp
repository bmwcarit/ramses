...
namespace ramses::internal
{
    ...
    void RendererResourceRegistry::setResourceUploaded(const ResourceContentHash& hash, DeviceResourceHandle deviceHandle, uint32_t vramSize)
    {
        assert(m_resources.contains(hash));
        ResourceDescriptor& rd = *m_resources.get(hash);
    ...
    void RendererResourceRegistry::updateListOfResourcesNotInUseByScenes(const ResourceContentHash& hash)
    {
        ...
        {
            auto it = find_c(m_resourcesNotInUseByScenes, hash);
