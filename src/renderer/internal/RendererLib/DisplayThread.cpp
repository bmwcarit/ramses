...
namespace ramses::internal
{
    ...
    DisplayThread::~DisplayThread()
    {
        ...
        {
            LOG_INFO(CONTEXT_RENDERER, "{} stopping", GetThreadName(m_displayHandle));
            {
            ...
            m_thread.join();
            LOG_INFO(CONTEXT_RENDERER, "{} stopped", GetThreadName(m_displayHandle));
        }
    ...
    void DisplayThread::startUpdating()
    {
        if (!m_thread.isRunning())
        {
            LOG_INFO(CONTEXT_RENDERER, "{} starting", GetThreadName(m_displayHandle));
            m_thread.start(*this);
            LOG_INFO(CONTEXT_RENDERER, "{} started", GetThreadName(m_displayHandle));
        }

        LOG_INFO(CONTEXT_RENDERER, "{} start update", GetThreadName(m_displayHandle));
        std::lock_guard<std::mutex> lock{ m_lock };
        m_isUpdating = true;
        m_sleepConditionVar.notify_one();
    }
    ...
    {
        LOG_INFO(CONTEXT_RENDERER, "{} stop update", GetThreadName(m_displayHandle));
        std::lock_guard<std::mutex> lock{ m_lock };
        m_isUpdating = false;
    }
    ...
    {
        LOG_INFO(CONTEXT_RENDERER, "{} loop mode set to {}", GetThreadName(m_displayHandle), loopMode == ELoopMode::UpdateAndRender ? "UpdateAndRender" : "UpdateOnly");
        std::lock_guard<std::mutex> lock{ m_lock };
        m_loopMode = loopMode;
    }
    ...
    {
        LOG_INFO(CONTEXT_RENDERER, "{} min frame duration set to {} us", GetThreadName(m_displayHandle), minLoopPeriod.count());
        std::lock_guard<std::mutex> lock{ m_lock };
    ...
    void DisplayThread::run()
    {
        ...
        LOG_INFO(CONTEXT_RENDERER, "releasing display bundle components");
        m_display.destroy();
    }
