...
namespace ramses
{
    ...
    */
    ...
    public:
        ...
        * to local renderer(s) and not published to remote renderer(s), this is default.
        ...
        * @details Logic is a crucial Ramses component and can be created from a #Scene,
        *          all its bindings (e.g. #ramses::NodeBinding, etc.) must be from
        *          this #Scene - the scene that owns the #ramses::LogicEngine instance.
        *          Like other scene objects #ramses::LogicEngine can be destroyed using #destroy
        *          and will be automatically destroyed when #Scene is destroyed.
        *
        *          Note that multiple instances of #ramses::LogicEngine can be created for single #Scene
        *          but generally this is only meant for special usecases and not recommended.
        *
        * @param[in] name The optional name of the #ramses::LogicEngine
        * @return A pointer to the created #ramses::LogicEngine instance.
        */
        ...
        *
        * @param[in] effect The effect which is used to create the Geometry.
        * @param[in] name The optional name of the created Geometry.
        * @return A pointer to the created Geometry, null on failure
        */
        ...
        */
        template <typename T = SceneObject> [[nodiscard]] T* findObject(std::string_view name);

        /**
        * @copydoc findObject(std::string_view)
        **/
        template <typename T = SceneObject> [[nodiscard]] const T* findObject(std::string_view name) const;
        ...
        * @return Pointer to the object if found and convertible to the demanded type, nullptr otherwise.
        */
        template <typename T = SceneObject> [[nodiscard]] T* findObject(sceneObjectId_t id);

        /**
        * @copydoc findObject(sceneObjectId_t id)
        **/
        template <typename T = SceneObject> [[nodiscard]] const T* findObject(sceneObjectId_t id) const;
        ...
        *
        * @details If an #ramses::ArrayResource object is created with type std::byte (#ramses::EDataType::ByteBlob) then an element
        ...
        * as vertex buffer in #ramses::Geometry.
        *
        ...
        *
        *          If the function returns true, #ramses::IClientEventHandler::dataLinked will be emitted after the link is
        ...
        * @details If the function returns true, #ramses::IClientEventHandler::dataUnlinked will be emitted after it is
        ...
        **/
        [[nodiscard]] Resource* getResource(resourceId_t id);

        /**
         * Get the internal data for implementation specifics of Scene.
         */
        [[nodiscard]] internal::SceneImpl& impl();

        /**
         * Get the internal data for implementation specifics of Scene.
         */
        [[nodiscard]] const internal::SceneImpl& impl() const;
    ...
    protected:
        ...
        */
        explicit Scene(std::unique_ptr<internal::SceneImpl> impl);

        /**
        * Stores internal data for implementation specifics of Scene.
        */
        internal::SceneImpl& m_impl;
    ...
    private:
        ...
        [[nodiscard]] SceneObject* findObjectById(sceneObjectId_t id);
    ...
    template <typename T> T* Scene::findObject(std::string_view name)
    {
        StaticTypeCheck<T>();
        return findObjectByNameInternal<T>(name);
    }

    template <typename T> const T* Scene::findObject(std::string_view name) const
    {
        StaticTypeCheck<T>();
        return findObjectByNameInternal<T>(name);
    }

    template <typename T> T* Scene::findObject(sceneObjectId_t id)
    {
        StaticTypeCheck<T>();
        if constexpr (std::is_same_v<SceneObject, T>)
            return findObjectById(id);
        return object_cast<T*>(findObjectById(id));
    }

    template <typename T> const T* Scene::findObject(sceneObjectId_t id) const
    {
        StaticTypeCheck<T>();
        if constexpr (std::is_same_v<SceneObject, T>)
            return findObjectById(id);
        return object_cast<const T*>(findObjectById(id));
    }
