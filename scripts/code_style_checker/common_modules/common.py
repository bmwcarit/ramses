...
def get_clean_file_contents(file_contents):
    ...
    # remove all strings
    # a string is something that starts with a (") and ends with a ("), can have any character inside it except a (")
    # that is not preceded by a slash (\) that is not preceded by another slash
...
def log_warning(testname, filename, line_number, description, line_content=None):
    ...
    if line_content is None:
        ...
        # gcc format
        print("{0}:{1}: error: {2} [{3}]".format(filename, line_number, description, testname), file=sys.stderr)
    else:
        # visual studio / msbuild format
        print("{0}({1}): warning PRJ9999: {2}: {3} [{4}]".format(filename, line_number, description, line_content, testname), file=sys.stderr)
        # gcc format
        print("{0}:{1}: error: {2}: {3} [{4}]".format(filename, line_number, description, line_content, testname), file=sys.stderr)
