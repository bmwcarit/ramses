...
def test_cmake_configuration(cli_context, build_dir, expect_success, **cmake_options):
    ...
    try:
        ...
        if not expect_success:
            print("Test failed! Death test was supposed to fail cmake configure, but it succeeded!")
            assert False

    except Exception:
        if expect_success:
            print("Test failed! Configuration did not succeed!")
            assert False
...
def main(cli_context, build_dir):
    ...
    # can not configure if trying to build shared lib (on by default) while no window types enabled
    test_cmake_configuration(cli_context, build_dir, False, headless=True)
