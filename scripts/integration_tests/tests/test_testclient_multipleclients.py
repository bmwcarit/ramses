...
class TestClass(test_classes.OnAllDefaultTargetsTest):
    ...
    def impl_setUp(self):
        ...
        self.testClient1 = self.target.start_client("ramses-test-client", "-tn 5 -ts 0 -cx 2 -cz 5")
        self.checkThatApplicationWasStarted(self.testClient1)
        self.addCleanup(self.target.kill_application, self.testClient1)
        self.testClient2 = self.target.start_client("ramses-test-client", "-tn 4 -ts 0 -cx -2 -cz 5")
    ...
    def impl_tearDown(self):
        ...
        self.save_application_output(self.testClient2, nr=2)
        self.save_application_output(self.testClient1, nr=1)
