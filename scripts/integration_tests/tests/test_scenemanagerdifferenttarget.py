...
class TestDaemonDifferentTarget(test_classes.MultipleConnectionsTest):

    def get_nr_targets(self):
        """
        this tests needs two connections
        first connection: daemon
        second connection: testclient and renderer
        """
        return 2
    ...
    def impl_setUp(self):
        ...
        self.testClient = self.targets[1].start_client("ramses-test-client", "--test-nr 5 --test-state 0 --cz 5", ramsesDaemonTarget=self.targets[0])
        self.checkThatApplicationWasStarted(self.testClient)
        self.addCleanup(self.targets[1].kill_application, self.testClient)

    def impl_tearDown(self):
        self.targets[1].kill_application(self.testClient)
        self.targets[1].kill_application(self.renderer)
        self.targets[0].kill_application(self.ramsesDaemon)
        log.info("all applications killed")
        self.save_application_output_on_target(self.testClient, self.targets[1])
        self.save_application_output_on_target(self.renderer, self.targets[1])
