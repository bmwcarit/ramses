...
# Tests if the system compositor controller can make a layer visible.
class TestSystemCompositorController(system_compositor_controller_base.SystemCompositorControllerBase):

    def impl_test(self):
        # Precondition: black background renderer visible
        ...
        # Make test layer containing both ivi-gears and renderer visible
        self.renderer.send_ramsh_command("sclv {0} 1".format(self.testLayer), waitForRendererConfirmation=True)

        ensureSystemCompositorRoundTrip(self.renderer, self.testSurfaceIVIIds["wlClient2"])
        # Postcondition: renderer and both gears visible
