...
class TestClass(test_classes.OnAllDefaultTargetsTest):
    ...
    def impl_setUp(self):
        ...
        self.renderer = self.target.start_default_renderer(args="--disableAutoMapping")
    ...
    def impl_test(self):
        ...
        #2. unmap scene, nothing is visible
        ...
        #3. unsubscribe scene, nothing is visible
        self.renderer.send_ramsh_command("unsubscribeScene -sceneId 26", waitForRendererConfirmation=True)
        self.validateScreenshot(self.renderer, "black.png")
        #show scene again then it has to get visible again
        self.renderer.showScene(26)
        self.validateScreenshot(self.renderer, "testClient_threeTriangles.png")
