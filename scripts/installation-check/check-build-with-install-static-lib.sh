...
echo "++++ Create test environment for install check of static lib (${RENDERER_PLATFORM}, ${GL_VERSION}) ++++"
...
cmake -DCMAKE_PREFIX_PATH=$INSTALL_DIR -DRAMSES_RENDERER_PLATFORM=${RENDERER_PLATFORM} -DRAMSES_VERSION=${RAMSES_VERSION} -DGL_VERSION=${GL_VERSION} --build test-cmake.config $SCRIPT_DIR/static-lib-check/
make

if [[ "$RENDERER_PLATFORM" == "LINUX" ]]; then
    env LD_LIBRARY_PATH=${INSTALL_DIR}/lib:${LD_LIBRARY_PATH} ./ramses-static-lib-check
fi
