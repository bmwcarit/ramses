...
echo "++++ Create test environment for install check of static lib (${RENDERER_PLATFORM}) ++++"
...
cmake -DCMAKE_PREFIX_PATH=$INSTALL_DIR -DRAMSES_RENDERER_PLATFORM=${RENDERER_PLATFORM} --build test-cmake.config $SCRIPT_DIR/static-lib-check/
make run-all
