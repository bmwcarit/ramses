...
static ramses::RamsesRenderer* CreateRenderer(ramses::RamsesFramework& fw, const ramses::RendererConfig& config)
{
    ...
    createdRenderer->m_impl.getDisplayDispatcher().injectPlatformFactory(std::make_unique<ramses_internal::PlatformFactoryNiceMock>());
...
TEST_F(ARamsesRendererWithDisplay, createsCommandForReadPixels)
{
    ...
    EXPECT_CALL(cmdVisitor, handleReadPixels(ramses_internal::DisplayHandle{ displayId.getValue() }, ramses_internal::OffscreenBufferHandle{ bufferId.getValue() }, 1u, 2u, 3u, 4u, false, std::string_view{}));
    cmdVisitor.visit(commandBuffer);
}
...
TEST(ARamsesRendererWithSeparateRendererThread, canNotifyPerWatchdog)
{
    ...
    ramses::RamsesFrameworkConfig frameworkConfig{ramses::EFeatureLevel_Latest};
    EXPECT_EQ(ramses::StatusOK, frameworkConfig.setWatchdogNotificationInterval(ramses::ERamsesThreadIdentifier::Renderer, 500));
