...
namespace ramses
{
    ...
    RamsesRendererImpl::RamsesRendererImpl(RamsesFrameworkImpl& framework, const RendererConfig& config)
        ...
        , m_displayDispatcher{ std::make_unique<ramses_internal::DisplayDispatcher>(std::make_unique<ramses_internal::PlatformFactory>(), config.m_impl.get().getInternalRendererConfig(), m_rendererFrameworkLogic, m_threadWatchdog) }
    ...
    status_t RamsesRendererImpl::systemCompositorTakeScreenshot(std::string_view fileName, int32_t screenIviId)
    {
        if (!m_systemCompositorEnabled)
        ...
        m_pendingRendererCommands.push_back(ramses_internal::RendererCommand::SCScreenshot{ screenIviId, ramses_internal::String{fileName} });
        return StatusOK;
    }
    ...
    status_t RamsesRendererImpl::dispatchEvents(IRendererEventHandler& rendererEventHandler)
    {
        ...
        for(const auto& event : m_tempRendererEvents)
        {
            ...
            case ramses_internal::ERendererEventType::ReadPixelsFromFramebufferFailed:
            {
                ...
                const auto eventResult = (event.eventType == ramses_internal::ERendererEventType::ReadPixelsFromFramebuffer ? ERendererEventResult::Ok : ERendererEventResult::Failed);
    ...
    ELoopMode RamsesRendererImpl::getLoopMode() const
    {
        ...
        case ramses_internal::ELoopMode::UpdateAndRender:
            return ELoopMode::UpdateAndRender;
        case ramses_internal::ELoopMode::UpdateOnly:
            return ELoopMode::UpdateOnly;
        }
