...
TEST(ARendererConfig, hasDefaultValuesUponConstruction)
{
    ...
    EXPECT_EQ(nullptr, config.m_impl.get().getBinaryShaderCache());

    const ramses_internal::RendererConfig& internalConfig = config.m_impl.get().getInternalRendererConfig();
...
TEST(ARendererConfig, canSetBinaryShaderCache)
{
    ...
    EXPECT_EQ(&cache, config.m_impl.get().getBinaryShaderCache());
}
...
TEST(ARendererConfig, setsAndGetsWaylandDisplay)
{
    ...
    EXPECT_EQ("xxx", config.m_impl.get().getInternalRendererConfig().getWaylandDisplayForSystemCompositorController());
}
