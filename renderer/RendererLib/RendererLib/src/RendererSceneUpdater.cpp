...
namespace ramses_internal
{
    ...
    bool RendererSceneUpdater::handleBufferCreateRequest(OffscreenBufferHandle buffer, uint32_t width, uint32_t height, uint32_t sampleCount, bool isDoubleBuffered, ERenderBufferType depthStencilBufferType)
    {
        bool success = false;
        if (!m_renderer.hasDisplayController())
        {
    ...
    void RendererSceneUpdater::handlePickEvent(SceneId sceneId, glm::vec2 coordsNormalizedToBufferSize)
    {
        ...
        const glm::ivec2 coordsInBufferSpace = { static_cast<int32_t>(std::lroundf((coordsNormalizedToBufferSize.x + 1.f) * buffer.viewport.width / 2.f)) ,
                                                static_cast<int32_t>(std::lroundf((coordsNormalizedToBufferSize.y + 1.f) * buffer.viewport.height / 2.f)) };
