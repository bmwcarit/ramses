...
TEST_F(ASceneExpirationMonitor, doesNotGenerateEventIfZeroLimit)
{
    ...
    expirationMonitor.onFlushApplied(scene2, FlushTime::InvalidTimestamp);
    expirationMonitor.checkExpiredScenes(currentFakeTime);
    expectNoEvent();
}
...
{
    expirationMonitor.onFlushApplied(scene1, FlushTime::Clock::time_point(currentFakeTime + std::chrono::hours(100)) + std::chrono::milliseconds(1));
    expirationMonitor.onRendered(scene1);
    expirationMonitor.checkExpiredScenes(currentFakeTime);
    expectNoEvent();
}
...
TEST_F(ASceneExpirationMonitor, evaluatesExpirationBasedOnLimitFromLastAppliedFlush)
{
    expirationMonitor.onFlushApplied(scene1, currentFakeTime + std::chrono::milliseconds(1));
    expirationMonitor.onFlushApplied(scene1, currentFakeTime + std::chrono::milliseconds(1));
    expirationMonitor.onFlushApplied(scene1, currentFakeTime + std::chrono::milliseconds(10000));
    expirationMonitor.onRendered(scene1);
...
{
    expirationMonitor.onFlushApplied(scene1, currentFakeTime + std::chrono::milliseconds(1));
    expirationMonitor.onFlushApplied(scene2, currentFakeTime + std::chrono::milliseconds(10000));
    expirationMonitor.onFlushApplied(scene3, currentFakeTime + std::chrono::milliseconds(1));
    expirationMonitor.onRendered(scene1);
    expirationMonitor.onRendered(scene2);
...
TEST_F(ASceneExpirationMonitor, generatesEventOnlyOnceIfSceneKeepsBeingExpired)
{
    ...
    {
        expirationMonitor.onFlushApplied(scene1, currentFakeTime + std::chrono::milliseconds(i) + std::chrono::milliseconds(1));
        expirationMonitor.onRendered(scene1);
...
TEST_F(ASceneExpirationMonitor, generatesRecoveryEventOnlyOnceWhenExceedingStopsAndKeepsBelowLimit)
{
    ...
    for (size_t i = 0; i < 5u; ++i)
    {
        expirationMonitor.onFlushApplied(scene1, currentFakeTime + std::chrono::milliseconds(i) + std::chrono::milliseconds(10000));
        expirationMonitor.onRendered(scene1);
...
{
    expirationMonitor.onFlushApplied(scene1, currentFakeTime + std::chrono::milliseconds(2));
...
{
    // flush 1
    expirationMonitor.onFlushApplied(scene1, FlushTime::Clock::now() - std::chrono::milliseconds(10) + std::chrono::milliseconds(1));

    // few cycles of scene 2
    expirationMonitor.onFlushApplied(scene2, FlushTime::Clock::now() - std::chrono::milliseconds(10) + std::chrono::milliseconds(1));
    expirationMonitor.onRendered(scene2);
    ...
    expirationMonitor.onFlushApplied(scene1, FlushTime::Clock::now() + std::chrono::milliseconds(10000));
    expirationMonitor.onFlushApplied(scene2, FlushTime::Clock::now() + std::chrono::milliseconds(10000));
    expirationMonitor.onRendered(scene1);
    expirationMonitor.onRendered(scene2);
    expirationMonitor.checkExpiredScenes(FlushTime::Clock::now());
...
TEST_F(ASceneExpirationMonitor, confidenceTest_severalScenesWithDifferentBehaviorAndLimits)
{
    ...
    expirationMonitor.onFlushApplied(scene1, FlushTime::Clock::now() - std::chrono::milliseconds(10) + std::chrono::milliseconds(10000));
    expirationMonitor.onFlushApplied(scene3, FlushTime::Clock::now() + std::chrono::milliseconds(10000));
    expirationMonitor.onFlushApplied(scene5, FlushTime::Clock::now() - std::chrono::milliseconds(10) + std::chrono::milliseconds(1));
    expirationMonitor.onRendered(scene2);
    ...
    expirationMonitor.onFlushApplied(scene4, FlushTime::Clock::now() - std::chrono::milliseconds(10) + std::chrono::milliseconds(1));
    expirationMonitor.onRendered(scene1);
    expirationMonitor.onRendered(scene3);
    ...
    expirationMonitor.onFlushApplied(scene5, FlushTime::Clock::now() + std::chrono::milliseconds(10000));
    expirationMonitor.onFlushApplied(scene1, FlushTime::Clock::now() - std::chrono::milliseconds(10) + std::chrono::milliseconds(10000));
    expirationMonitor.onRendered(scene2);
