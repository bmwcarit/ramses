...
namespace MockResourceHash
{
    ...
    static inline ManagedResource GetManagedResource(const ResourceContentHash& hash)
    {
        ...
        if (hash == EffectHash)
            res = std::make_unique<EffectResource>("", "", "", std::optional<EDrawMode>{}, EffectInputInformationVector(), EffectInputInformationVector(), "", ResourceCacheFlag_DoNotCache);
        else if (hash == VertArrayHash || hash == VertArrayHash2)
            res = std::make_unique<ArrayResource>(EResourceType_VertexArray, 0, EDataType::Float, nullptr, ResourceCacheFlag_DoNotCache, std::string_view{});
        else if (hash == IndexArrayHash || hash == IndexArrayHash2 || hash == IndexArrayHash3)
            res = std::make_unique<ArrayResource>(EResourceType_IndexArray, 0, EDataType::UInt16, nullptr, ResourceCacheFlag_DoNotCache, std::string_view{});
        else if (hash == TextureHash)
            res = std::make_unique<TextureResource>(EResourceType_Texture2D, TextureMetaInfo(1u, 1u, 1u, ETextureFormat::R8, false, {}, { 1u }), ResourceCacheFlag_DoNotCache, std::string_view{});
        else if (hash == TextureHash2)
            res = std::make_unique<TextureResource>(EResourceType_Texture2D, TextureMetaInfo(2u, 2u, 1u, ETextureFormat::R8, true, {}, { 4u }), ResourceCacheFlag_DoNotCache, std::string_view{});
