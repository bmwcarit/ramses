...
// but needs explicit enabling for Desktop GL
    ...
    DeviceResourceHandle Device_GL::uploadStreamTexture2D(DeviceResourceHandle handle, UInt32 width, UInt32 height, ETextureFormat format, const uint8_t* data, const TextureSwizzleArray& swizzle)
    {
        if (!handle.isValid())
        {
    ...
    void Device_GL::uploadTextureMipMapData(UInt32 mipLevel, UInt32 x, UInt32 y, UInt32 z, UInt32 width, UInt32 height, UInt32 depth, const GLTextureInfo& texInfo, const uint8_t *pData, UInt32 dataSize) const
    {
    ...
    DeviceResourceHandle Device_GL::uploadBinaryShader(const EffectResource& shader, const uint8_t* binaryShaderData, UInt32 binaryShaderDataSize, BinaryShaderFormatID binaryShaderFormat)
    {
        ShaderProgramInfo programInfo;
        std::string debugErrorLog;
    ...
    void Device_GL::readPixels(uint8_t* buffer, UInt32 x, UInt32 y, UInt32 width, UInt32 height)
    {
