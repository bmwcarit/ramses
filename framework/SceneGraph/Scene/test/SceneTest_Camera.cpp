...
namespace ramses_internal
{
    ...
    TYPED_TEST(AScene, CountsItsCamerasCorrectly)
    {
        ...
        AllocCamera(ECameraProjectionType::Renderer, this->m_scene);
        AllocCamera(ECameraProjectionType::Renderer, this->m_scene);
    ...
    {
        AllocCamera(ECameraProjectionType::Renderer, this->m_scene);
        const CameraHandle camera2 = AllocCamera(ECameraProjectionType::Renderer, this->m_scene);
    ...
    TYPED_TEST(AScene, GetsAssignedCameraNode)
    {
        ...
        const CameraHandle camera = this->m_scene.allocateCamera(ECameraProjectionType::Renderer, node, AllocDummyDataInstance(this->m_scene));
    ...
    TYPED_TEST(AScene, GetsAssignedDataInstance)
    {
        ...
        const CameraHandle camera = this->m_scene.allocateCamera(ECameraProjectionType::Renderer, this->m_scene.allocateNode(), dataInst);
        EXPECT_EQ(dataInst, this->m_scene.getCamera(camera).dataInstance);
    }

    TYPED_TEST(AScene, CreatesARendererTypeCameraByDefault)
    {
        const CameraHandle camera = AllocCamera(ECameraProjectionType::Renderer, this->m_scene);
        EXPECT_EQ(ECameraProjectionType::Renderer, this->m_scene.getCamera(camera).projectionType);
    }
