...
{
    Image::Image(UInt32 width, UInt32 height, std::vector<uint8_t>&& data)
        : m_width(width)
        , m_height(height)
    ...
    Image Image::createEnlarged(UInt32 width, UInt32 height, std::array<uint8_t, 4> fillValue) const
    {
    ...
    UInt32 Image::getNumberOfNonBlackPixels(uint8_t maxDiffPerColorChannel) const
    {
        UInt32 result = 0;
        for (size_t px = 0; px < m_data.size() / 4; ++px)
        {
